...
double MothurFisher::fexact(double n11_, double n12_, double n21_, double n22_, string o) {
	try {
		...
		if (app.isDebug) { LOG(DEBUG) << "fisher:fexact n11_, n1_, n_1, n " + toString(n11_) + " " + toString(n1_) + " " + toString(n_1) + " " + toString(n) + " \n"; }
		...
		LOG(FATAL) << e.what() << " in MothurFisher, fexact";
		exit(1);
	}
}
/***********************************************************/
double MothurFisher::lngamm(double z) {
	...
	try {
		...
		LOG(FATAL) << e.what() << " in MothurFisher, lngamm";
		exit(1);
	}
}
...
double MothurFisher::lnfact(double n) {
	try {
		...
		LOG(FATAL) << e.what() << " in MothurFisher, lnfact";
		exit(1);
	}
}
/***********************************************************/
double MothurFisher::lnbico(double n, double k) {
	try {
		...
		LOG(FATAL) << e.what() << " in MothurFisher, lnbico";
		exit(1);
	}
}
/***********************************************************/
double MothurFisher::hyper_323(double n11, double n1_, double n_1, double n) {
	try {
		...
		LOG(FATAL) << e.what() << " in MothurFisher, hyper_323";
		exit(1);
	}
}
/***********************************************************/
double MothurFisher::myhyper(double n11) {
	try {
		...
		LOG(FATAL) << e.what() << " in MothurFisher, myhyper";
		exit(1);
	}
}
/***********************************************************/
double MothurFisher::hyper0(double n11i, double n1_i, double n_1i, double ni) {
	try {
		...
		LOG(FATAL) << e.what() << " in MothurFisher, hyper0";
		exit(1);
	}
}
/***********************************************************/
double MothurFisher::exact(double n11, double n1_, double n_1, double n) {
	try {
		...
		for (i = min + 1; p < 0.99999999*prob; i++)
		{
			...
			p = myhyper(i);
			if (i > max) {
				LOG(WARNING) << "i value too high. Take a closer look at the pvalue for " + otuLabel + ".\n";
				break;
			}
		}
		i--;
		...
		for (j = max - 1; p < 0.99999999*prob; j--)
		{
			...
			p = myhyper(j);
			if (j < 0) {
				LOG(WARNING) << "j value too low. Take a closer look at the pvalue for " + otuLabel + ".\n";
				break;
			}
		}
		j++;
		...
		LOG(FATAL) << e.what() << " in MothurFisher, exact";
		exit(1);
	}
}
/***********************************************************/
