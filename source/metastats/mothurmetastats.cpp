...
MothurMetastats::MothurMetastats(double t, int n) {
	try {
		...
		LOG(FATAL) << e.what() << " in MothurMetastats, MothurMetastats";
		exit(1);
	}
}
/***********************************************************/
	...
	try {
		...
		if (column == 2) {  //# then we have a two sample comparison
			...
			for (int i = 0; i < row; i++) {
				...
				MothurFisher fisher;
				double pre = fisher.fexact(f11, f12, f21, f22, settings.currentSharedBinLabels[i]);
		   ...
		   //#*************************************
			...
			if (app.isDebug) { for (int i = 0; i < row; i++) { LOG(DEBUG) << "" + settings.currentSharedBinLabels[i] + " pvalue = " + toString(pvalues[i]) + "\n"; } }
			...
			for (int i = 0; i < row; i++) {
				...
				if ((fish[i] < secondGroupingStart) && (fish2[i] < (column - secondGroupingStart))) {
					...
					MothurFisher fisher;
					if (app.isDebug) { LOG(DEBUG) << "about to run fisher for Otu " + settings.currentSharedBinLabels[i] + " F11, F12, F21, F22 = " + toString(f11) + " " + toString(f12) + " " + toString(f21) + " " + toString(f22) + " " + "\n"; }
					double pre = fisher.fexact(f11, f12, f21, f22, settings.currentSharedBinLabels[i]);
					if (app.isDebug) { LOG(DEBUG) << "about to completed fisher for Otu " + settings.currentSharedBinLabels[i] + " pre = " + toString(pre) + "\n"; }
		...
		for (int i = 0; i < row; i++) {
			...
			if (i < settings.currentSharedBinLabels.size()) { out << settings.currentSharedBinLabels[i] << '\t'; }
			else { out << (i + 1) << '\t'; }
		...
		LOG(FATAL) << e.what() << " in MothurMetastats, runMetastats";
		exit(1);
	}
}
/***********************************************************/
vector<double> MothurMetastats::permuted_pvalues(vector< vector<double> >& Imatrix, vector<double>& tstats, vector< vector<double> >& Fmatrix) {
	try {
		...
		LOG(FATAL) << e.what() << " in MothurMetastats, permuted_pvalues";
		exit(1);
	}
}
/***********************************************************/
vector<double> MothurMetastats::permute_and_calc_ts(vector< vector<double> >& Imatrix) {
	try {
		...
		LOG(FATAL) << e.what() << " in MothurMetastats, permuted_ttests";
		exit(1);
	}
}
/***********************************************************/
int MothurMetastats::OrderPValues(int low, int high, vector<double>& p, vector<int>& order) {
	try {
		...
		LOG(FATAL) << e.what() << " in MothurMetastats, OrderPValues";
		exit(1);
	}
}
/***********************************************************/
int MothurMetastats::swapElements(int i, int j, vector<double>& p, vector<int>& order) {
	try {
		...
		LOG(FATAL) << e.what() << " in MothurMetastats, swapElements";
		exit(1);
	}
}
...
vector<int> MothurMetastats::getSequence(int start, int end, int length) {
	try {
		...
		LOG(FATAL) << e.what() << " in MothurMetastats, getSequence";
		exit(1);
	}
}
/***********************************************************/
