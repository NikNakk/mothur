		...
		if (userGroups[0] != "all") {
			...
			for (int i = 0; i < userGroups.size(); i++) {
				if (isValidGroup(userGroups[i], allGroups) != true) {
					LOG(INFO) << userGroups[i] + " is not a valid group, and will be disregarded." << '\n';
			...
			if (userGroups.size() == 0) {
				LOG(INFO) << "You provided no valid groups. I will run the command using all the groups in your file." << '\n';
	...
	try {
		...
		if (mode == "weighted") {
			...
			if ((userGroups.size() == 0) && (numGroups == 0)) {
				LOG(INFO) << "When using the groups parameter you must have at least 1 valid group. I will run the command using all the groups in your groupfile." << '\n';
				for (int i = 0; i < allGroups.size(); i++) {
					if (allGroups[i] != "xxx") {
						userGroups.push_back(allGroups[i]);
					}
		...
		LOG(FATAL) << e.what() << " in SharedUtil, setGroups";
		exit(1);
	}
}
/**************************************************************************************/
void SharedUtil::getCombos(vector<string>& groupComb, vector<string> userGroups, int& numComp) { //groupcomb, globaldata->Groups, numcomb
	try {
		...
		LOG(FATAL) << e.what() << " in SharedUtil, getCombos";
		exit(1);
	}
}
/**************************************************************************************/
