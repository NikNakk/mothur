...
int Rarefact::driver(RarefactionCurveData* rcd, int increment, int nIters = 1000) {
	...
	for (int iter = 0;iter < nIters;iter++) {
		...
		for (int i = 0;i < numSeqs;i++) {
			...
			if (ctrlc_pressed) { delete lookup; delete rank; delete rcd; return 0; }
...
int Rarefact::createProcesses(vector<int>& procIters, RarefactionCurveData* rcd, int increment, int nIters) {
		...
		if (pid > 0) {
			...
			for (int i = 0;i < processIDS.size();i++) {
				for (int j = 0;j < displays.size();j++) {
					File::remove(toString(processIDS[i]) + toString(j) + ".rarefact.temp");
				}
			}
			recalc = true;
			break;
	...
	if (recalc) {
		...
		//for (int i = 0; i < processIDS.size(); i++) { kill (processIDS[i], SIGINT); } for (int i=0;i<processIDS.size();i++) { int temp = processIDS[i]; wait(&temp); } ctrlc_pressed = false;  for (int i=0;i<processIDS.size();i++) {for(int j=0;j<displays.size();j++){File::remove(toString(processIDS[i]) + toString(j) + ".rarefact.temp");}}processors=3; LOG(LOGERROR) << "unable to spawn the number of processes you requested, reducing number to " + toString(processors) + "\n";
	...
	try {
		...
		for (int iter = 0;iter < nIters;iter++) {
			...
			for (int k = 0; k < lookup.size(); k++) {
				if (ctrlc_pressed) { delete merge; delete rcd; return 0; }
		...
		LOG(FATAL) << e.what() << " in Rarefact, getSharedCurve";
		exit(1);
	}
}
...
void Rarefact::mergeVectors(SharedRAbundVector* shared1, SharedRAbundVector* shared2) {
	try {
		...
		LOG(FATAL) << e.what() << " in Rarefact, mergeVectors";
		exit(1);
	}
}
