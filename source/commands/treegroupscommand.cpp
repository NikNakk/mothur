...
vector<string> TreeGroupCommand::setParameters() {
	try {
		...
		LOG(FATAL) << e.what() << " in TreeGroupCommand, setParameters";
		exit(1);
	}
}
//**********************************************************************************************************************
string TreeGroupCommand::getHelpString() {
	try {
		...
		ValidCalculators validCalculator
			"The tree.shared command creates a .tre to represent the similarity between groups or sequences.\n"
			"The tree.shared command parameters are shared, groups, calc, phylip, column, name, cutoff, precision, processors, subsample, iters and label.\n"
			"The groups parameter allows you to specify which of the groups in your groupfile you would like included used.\n"
			"The group names are separated by dashes. The label allow you to select what distance levels you would like trees created for, and are also separated by dashes.\n"
			"The phylip or column parameter are required if you do not provide a sharedfile, and only one may be used.  If you use a column file the name filename is required. \n"
			"If you do not provide a cutoff value 10.00 is assumed. If you do not provide a precision value then 100 is assumed.\n"
			"The tree.shared command should be in the following format: tree.shared(groups=yourGroups, calc=yourCalcs, label=yourLabels).\n"
			"The iters parameter allows you to choose the number of times you would like to run the subsample.\n"
			"The subsample parameter allows you to enter the size pergroup of the sample or you can set subsample=T and mothur will use the size of your smallest group. The subsample parameter may only be used with a shared file.\n"
			"Example tree.shared(groups=A-B-C, calc=jabund-sorabund).\n"
			"The default value for groups is all the groups in your groupfile.\n"
			"The default value for calc is jclass-thetayc.\n"
			"The tree.shared command outputs a .tre file for each calculator you specify at each distance you choose.\n";
		helpString += validCalculator.printCalc("treegroup")
			"Or the tree.shared command can be in the following format: tree.shared(phylip=yourPhylipFile).\n"
			"Example tree.shared(phylip=abrecovery.dist).\n"
			"Note: No spaces between parameter labels (i.e. groups), '=' and parameters (i.e.yourGroups).\n";
		return helpString;
	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in TreeGroupCommand, getHelpString";
		exit(1);
	}
}
//**********************************************************************************************************************
...
//**********************************************************************************************************************
TreeGroupCommand::TreeGroupCommand(Settings& settings) : Command(settings) {
	try {
		...
		LOG(FATAL) << e.what() << " in TreeGroupCommand, TreeGroupCommand";
		exit(1);
	}
}
//**********************************************************************************************************************

TreeGroupCommand::TreeGroupCommand(Settings& settings, string option) : Command(settings, option) {
	...
	if (option == "help") { help(); abort = true; calledHelp = true; }
		...
		else { inputfile = phylipfile;  format = "phylip"; settings.setCurrent("phylip", phylipfile); }

		columnfile = validParameter.validFile(parameters, "column", true);
		if (columnfile == "not open") { columnfile = ""; abort = true; }
		else if (columnfile == "not found") { columnfile = ""; }
		else { inputfile = columnfile; format = "column";	settings.columnfileetCurrent("column", $2); }

		sharedfile = validParameter.validFile(parameters, "shared", true);
		if (sharedfile == "not open") { sharedfile = ""; abort = true; }
		else if (sharedfile == "not found") { sharedfile = ""; }
		else { inputfile = sharedfile; format = "sharedfile";	settings.setCurrent("shared", sharedfile); }
		...
		if ((phylipfile == "") && (columnfile == "") && (sharedfile == "")) {
			...
			if (sharedfile != "") { inputfile = sharedfile; format = "sharedfile"; LOG(INFO) << "Using " + sharedfile + " as input file for the shared parameter." << '\n'; }
				...
				if (columnfile != "") { inputfile = columnfile; format = "column";  LOG(INFO) << "Using " + columnfile + " as input file for the column parameter." << '\n'; }
				else {
					phylipfile = settings.getCurrent("phylip");
					if (phylipfile != "") { inputfile = phylipfile;  format = "phylip";  LOG(INFO) << "Using " + phylipfile + " as input file for the phylip parameter." << '\n'; }
					else {
						LOG(INFO) << "No valid current files. You must provide a shared, phylip or column file." << '\n';
						abort = true;
					}
				}
			}
		}
		else if ((phylipfile != "") && (columnfile != "")) { LOG(INFO) << "When running the tree.shared command with a distance file you may not use both the column and the phylip parameters." << '\n'; abort = true; }
		...
		if (subsample && (format != "sharedfile")) { LOG(LOGERROR) << "the subsample parameter can only be used with a shared file.\n"; abort = true; }
...
int TreeGroupCommand::execute() {
	...
	if (format == "sharedfile") {
		...
		if (treeCalculators.size() == 0) { LOG(INFO) << "You have given no valid calculators." << '\n'; return 0; }
...
Tree* TreeGroupCommand::createTree(vector< vector<double> >& simMatrix) {
	...
	if (ctrlc_pressed) { delete t; t = NULL; return t; }
...
int TreeGroupCommand::writeTree(string out, Tree* T) {
	...
	if (ctrlc_pressed) { File::remove(out); outputNames.pop_back(); return 1; }
	...
	for (int i = 0; i < matrix->seqVec.size(); i++) {
		for (int j = 0; j < matrix->seqVec[i].size(); j++) {
			...
			if (i < matrix->seqVec[i][j].index) {
				...
				if (ctrlc_pressed) { return simMatrix; }
			}
		}
	}
...
int TreeGroupCommand::makeSimsShared() {
	...
	while ((lookup[0] != NULL) && ((allLines == 1) || (userLabels.size() != 0))) {
		if (ctrlc_pressed) { for (int i = 0; i < lookup.size(); i++) { delete lookup[i]; } for (int i = 0; i < treeCalculators.size(); i++) { delete treeCalculators[i]; } return 1; }
...
int TreeGroupCommand::process(vector<SharedRAbundVector*> thisLookup) {
	try {
		...
		for (int thisIter = 0; thisIter < iters; thisIter++) {
			...
			if (processors == 1) {
				...
				while (process != processors) {
					...
					if (pid > 0) {
						...
						string tempdistFileName = File::getRootName(File::getSimpleName(sharedfile)) + m->mothurGetpid(process) + ".dist";
						ofstream outtemp;
						...
						ctrlc_pressed = false;
						for (int i = 0;i < processIDS.size();i++) {
							File::remove(File::getRootName(File::getSimpleName(sharedfile)) + (toString(processIDS[i]) + ".dist"));
						}
						recalc = true;
						break;
					}
				...
				if (recalc) {
					...
					//for (int i = 0; i < processIDS.size(); i++) { kill (processIDS[i], SIGINT); } for (int i=0;i<processIDS.size();i++) { int temp = processIDS[i]; wait(&temp); } ctrlc_pressed = false;  for (int i=0;i<processIDS.size();i++) {File::remove(File::getRootName(File::getSimpleName(sharedfile)) + (toString(processIDS[i]) + ".dist"));}processors=3; LOG(LOGERROR) << "unable to spawn the number of processes you requested, reducing number to " + toString(processors) + "\n";
				...
				for (int i = 0; i < processIDS.size(); i++) {
					string tempdistFileName = File::getRootName(File::getSimpleName(sharedfile)) + toString(processIDS[i]) + ".dist";
					ifstream intemp;
			...
			if (app.isDebug) { LOG(DEBUG) << "iter = " + toString(thisIter) + ".\n"; }
		}

		if (app.isDebug) { LOG(DEBUG) << "done with iters.\n"; }

		if (iters != 1) {
			//we need to find the average distance and standard deviation for each groups distance
			vector< vector<seqDist>  > calcAverages = m->getAverages(calcDistsTotals);

			if (app.isDebug) { LOG(DEBUG) << "found averages.\n"; }
			...
			if (app.isDebug) { LOG(DEBUG) << "done averages trees.\n"; }
			...
			for (int i = 0; i < calcDists.size(); i++) {
				...
				outAll.close();
				if (ctrlc_pressed) { for (int k = 0; k < trees.size(); k++) { delete trees[k]; } }

				if (app.isDebug) { LOG(DEBUG) << "done all trees.\n"; }

				Consensus consensus;
				//clear old tree names if any
				m->Treenames.clear(); m->Treenames = settings.getGroups(); //may have changed if subsample eliminated groups
				Tree* conTree = consensus.getTree(trees);

				if (app.isDebug) { LOG(DEBUG) << "done cons tree.\n"; }
		...
		LOG(FATAL) << e.what() << " in TreeGroupCommand, process";
		exit(1);
	}
}
/**************************************************************************************************/
int TreeGroupCommand::driver(vector<SharedRAbundVector*> thisLookup, int start, int end, vector< vector<seqDist> >& calcDists) {
	try {
		...
		LOG(FATAL) << e.what() << " in TreeGroupCommand, driver";
		exit(1);
	}
}
/***********************************************************/
