...
string SRACommand::getHelpString() {
	string helpString = "The make.sra command creates the necessary files for a NCBI submission. The xml file and individual sff or fastq files parsed from the original sff or fastq file.\n"
		"The make.sra command parameters are: sff, fastq, file, oligos, project, mimarksfile, pdiffs, bdiffs, ldiffs, sdiffs, tdiffs, checkorient, platform, orientation, libstrategy, datatype, libsource, libselection and instrument.\n"
		"The sff parameter is used to provide the original sff file.\n"
		"The fastq parameter is used to provide the original fastq file.\n"
		"The project parameter is used to provide your project file.\n"
		"The oligos parameter is used to provide an oligos file to parse your sff or fastq file by. It is required and must contain barcodes and primers, or you must provide a file option. \n"
		"The mimark parameter is used to provide your mimarks file.  You can create the template for this file using the get.mimarkspackage command.\n"
		...
		"make.sra(sff=sff=GHL4YHV01.sff, GHL4YHV01.oligos, project=test.project, mimark=MIMarksData.txt)\n";
	return helpString;
}
//**********************************************************************************************************************
...
//**********************************************************************************************************************
SRACommand::SRACommand(Settings& settings) : Command(settings) {
...
//**********************************************************************************************************************
SRACommand::SRACommand(Settings& settings, string option) : Command(settings, option) {
	...
	if (option == "help") { help(); abort = true; calledHelp = true; }
		...
		else { settings.setCurrent("oligos", oligosfile); setOligosParameter = true; }

		contactfile = validParameter.validFile(parameters, "project", true);
		if (contactfile == "not found") { contactfile = ""; LOG(LOGERROR) << "You must provide a project file before you can use the sra command." << '\n'; abort = true; }
		else if (contactfile == "not open") { abort = true; }

		mimarksfile = validParameter.validFile(parameters, "mimark", true);
		if (mimarksfile == "not found") { mimarksfile = ""; LOG(LOGERROR) << "You must provide a mimark file before you can use the sra command. You can create a template for this file using the get.mimarkspackage command." << '\n'; abort = true; }
		...
		if ((file == "") && (oligosfile == "")) {
			LOG(LOGERROR) << "You must provide an oligos file or file with oligos files in them before you can use the sra command." << '\n'; abort = true;
		}

		if ((fastqfile == "") && (file == "") && (sfffile == "")) {
			LOG(LOGERROR) << "You must provide a file, sff file or fastq file before you can use the sra command." << '\n'; abort = true;
		}
		...
		else { LOG(LOGERROR) << "" + orientation + " is not a valid orientation option. Choices are: forward and reverse.\n" << '\n'; abort = true; }
...
int SRACommand::execute() {
	...
	readContactFile();
	if (app.isDebug) { LOG(DEBUG) << "read contact file.\n"; }
	readMIMarksFile();
	if (app.isDebug) { LOG(DEBUG) << "read mimarks file.\n"; }
	if (oligosfile != "") { readOligos(); }
	if (app.isDebug) { LOG(DEBUG) << "read oligos file.\n"; }
	...
	if (app.isDebug) { LOG(DEBUG) << "finished sanity check.\n"; }
	...
	ofstream outT;
	File::openOutputFile(blankFile, outT); outT.close();
	...
	for (int i = 0; i < Groups.size(); i++) {
		...
		for (int j = 0; j < thisGroupsFiles.size(); j++) {
			string libId = File::getSimpleName(thisGroupsFiles[j]) + "." + Groups[i];
			...
			if (libLayout == "paired") { //adjust the libID because the thisGroupsFiles[j] contains two filenames
				...
				libId = File::getSimpleName(pieces[0]) + "." + Groups[i];
				out << "\t\t\t<File file_path=\"" + File::getSimpleName(pieces[0]) + "\">\n";
				out << "\t\t\t\t<DataType>generic-data</DataType> \n";
				out << "\t\t\t</File>\n";
				out << "\t\t\t<File file_path=\"" + File::getSimpleName(pieces[1]) + "\">\n";
				out << "\t\t\t\t<DataType>generic-data</DataType> \n";
				out << "\t\t\t</File>\n";
				...
				out << "\t\t\t<File file_path=\"" + File::getSimpleName(thisGroupsFiles[j]) + "\">\n";
				out << "\t\t\t\t<DataType>generic-data</DataType> \n";
				out << "\t\t\t</File>\n";
				//attributes
...
int SRACommand::readContactFile() {
	...
	ifstream in;
	File::openInputFile(contactfile, in);
	...
	while (!in.eof()) {
		...
		string key, value;
		in >> key; File::gobble(in);
		value = File::getline(in); File::gobble(in);
		...
		if (key == "USERNAME") { submissionName = value; }
			...
			else { LOG(LOGERROR) << "" + centerType + " is not a center type option.  Valid center type options are consortium, center, institute and lab. This is a controlled vocabulary section in the XML file that will be generated." << '\n'; ctrlc_pressed = true; }
		}
		else if (key == "OWNERSHIP") {
			ownership = value;
			for (int i = 0; i < centerType.length(); i++) { centerType[i] = tolower(ownership[i]); }
			if ((ownership == "owner") || (ownership == "participant")) {}
			else { LOG(LOGERROR) << "" + ownership + " is not a ownership option.  Valid ownership options are owner or participant. This is a controlled vocabulary section in the XML file that will be generated." << '\n'; ctrlc_pressed = true; }
		}
			...
			for (int i = 0; i < values.size(); i++) {
				...
				if (items.size() != 2) { LOG(LOGERROR) << "error parsing grant info for line \"" + value + "\", skipping it.\n"; break; }
				else {
					...
					else { LOG(LOGERROR) << "unknown identifier '" + items[0] + "', skipping it.\n"; }
				}
			}
			if ((thisGrant.grantId == "") || (thisGrant.grantAgency == "")) { LOG(LOGERROR) << "Missing info for line \"" + value + "\", skipping it. Note: the id and agency fields are required. Example: Grant id=yourID, agency=yourAgency.\n"; }
	...
	if (lastName == "") { LOG(LOGERROR) << "missing last name from project file, quitting." << '\n'; ctrlc_pressed = true; }
	if (firstName == "") { LOG(LOGERROR) << "missing first name from project file, quitting." << '\n'; ctrlc_pressed = true; }
	if (submissionName == "") { LOG(LOGERROR) << "missing submission name from project file, quitting." << '\n'; ctrlc_pressed = true; }
	if (email == "") { LOG(LOGERROR) << "missing email from project file, quitting." << '\n'; ctrlc_pressed = true; }
	if (centerName == "") { LOG(LOGERROR) << "missing center name from project file, quitting." << '\n'; ctrlc_pressed = true; }
	if (centerType == "") { LOG(LOGERROR) << "missing center type from project file, quitting." << '\n'; ctrlc_pressed = true; }
	if (description == "") { LOG(LOGERROR) << "missing description from project file, quitting." << '\n'; ctrlc_pressed = true; }
	if (projectTitle == "") { LOG(LOGERROR) << "missing project title from project file, quitting." << '\n'; ctrlc_pressed = true; }
	if (projectName == "") { LOG(LOGERROR) << "missing project name from project file, quitting." << '\n'; ctrlc_pressed = true; }
...
int SRACommand::readMIMarksFile() {
	...
	ifstream in;
	File::openInputFile(mimarksfile, in);
	...
	while (!in.eof()) {
		...
		if (ctrlc_pressed) { break; }
		temp = File::getline(in); File::gobble(in);

		if (app.isDebug) { LOG(DEBUG) << "" + temp + "\n"; }
	...
	for (int i = 0; i < headers.size(); i++) {
		...
		if (app.isDebug) { LOG(DEBUG) << "" + headers[i] + "\n"; }
	}

	if (app.isDebug) { LOG(DEBUG) << "packageType = '" + packageType + "'\n"; }
	...
	if (!m->isSubset(headers, requiredFieldsForPackage)) {
		...
		LOG(LOGERROR) << "missing required fields for package, please correct. Required fields are " + requiredFields + ".\n"; ctrlc_pressed = true; in.close(); return 0;


	}

	//if (app.isDebug) {  LOG(DEBUG) << "chooseAtLeastOneForPackage.size() = " + toString(chooseAtLeastOneForPackage.size()) + "\n";   }
	...
	//if (!m->inUsersGroups(chooseAtLeastOneForPackage, headers)){ //returns true if any of the choose at least ones are in headers
		...
		//LOG(LOGERROR) << "missing a choose at least one fields for the package, please correct. These are marked with '**'. Required fields are " + requiredFields + ".\n"; ctrlc_pressed = true; in.close(); return 0;
   // }
	...
	while (!in.eof()) {
		...
		if (linePieces.size() != headers.size()) { LOG(LOGERROR) << "line: " + original + " contains " + toString(linePieces.size()) + " columns, but you have " + toString(headers.size()) + " column headers, please correct.\n"; ctrlc_pressed = true; }
		else {
			...
			if (it == mimarks.end()) {
				...
				for (map<string, map<string, string> >::iterator it2 = mimarks.begin(); it2 != mimarks.end(); it2++) {
					...
					if (allSame) { LOG(LOGERROR) << "" + linePieces[0] + " is a duplicate sample to " + it2->first + ". It has all the same attributes in the MIMarks file. Samples must have distinguishing features to be uploaded to the NCBI library, please correct.\n"; ctrlc_pressed = true; isOkaySample = false; }
				}
				if (isOkaySample) { mimarks[linePieces[0]] = categories; }
			}
			else {
				LOG(LOGERROR) << "" + linePieces[0] + " is a duplicate sampleName. Sample names must be unique, please correct.\n"; ctrlc_pressed = true;
			}
		}
	}
	in.close();
...
int SRACommand::readFile(map<string, vector<string> >& files) {
	...
	while (!in.eof()) {
		...
		if (pieces.size() == 2) {
			...
			group = pieces[0];
			if (setOligosParameter) { LOG(LOGERROR) << "You cannot have an oligosfile and 3 column file option at the same time. Aborting. \n"; ctrlc_pressed = true; }
			if ((thisFileName2 != "none") && (thisFileName2 != "NONE")) { if (!using3NONE) { libLayout = "paired"; } else { LOG(LOGERROR) << "You cannot have a 3 column file with paired and unpaired files at the same time. Aborting. \n"; ctrlc_pressed = true; } }
			else { thisFileName2 = ""; libLayout = "single"; using3NONE = true; }
		}
		else if (pieces.size() == 4) {
			if (!setOligosParameter) { LOG(LOGERROR) << "You must have an oligosfile with the index file option. Aborting. \n"; ctrlc_pressed = true; }
			thisFileName1 = pieces[0];
			thisFileName2 = pieces[1];
			findex = pieces[2];
			rindex = pieces[3];
			...
			LOG(LOGERROR) << "file lines can be 2, 3 or 4 columns. The 2 column files are sff file then oligos or fastqfile then oligos or ffastq and rfastq. You may have multiple lines in the file.  The 3 column files are for paired read libraries. The format is groupName, forwardFastqFile reverseFastqFile. Four column files are for inputting file pairs with index files. Example: My.forward.fastq My.reverse.fastq NONE My.rindex.fastq. The keyword NONE can be used when there is not a index file for either the forward or reverse file.\n"; ctrlc_pressed = true;
		}

		if (app.isDebug) { LOG(DEBUG) << "group = " + group + ", thisFileName1 = " + thisFileName1 + ", thisFileName2 = " + thisFileName2 + ".\n"; }

		if (inputDir != "") {
			string path = File::getPath(thisFileName1);
			if (path == "") { thisFileName1 = inputDir + thisFileName1; }

			if (thisFileName2 != "") {
				path = File::getPath(thisFileName2);
		...
		ifstream in2;
		int openForward = File::openInputFile(thisFileName1, in2, "noerror");

		//if you can't open it, try default location
		if (openForward == 1) {

			if (settings.getDefaultPath() != "") { //default path is set
				string tryPath = settings.getDefaultPath() + File::getSimpleName(thisFileName1);
				LOG(INFO) << "Unable to open " + thisFileName1 + ". Trying default " + tryPath << '\n';
				ifstream in3;
		...
		if (openForward == 1) {
			if (settings.getOutputDir() != "") { //default path is set
				string tryPath = settings.getOutputDir() + File::getSimpleName(thisFileName1);
				LOG(INFO) << "Unable to open " + thisFileName1 + ". Trying output directory " + tryPath << '\n';
				ifstream in4;
		...
		if (openForward == 1) { //can't find it
			LOG(WARNING) << "can't find " + thisFileName1 + ", ignoring.\n";
		}
		else { in2.close(); }
		...
		if (thisFileName2 != "") {
			ifstream in3;
			openReverse = File::openInputFile(thisFileName2, in3, "noerror");

			//if you can't open it, try default location
			if (openReverse == 1) {
				if (settings.getDefaultPath() != "") { //default path is set
					string tryPath = settings.getDefaultPath() + File::getSimpleName(thisFileName2);
					LOG(INFO) << "Unable to open " + thisFileName2 + ". Trying default " + tryPath << '\n';
					ifstream in3;
			...
			if (openReverse == 1) {
				if (settings.getOutputDir() != "") { //default path is set
					string tryPath = settings.getOutputDir() + File::getSimpleName(thisFileName2);
					LOG(INFO) << "Unable to open " + thisFileName2 + ". Trying output directory " + tryPath << '\n';
					ifstream in4;
			...
			if (openReverse == 1) { //can't find it
				LOG(WARNING) << "can't find " + thisFileName2 + ", ignoring pair.\n";
			}
			else { in3.close(); }
		}
		...
		if ((pieces.size() == 2) && (openForward != 1) && (openReverse != 1)) { //good pair and sff or fastq and oligos
			...
			if (!setOligosParameter) {
				...
				if (pos != string::npos) {//these files are sff files
					...
					if (app.isDebug) { LOG(DEBUG) << "about to read oligos\n"; }
					readOligos();
					if (app.isDebug) { LOG(DEBUG) << "about to parse\n"; }
					parseSffFile(files);
					if (app.isDebug) { LOG(DEBUG) << "done parsing " + sfffile + "\n"; }
				}
				else {
					fileOption = 2;
					isSFF = false;
					...
					if (app.isDebug) { LOG(DEBUG) << "done parsing " + fastqfile + "\n"; }
				}
			}
	...
	if (runParseFastqFile) {
		...
		LOG(INFO) << '\n' << "/******************************************/" << '\n';
		LOG(INFO) << "Running command: fastq.info(" + commandString + ")" << '\n';
		...
		else { ctrlc_pressed = true; } // error in sffinfo
...
int SRACommand::parseSffFile(map<string, vector<string> >& files) {
	...
	LOG(INFO) << '\n' << "/******************************************/" << '\n';
	LOG(INFO) << "Running command: sffinfo(" + commandString + ")" << '\n';
	m->mothurCalling = true;
...
int SRACommand::mapGroupToFile(map<string, vector<string> >& files, vector<string> theseFiles) {
	for (int i = 0; i < Groups.size(); i++) {
		...
		for (int j = 0; j < theseFiles.size(); j++) {
			...
			string tempName = File::getSimpleName(theseFiles[j]);
...
int SRACommand::fixMap(map<string, vector<string> >& files) {
	for (map<string, vector<string> >::iterator it = files.begin(); it != files.end(); it++) {
		...
		if (theseFiles.size() != 2) { LOG(LOGERROR) << "unexpected number of files, quitting. \n."; ctrlc_pressed = true; }
		...
		for (int j = 0; j < theseFiles.size(); j++) {
			...
			if (pos != string::npos) { //you have a potential match for the forward file
				...
				if (pos != string::npos) { //you have a potential match for the reverse file
					...
					LOG(LOGERROR) << "unexpected parsing results, quitting. \n."; ctrlc_pressed = true;  //shouldn't get here unless the fastq.info changes the format of the output filenames???
				}
			}
		}
...
int SRACommand::checkGroups(map<string, vector<string> >& files) {
	...
	for (int i = 0; i < Groups.size(); i++) {
		if (app.isDebug) { LOG(DEBUG) << "group " + toString(i) + " = " + Groups[i] + "\n"; }
...
int SRACommand::readOligos() {
	...
	if (thisFilesLinkers.size() > 1) { LOG(WARNING) << "the make.sra command only allows for the use of one linker at a time, disregarding all but first one.\n"; }
	...
	if (thisFilesSpacers.size() > 1) { LOG(WARNING) << "the make.sra command only allows for the use of one spacer at a time, disregarding all but first one.\n"; }
				...
				if ((primerName == "ignore") || (barcodeName == "ignore")) {} //do nothing
					...
					if (itGroup2Barcode == Group2Barcode.end()) {
						...
						if ((temp != ".") && (temp != itGroup2Barcode->second)) {
							LOG(LOGERROR) << "group and barcodes/primers not unique. Should never get here.\n";
						}
					}
...
bool SRACommand::checkCasesPlatforms(string& platform) {
	...
	if (isOkay) {
		...
		LOG(LOGERROR) << "" + original + " is not a valid platform option.  Valid platform options are _LS454, ILLUMINA-ION, TORRENT or PACBIO_SMRT." << '\n'; abort = true;
	}
...
bool SRACommand::checkCasesInstrumentModels(string& instrumentModel) {
	...
	if (platform == "_LS454") { //instrument model options are 454_GS-454_GS_20-454_GS_FLX-454_GS_FLX_Titanium-454_GS_Junior-unspecified
		...
		if (isOkay) {
			...
			LOG(LOGERROR) << "" + original + " is not a valid instrument option for the " + platform + " platform.  Valid instrument options are 454_GS, 454_GS_20, 454_GS_FLX, 454_GS_FLX_Titanium, 454_GS_Junior or unspecified." << '\n'; abort = true;
		}
		...
		if (isOkay) {
			...
			LOG(LOGERROR) << "" + original + " is not a valid instrument option for the " + platform + " platform.  Valid instrument options are Illumina_Genome_Analyzer, Illumina_Genome_Analyzer_II, Illumina_Genome_Analyzer_IIx, Illumina_HiSeq_2000, Illumina_HiSeq_1000, Illumina_MiSeq or unspecified." << '\n'; abort = true;
		}
		...
		if (isOkay) {
			...
			LOG(LOGERROR) << "" + original + " is not a valid instrument option for the " + platform + " platform.  Valid instrument options are Ion_Torrent_PGM or unspecified." << '\n'; abort = true;
		}
	}
		...
		if (isOkay) {
			...
			LOG(LOGERROR) << "" + original + " is not a valid instrument option for the " + platform + " platform.  Valid instrument options are PacBio_RS or unspecified." << '\n'; abort = true;
		}
	}
	return isOkay;
}
...
bool SRACommand::checkCasesLibStrategy(string& libStrategy) {
	...
	if (isOkay) {
		...
		LOG(LOGERROR) << "" + original + " is not a valid libstrategy option.  Valid libstrategy options are AMPLICON,WGA,WGS,WGX,RNA-Seq,miRNA-Seq,WCS,CLONE,POOLCLONE,CLONEEND,FINISHING,ChIP-Seq,MNase-Seq,DNase-Hypersensitivity,Bisulfite-Seq,Tn-Seq,EST,FL-cDNA,CTS,MRE-Seq,MeDIP-Seq,MBD-Seq or OTHER." << '\n'; abort = true;
	}
...
bool SRACommand::checkCasesLibSource(string& libSource) {
	...
	if (isOkay) {
		...
		LOG(LOGERROR) << "" + original + " is not a valid libsource option.  Valid libsource options are METAGENOMIC,GENOMIC,TRANSCRIPTOMIC,METATRANSCRIPTOMIC,SYNTHETIC,VIRAL_RNA or OTHER." << '\n'; abort = true;
	}
...
bool SRACommand::checkCasesDataType(string& dataType) {
	...
	if (isOkay) {
		...
		LOG(LOGERROR) << "" + original + " is not a valid datatype option.  Valid datatype options are METAGENOME,GENOME_SEQUENCING,METAGENOMIC_ASSEMBLY,ASSEMBLY,TRANSCRIPTOME,PROTEOMIC,MAP,CLONE_ENDS,TARGETED_LOCI,RANDOM_SURVEY,EXOME,VARIATION,EPIGENOMICS,PHENOTYPE,GENOTYPE,OTHER." << '\n'; abort = true;
	}
...
bool SRACommand::sanityCheckMiMarksGroups() {
	...
	for (int i = 0; i < Groups.size(); i++) {
		...
		if (it == mimarks.end()) {
			isOkay = false;
			LOG(LOGERROR) << "MIMarks file is missing group " + Groups[i] + ", please correct.\n";
		}
	}

	if (!isOkay) { ctrlc_pressed = true; }
...
bool SRACommand::checkDateFormat(string& date) {
	...
	if (app.isDebug) { LOG(DEBUG) << "date = " + date + "\n"; }
	...
	if (m->containsAlphas(date)) { // then format == "DD-Mmm-YYYY", "Mmm-YYYY"
		...
		if (date.find_first_of('-') != string::npos) { Utility::split(date, '-', pieces); }
		else { pieces = m->splitWhiteSpace(date); }

		if (app.isDebug) { LOG(DEBUG) << "in alpha\n"; }
		//check "Mmm-YYYY"
		if (pieces.size() == 2) { //"Mmm-YYYY"
			if (app.isDebug) { LOG(DEBUG) << "pieces = 2 -> " + pieces[0] + '\t' + pieces[1] + "\n"; }
			map<string, int>::iterator it;
			it = months.find(pieces[0]);  //is this a valid month
			if (it != months.end()) {
				if (pieces[1].size() != 4) { LOG(LOGERROR) << "" + pieces[1] + " is not a valid format for the year. Must be YYYY. \n"; isOkay = false; }
			}
			else {
				...
				//look again
				it = months.find(pieces[0]);  //is this a valid month
				if (it == months.end()) { LOG(INFO) << "[ERROR] " + pieces[0] + " is not a valid month. Looking for ""Mmm-YYYY\" format.\n"; isOkay = false; }
				else {
					if (pieces[1].size() != 4) { LOG(LOGERROR) << "" + pieces[1] + " is not a valid format for the year. Must be YYYY. \n"; isOkay = false; }
				}
			}
			...
			if (app.isDebug) { LOG(DEBUG) << "pieces = 3 -> " + pieces[0] + '\t' + pieces[1] + '\t' + pieces[2] + "\n"; }
			map<string, int>::iterator it;
			it = months.find(pieces[1]);  //is this a valid month
			if (it != months.end()) {
				if (pieces[2].size() != 4) { LOG(LOGERROR) << "" + pieces[2] + " is not a valid format for the year. Must be YYYY. \n"; isOkay = false; }
			}
			else {
				//see if we can correct if
				...
				//look again
				it = months.find(pieces[1]);  //is this a valid month
				if (it == months.end()) { LOG(INFO) << "[ERROR] " + pieces[1] + " is not a valid month. Looking for ""Mmm-YYYY\" format.\n"; isOkay = false; }
				else {
					if (pieces[2].size() != 4) { LOG(LOGERROR) << "" + pieces[2] + " is not a valid format for the year. Must be YYYY. \n"; isOkay = false; }
				}
			}
			...
			if (isOkay) { //check to make sure day is correct for month chosen
				int dayNumber;  Utility::mothurConvert(pieces[0], dayNumber);
		...
		if (app.isDebug) { LOG(DEBUG) << "in nonAlpha\n"; }
		vector<string> pieces;
		if (date.find_first_of('-') != string::npos) { Utility::split(date, '-', pieces); }
		else { pieces = m->splitWhiteSpace(date); }
		...
		if (format == "yearFirst") {
			if (app.isDebug) { LOG(DEBUG) << "yearFirst pieces = 3 -> " + pieces[0] + '\t' + pieces[1] + '\t' + pieces[2] + "\n"; }
			//just year
			if (pieces.size() == 1) {
				if (pieces[0].size() != 4) { LOG(LOGERROR) << "" + pieces[0] + " is not a valid format for the year. Must be YYYY. \n"; isOkay = false; }
				else { date = pieces[0]; }
			}
				...
				if (it == monthsN.end()) {
					LOG(LOGERROR) << "" + pieces[1] + " is not a valid format for the month. Must be mm. \n"; isOkay = false;
				}
				if (isOkay) { date = pieces[0] + "-" + pieces[1]; }
			}
				...
				if (it == monthsN.end()) {
					...
					int day; Utility::mothurConvert(pieces[2], day);
					if (day <= maxDays) {}
					else {
						LOG(LOGERROR) << "" + pieces[2] + " is not a valid day for the month " + pieces[1] + ". \n"; isOkay = false;
					}
				}
				if (isOkay) { date = pieces[0] + "-" + pieces[1] + "-" + pieces[2]; }
			}
		   ...
		   //if year last, then it could be dd-mm-yyyy or mm-dd-yyyy -> yyyy-mm-dd
			...
			if (app.isDebug) { LOG(DEBUG) << "yearLast pieces = 3 -> " + pieces[0] + '\t' + pieces[1] + '\t' + pieces[2] + "\n"; }

			if (pieces[2].size() != 4) { LOG(LOGERROR) << "" + pieces[2] + " is not a valid format for the year. Must be YYYY. \n"; isOkay = false; }

			int first, second;
			Utility::mothurConvert(pieces[0], first);
			Utility::mothurConvert(pieces[1], second);

			if ((first <= 12) && (second <= 12)) { //we can't figure out which is the day and which is the month
				LOG(LOGERROR) << "" + pieces[0] + " and " + pieces[1] + " are both <= 12.  Cannot determine which is the day and which is the month. \n"; isOkay = false;
			}
				...
				if (it == monthsN.end()) {
					LOG(LOGERROR) << "" + pieces[0] + " is not a valid format for the month. Must be mm. \n"; isOkay = false;
				}
				else {
					//is the day in range
					int maxDays = it->second;
					if (second <= maxDays) { //reformat to acceptable format
						...
						LOG(LOGERROR) << "" + pieces[1] + " is not a valid day for the month " + pieces[0] + ". \n"; isOkay = false;
					}
				}
			}
				...
				if (it == monthsN.end()) {
					...
					if (first <= maxDays) { //reformat to acceptable format
						...
						LOG(LOGERROR) << "" + pieces[0] + " is not a valid day for the month " + pieces[1] + ". \n"; isOkay = false;
					}
				}

			}
			else {
				LOG(LOGERROR) << "" + pieces[0] + " and " + pieces[1] + " are both > 12.  No valid date. \n"; isOkay = false;
			}
		}
	}
	if (!isOkay) { LOG(LOGERROR) << "The date must be in one of the following formats: Date of sampling, in ""DD-Mmm-YYYY/"", ""Mmm-YYYY/"" or ""YYYY/"" format (eg., 30-Oct-1990, Oct-1990 or 1990) or ISO 8601 standard ""YYYY-mm-dd/"", ""YYYY-mm/""  (eg., 1990-10-30, 1990-10/"")"; }
