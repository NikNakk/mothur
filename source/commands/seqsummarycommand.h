...
public:
	SeqSummaryCommand(Settings& settings, string option);
	SeqSummaryCommand(Settings& settings);
	~SeqSummaryCommand() {}
...
static DWORD WINAPI MySeqSumThreadFunction(LPVOID lpParam) {
	...
	try {
		ofstream outSummary;
		File::openOutputFile(pDataArray->sumFile, outSummary);
		...
		for (int i = 0; i < pDataArray->end; i++) { //end is the number of sequences to process
			...
			if (ctrlc_pressed) { in.close(); outSummary.close(); pDataArray->count = 1; return 1; }

			Sequence current(in); pDataArray->File::gobble(in);
			...
			if (current.getName() != "") {
				...
				if (pDataArray->hasNameMap) {
					...
					if (it == pDataArray->nameMap.end()) { LOG(LOGERROR) << "" + current.getName() + " is not in your name or count file, please correct." << '\n'; ctrlc_pressed = true; }
					else { num = it->second; }
				}
		...
		LOG(FATAL) << e.what() << " in SeqSummaryCommand, MySeqSumThreadFunction";
		exit(1);
	}
}
#endif
