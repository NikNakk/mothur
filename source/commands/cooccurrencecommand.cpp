...
vector<string> CooccurrenceCommand::setParameters() {
	try {
		...
		LOG(FATAL) << e.what() << " in CooccurrenceCommand, setParameters";
		exit(1);
	}
}
//**********************************************************************************************************************
string CooccurrenceCommand::getHelpString() {
	try {
		string helpString = "The cooccurrence command calculates four metrics and tests their significance to assess whether presence-absence patterns are different than what one would expect by chance."
			"The cooccurrence command parameters are shared, metric, matrixmodel, iters, label and groups."
			"The matrixmodel parameter options are sim1, sim2, sim3, sim4, sim5, sim6, sim7, sim8 and sim9. Default=sim2"
			"The metric parameter options are cscore, checker, combo and vratio. Default=cscore"
			...
			"The cooccurrence command should be in the following format: \n"
			"cooccurrence(shared=yourSharedFile) \n"
			"Example cooccurrence(shared=final.an.shared).\n"
			"Note: No spaces between parameter labels (i.e. shared), '=' and parameters (i.e.yourShared).\n";
		return helpString;
	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in CooccurrenceCommand, getHelpString";
		exit(1);
	}
}
//**********************************************************************************************************************
...
//**********************************************************************************************************************
CooccurrenceCommand::CooccurrenceCommand(Settings& settings) : Command(settings) {
	try {
		...
		LOG(FATAL) << e.what() << " in CooccurrenceCommand, CooccurrenceCommand";
		exit(1);
	}
}
//**********************************************************************************************************************
CooccurrenceCommand::CooccurrenceCommand(Settings& settings, string option) : Command(settings, option) {
	...
	if (option == "help") { help(); abort = true; calledHelp = true; }
		...
		if ((metric != "cscore") && (metric != "checker") && (metric != "combo") && (metric != "vratio")) {
			LOG(LOGERROR) << "" + metric + " is not a valid metric option for the cooccurrence command. Choices are cscore, checker, combo, vratio." << '\n'; abort = true;
		}
		...
		if ((matrix != "sim1") && (matrix != "sim2") && (matrix != "sim3") && (matrix != "sim4") && (matrix != "sim5") && (matrix != "sim6") && (matrix != "sim7") && (matrix != "sim8") && (matrix != "sim9")) {
			LOG(LOGERROR) << "" + matrix + " is not a valid matrix option for the cooccurrence command. Choices are sim1, sim2, sim3, sim4, sim5, sim6, sim7, sim8, sim9." << '\n'; abort = true;
		}
		...
		Utility::mothurConvert(temp, runs);
...
int CooccurrenceCommand::execute() {
	...
	while ((lookup[0] != NULL) && ((allLines == 1) || (userLabels.size() != 0))) {
		...
		if (ctrlc_pressed) { for (int i = 0; i < lookup.size(); i++) { delete lookup[i]; } delete input; out.close(); File::remove(outputFileName); return 0; }
		...
		if (ctrlc_pressed) { outputTypes.clear(); delete input; out.close(); File::remove(outputFileName); return 0; }

		//get next line to process
		lookup = input->getSharedRAbundVectors();
	}

	if (ctrlc_pressed) { delete input; out.close(); File::remove(outputFileName); return 0; }
...
int CooccurrenceCommand::getCooccurrence(vector<SharedRAbundVector*>& thisLookUp, ofstream& out) {
	...
	if (numOTUS < 2) {
		LOG(INFO) << "Not enough OTUs for co-occurrence analysis, skipping" << '\n';
		return 0;
	}
	...
	//        }
		...
		LOG(LOGERROR) << "No model selected! \n";
		ctrlc_pressed = true;
	}
	...
	else { LOG(LOGERROR) << "No metric selected!\n"; ctrlc_pressed = true; return 1; }

	LOG(INFO) << "Initial c score: " + toString(initscore) << '\n';
	...
	for (int k = 0;k < runs;k++) {
		...
		//swap_checkerboards takes the original matrix and swaps checkerboards
			...
			LOG(LOGERROR) << "No null model selected!\n\n"; ctrlc_pressed = true;
			return 1;
		}
		...
		if (metric == "cscore") {
			...
			LOG(LOGERROR) << "No metric selected!\n\n"; ctrlc_pressed = true;
			return 1;
		}
	...
	LOG(INFO) << '\n' << "average metric score: " + toString(nullMean) << '\n';
	...
	LOG(INFO) << "zscore: " + toString(zscore) << '\n';
	LOG(INFO) << "standard deviation: " + toString(sd) << '\n';
	LOG(INFO) << "non-parametric p-value: " + toString(pvalue) << '\n';
