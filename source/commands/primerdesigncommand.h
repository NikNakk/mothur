...
public:
	PrimerDesignCommand(Settings& settings, string option);
	PrimerDesignCommand(Settings& settings);
	~PrimerDesignCommand() {}
...
static DWORD WINAPI MyPrimerThreadFunction(LPVOID lpParam) {
	...
	try {
		ofstream outSum;
		File::openOutputFileAppend(pDataArray->summaryFileName, outSum);
				...
				for (set<string>::iterator it = pDataArray->primers.begin(); it != pDataArray->primers.end(); it++) {
					...
					if (rawSequence.length() < primer.length()) { found = false; }
						...
						for (int j = 0; j < rawSequence.length() - pDataArray->length; j++) {
							...
							if (ctrlc_pressed) { found = false; break; }
		...
		LOG(FATAL) << e.what() << " in PrimerDesignCommand, MyPrimerThreadFunction";
		exit(1);
	}
}
#endif
