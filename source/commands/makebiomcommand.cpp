...
vector<string> MakeBiomCommand::setParameters() {
	try {
		...
		LOG(FATAL) << e.what() << " in MakeBiomCommand, setParameters";
		exit(1);
	}
}
//**********************************************************************************************************************
string MakeBiomCommand::getHelpString() {
	try {
		string helpString = "The make.biom command parameters are shared, relabund, contaxonomy, metadata, groups, matrixtype, picrust, reftaxonomy and label.  shared or relabund are required, unless you have a valid current file.\n"; //
		helpString += "The groups parameter allows you to specify which of the groups in your groupfile you would like included. The group names are separated by dashes.\n"
			...
			"The metadata parameter is used to provide experimental parameters to the columns.  Things like 'sample1 gut human_gut'. \n"
			"The picrust parameter is used to provide the greengenes OTU IDs map table.  NOTE: Picrust requires a greengenes taxonomy. \n"
			"The referencetax parameter is used with the picrust parameter.  Picrust requires the greengenes OTU IDs to be in the biom file. \n"
			"The make.biom command should be in the following format: make.biom(shared=yourShared, groups=yourGroups, label=yourLabels).\n"
			"Example make.biom(shared=abrecovery.an.shared, groups=A-B-C).\n"
			"The default value for groups is all the groups in your groupfile, and all labels in your inputfile will be used.\n"
			"The make.biom command outputs a .biom file.\n"
			"Note: No spaces between parameter labels (i.e. groups), '=' and parameters (i.e.yourGroups).\n";
		return helpString;
	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in MakeBiomCommand, getHelpString";
		exit(1);
	}
}
//**********************************************************************************************************************
...
//**********************************************************************************************************************
MakeBiomCommand::MakeBiomCommand(Settings& settings) : Command(settings) {
	try {
		...
		LOG(FATAL) << e.what() << " in MakeBiomCommand, MakeBiomCommand";
		exit(1);
	}
}
//**********************************************************************************************************************
MakeBiomCommand::MakeBiomCommand(Settings& settings, string option) : Command(settings, option) {
	...
	if (option == "help") { help(); abort = true; calledHelp = true; }
		...
		else { inputFileName = relabundfile; fileFormat = "relabund"; settings.setCurrent("relabund", relabundfile); }

		sharedfile = validParameter.validFile(parameters, "shared", true);
		if (sharedfile == "not open") { abort = true; }
		else if (sharedfile == "not found") { sharedfile = ""; }
		else { inputFileName = sharedfile; fileFormat = "sharedfile"; settings.setCurrent("shared", sharedfile); }
		...
		if ((relabundfile == "") && (sharedfile == "")) {
			...
			if (sharedfile != "") { inputFileName = sharedfile; fileFormat = "sharedfile"; LOG(INFO) << "Using " + sharedfile + " as input file for the shared parameter." << '\n'; }
			else {
				relabundfile = settings.getCurrent("relabund");
				if (relabundfile != "") { inputFileName = relabundfile; fileFormat = "relabund"; LOG(INFO) << "Using " + relabundfile + " as input file for the relabund parameter." << '\n'; }
				else {
		...
		if (picrustOtuFile != "") {
			picrust = true;
			if (contaxonomyfile == "") { LOG(LOGERROR) << "the constaxonomy parameter is required with the picrust parameter, aborting." << '\n'; abort = true; }
			if (referenceTax == "") { LOG(LOGERROR) << "the reftaxonomy parameter is required with the picrust parameter, aborting." << '\n'; abort = true; }
		}
		else { picrust = false; }

		if ((contaxonomyfile != "") && (labels.size() > 1)) { LOG(LOGERROR) << "the contaxonomy parameter cannot be used with multiple labels." << '\n'; abort = true; }
		...
		if ((format != "sparse") && (format != "dense")) {
			LOG(INFO) << format + " is not a valid option for the matrixtype parameter. Options are sparse and dense." << '\n'; abort = true;
		}
	}
...
int MakeBiomCommand::execute() {
	...
	if (fileFormat == "sharedfile") {
		...
		while ((lookup[0] != NULL) && ((allLines == 1) || (userLabels.size() != 0))) {
			...
			if (ctrlc_pressed) { for (int i = 0; i < outputNames.size(); i++) { File::remove(outputNames[i]); } for (int i = 0; i < lookup.size(); i++) { delete lookup[i]; }  return 0; }
		...
		while ((lookupRel[0] != NULL) && ((allLines == 1) || (userLabels.size() != 0))) {
			...
			if (ctrlc_pressed) { for (int i = 0; i < outputNames.size(); i++) { File::remove(outputNames[i]); } for (int i = 0; i < lookupRel.size(); i++) { delete lookupRel[i]; }  return 0; }

			if (allLines == 1 || labels.count(lookupRel[0]->getLabel()) == 1) {

				LOG(INFO) << lookupRel[0]->getLabel() << '\n';
				getBiom(lookupRel);
			...
			if ((Utility::anyLabelsToProcess(lookupRel[0]->getLabel(), userLabels, "") == true) && (processedLabels.count(lastLabel) != 1)) {
	...
	if (itTypes != outputTypes.end()) {
		if ((itTypes->second).size() != 0) { current = (itTypes->second)[0]; settings.setCurrent("biom", current); }
	}
...
int MakeBiomCommand::getBiom(vector<SharedRAbundVector*>& lookup) {
	...
	for (int i = 0; i < numBins - 1; i++) {
		if (ctrlc_pressed) { out.close(); return 0; }
		out << rowFront << settings.currentSharedBinLabels[i] << rowBack << metadata[i] << "},\n";
	}
	out << rowFront << settings.currentSharedBinLabels[(numBins - 1)] << rowBack << metadata[(numBins - 1)] << "}\n" + spaces + "],\n";
...
vector<string> MakeBiomCommand::getMetaData(vector<SharedRAbundVector*>& lookup) {
	...
	if (contaxonomyfile == "") { for (int i = 0; i < lookup[0]->getNumBins(); i++) { metadata.push_back("null"); } }
		...
		while (!in.eof()) {
			...
			if (ctrlc_pressed) { in.close(); return metadata; }

			in >> otuLabel >> size >> tax; File::gobble(in);
		...
		string firstBin = settings.currentSharedBinLabels[0];
		string binTag = "Otu";
		...
		for (int i = 0; i < otuLabels.size(); i++) {
			...
			if (ctrlc_pressed) { return metadata; }
			...
			if (m->isContainingOnlyDigits(otuLabels[i])) {
				...
				if (it == labelTaxMap.end()) {
					...
					LOG(LOGERROR) << "Cannot add OTULabel " + otuLabels[i] + " because it's simple label " + m->getSimpleLabel(otuLabels[i]) + " has already been added and will result in downstream errors. Have you mixed mothur labels and non mothur labels? To make the files work well together and backwards compatible mothur treats 1, OTU01, OTU001, OTU0001 all the same. We do this by removing any non numeric characters and leading zeros. For eaxample: Otu000018 and OtuMY18 both map to 18.\n"; ctrlc_pressed = true;
				}
			}
		}
		...
		for (int i = 0; i < lookup[0]->getNumBins(); i++) {
			...
			it = labelTaxMap.find(m->getSimpleLabel(settings.currentSharedBinLabels[i]));

			if (it == labelTaxMap.end()) { LOG(LOGERROR) << "can't find taxonomy information for " + settings.currentSharedBinLabels[i] + ".\n"; ctrlc_pressed = true; }
			else {
...
int MakeBiomCommand::getGreenGenesOTUIDs(vector<SharedRAbundVector*>& lookup, map<string, string>& labelTaxMap) {
	...
	for (map<string, string>::iterator it = labelTaxMap.begin(); it != labelTaxMap.end(); it++) { //maps label -> consensus taxonomy
		...
		else { LOG(LOGERROR) << "could not find OTUId for " + it->second + ". Its reference sequences are " + referenceString + ".\n"; ctrlc_pressed = true; }
	...
	for (int i = 0; i < settings.currentSharedBinLabels.size(); i++) { labelIndex[m->getSimpleLabel(settings.currentSharedBinLabels[i])] = i; }
	...
	for (map<string, vector<string> >::iterator itMap = ggOTUIDs.begin(); itMap != ggOTUIDs.end(); itMap++) {
			...
			if (scores[0] != "null") {
				...
				for (int i = 0; i < boots.size(); i++) {
					if (scores[i] == "null") { scoresNULL = true; break; }
						...
						float tempScore; Utility::mothurConvert(scores[i], tempScore);
						boots[i] += tempScore;
					}
				}
			}
		...
		if (app.isDebug) { LOG(DEBUG) << "merging " + mergeString + " for ggOTUid = " + itMap->first + ".\n"; }
...
map<string, string> MakeBiomCommand::readGGOtuMap() {
	...
	ifstream in;
	File::openInputFile(picrustOtuFile, in);
...
int MakeBiomCommand::getSampleMetaData(vector<SharedRAbundVector*>& lookup) {
	...
	if (metadatafile == "") { for (int i = 0; i < lookup.size(); i++) { sampleMetadata.push_back("null"); } }
		...
		while (!in.eof()) {
			...
			if (thisPieces.size() != count) { LOG(LOGERROR) << "expected " + toString(count) + " items of data for sample " + group + " read " + toString(thisPieces.size()) + ", quitting.\n"; }
		...
		for (int i = 0; i < lookup.size(); i++) {
			...
			if (it == lines.end()) { LOG(LOGERROR) << "can't find metadata information for " + lookup[i]->getGroup() + ", quitting.\n"; ctrlc_pressed = true; }
			else {
