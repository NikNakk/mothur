...
vector<string> IndicatorCommand::setParameters() {
	try {
		...
		LOG(FATAL) << e.what() << " in IndicatorCommand, setParameters";
		exit(1);
	}
}
//**********************************************************************************************************************
string IndicatorCommand::getHelpString() {
	try {
		string helpString = "The indicator command can be run in 3 ways: with a shared or relabund file and a design file, or with a shared or relabund file and a tree file, or with a shared or relabund file, tree file and design file. \n"
		...
		helpString += "The groups parameter allows you to specify which of the groups in your shared or relabund you would like analyzed, or if you provide a design file the groups in your design file.  The groups may be entered separated by dashes.\n"
			...
			"The indicator command should be used in the following format: indicator(tree=test.tre, shared=test.shared, label=0.03)\n"
			"Note: No spaces between parameter labels (i.e. tree), '=' and parameters (i.e.yourTreefile).\n";
		return helpString;
	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in IndicatorCommand, getHelpString";
		exit(1);
	}
}
//**********************************************************************************************************************
...
//**********************************************************************************************************************
IndicatorCommand::IndicatorCommand(Settings& settings) : Command(settings) {
	try {
		...
		LOG(FATAL) << e.what() << " in IndicatorCommand, IndicatorCommand";
		exit(1);
	}
}
//**********************************************************************************************************************
IndicatorCommand::IndicatorCommand(Settings& settings, string option) : Command(settings, option) {
		...
		if ((designfile == "") && (treefile == "")) {
			...
			if (treefile != "") { LOG(INFO) << "Using " + treefile + " as input file for the tree parameter." << '\n'; }
				...
				if (designfile != "") { LOG(INFO) << "Using " + designfile + " as input file for the design parameter." << '\n'; }
					...
					LOG(LOGERROR) << "You must provide either a tree or design file." << '\n'; abort = true;
				}
			}
		}

		if ((relabundfile != "") && (sharedfile != "")) { LOG(LOGERROR) << "You may not use both a shared and relabund file." << '\n'; abort = true; }
...
int IndicatorCommand::execute() {
	...
	if (sharedfile != "") {
		getShared();
		if (ctrlc_pressed) { if (designfile != "") { delete designMap; } for (int i = 0; i < lookup.size(); i++) { delete lookup[i]; } return 0; }
		if (lookup[0] == NULL) { LOG(INFO) << "[ERROR] reading shared file." << '\n'; return 0; }
	}
	else {
		getSharedFloat();
		if (ctrlc_pressed) { if (designfile != "") { delete designMap; } for (int i = 0; i < lookupFloat.size(); i++) { delete lookupFloat[i]; } return 0; }
		if (lookupFloat[0] == NULL) { LOG(INFO) << "[ERROR] reading relabund file." << '\n'; return 0; }
	}
	...
	if (treefile != "") {
		...
		for (int i = 0; i < m->Treenames.size(); i++) {
			...
			if (designfile == "") {
				if (!(m->inUsersGroups(m->Treenames[i], m->getAllGroups()))) {
					LOG(LOGERROR) << "" + m->Treenames[i] + " is not a group in your shared or relabund file." << '\n';
					mismatch = true;
				}
				...
				for (int k = 0; k < myNames.size(); k++) {
					if (!(m->inUsersGroups(myNames[k], m->getAllGroups()))) {
						LOG(LOGERROR) << "" + myNames[k] + " is not a group in your shared or relabund file." << '\n';
						mismatch = true;
					}
				}
				groupMap[m->Treenames[i]] = "Group1";
		...
		if ((designfile != "") && (m->Treenames.size() != Groups.size())) { cout << Groups.size() << '\t' << m->Treenames.size() << endl; LOG(LOGERROR) << "You design file does not match your tree, aborting." << '\n'; mismatch = true; }
		...
		if (readOk != 0) { LOG(INFO) << "Read Terminated." << '\n'; delete ct; delete read; return 0; }
		...
		/***************************************************/
		Tree* outputTree = new Tree(settings.getNumGroups(), ct);

		outputTree->getSubTree(T[0], settings.getGroups());
...
int IndicatorCommand::GetIndicatorSpecies() {
	...
	if (outputDir == "") { thisOutputDir += File::getPath(inputFileName); }
	map<string, string> variables;
	variables["[filename]"] = thisOutputDir + File::getRootName(File::getSimpleName(inputFileName));
	...
	if (sharedfile != "") {
		...
		if (groupsAlreadyAdded.size() != lookup.size()) { LOG(LOGERROR) << "could not make proper groupings." << '\n'; }
		...
		if (groupsAlreadyAdded.size() != lookupFloat.size()) { LOG(LOGERROR) << "could not make proper groupings." << '\n'; }
	...
	for (int j = 0; j < indicatorValues.size(); j++) {
		...
		out << settings.currentSharedBinLabels[j] << '\t' << indicatorGroups[j] << '\t' << indicatorValues[j] << '\t';

		if (pValues[j] > (1 / (float)iters)) { out << pValues[j] << endl; }
		else { out << "<" << (1 / (float)iters) << endl; }

		if (pValues[j] <= 0.05) {
			cout << settings.currentSharedBinLabels[j] << '\t' << indicatorGroups[j] << '\t' << indicatorValues[j] << '\t';
			...
			LOG(FILEONLY) << settings.currentSharedBinLabels[j] + "\t" + indicatorGroups[j] + "\t" + toString(indicatorValues[j]) + "\t" + pValueString;
			LOG(INFO) << "";
		}
	}
...
int IndicatorCommand::GetIndicatorSpecies(Tree*& T) {
	...
	//print headings
	out << "TreeNode\t";
	for (int i = 0; i < numBins; i++) { out << settings.currentSharedBinLabels[i] << "_IndGroups" << '\t' << settings.currentSharedBinLabels[i] << "_IndValue" << '\t' << "pValue" << '\t'; }
	out << endl;
	...
	for (int i = T->getNumLeaves(); i < T->getNumNodes(); i++) {
		...
		for (int j = 0; j < indicatorValues.size(); j++) {
			...
			if (pValues[j] <= 0.05) {
				cout << i + 1 << '\t' << settings.currentSharedBinLabels[j] << '\t' << indicatorGroups[j] << '\t' << indicatorValues[j];
				string pValueString = "\t<" + toString((1 / (float)iters));
				if (pValues[j] > (1 / (float)iters)) { pValueString = toString('\t' + pValues[j]); cout << '\t' << pValues[j]; }
				else { cout << "\t<" << (1 / (float)iters); }
				LOG(FILEONLY) << toString(i) + "\t" + settings.currentSharedBinLabels[j] + "\t" + indicatorGroups[j] + "\t" + toString(indicatorValues[j]) + "\t" + pValueString;
				LOG(INFO) << "";
			}
		}
		out << endl;
...
vector<float> IndicatorCommand::getValues(vector< vector<SharedRAbundFloatVector*> >& groupings, vector<string>& indicatorGroupings, map< vector<int>, vector<int> > groupingsMap) {
	...
	for (int i = 0; i < groupings[0][0]->getNumBins(); i++) {
		...
		if (ctrlc_pressed) { return values; }
...
vector<float> IndicatorCommand::driver(vector< vector<SharedRAbundFloatVector*> >& groupings, int num, vector<float> indicatorValues, int numIters) {
	...
	LOG(FATAL) << e.what() << " in IndicatorCommand, driver";
	exit(1);
}
}
//**********************************************************************************************************************
			...
			for (int i = 1; i < processors; i++) {
				...
				for (int l = 0; l < groupings.size(); l++) {
					for (int k = 0; k < groupings[l][0]->getNumBins(); k++) {
						if (ctrlc_pressed) { for (int j = 0; j < newGroupings.size(); j++) { for (int u = 0; u < newGroupings[j].size(); u++) { delete newGroupings[j][u]; } } return pvalues; }
