...
vector<string> VennCommand::setParameters() {
	try {
		...
		nkParameters.add(new StringParameter("calc", "", false, false));
		nkParameters.add(new NumberParameter("abund", -INFINITY, INFINITY, 10, false, false));
		nkParameters.add(new BooleanParameter("nseqs", false, false, false));
		nkParameters.add(new BooleanParameter("sharedotus", true, false, false));
		nkParameters.add(new NumberParameter("fontsize", -INFINITY, INFINITY, 24, false, false));
		CommandParameter ppermute("permute", "Multiple", "1-2-3-4", "4", "", "", "", "", false, false); parameters.push_back(ppermute);		nkParameters.add(new NumberParameter("seed", -INFINITY, INFINITY, 0, false, false));
		...
		LOG(FATAL) << e.what() << " in VennCommand, setParameters";
		exit(1);
	}
}
//**********************************************************************************************************************
string VennCommand::getHelpString() {
	try {
		string helpString = "The venn command parameters are list, shared, groups, calc, abund, nseqs, permute, sharedotus, fontsize and label.   shared, relabund, list, rabund or sabund is required unless you have a valid current file.\n"
			"The groups parameter allows you to specify which of the groups in your groupfile you would like included in your venn diagram, you may only use a maximum of 4 groups.\n"
			"The group names are separated by dashes. The label allows you to select what distance levels you would like a venn diagram created for, and are also separated by dashes.\n"
			"The fontsize parameter allows you to adjust the font size of the picture created, default=24.\n"
			"The venn command should be in the following format: venn(groups=yourGroups, calc=yourCalcs, label=yourLabels, abund=yourAbund).\n"
			"Example venn(groups=A-B-C, calc=sharedsobs-sharedchao, abund=20).\n"
			"The default value for groups is all the groups in your groupfile up to 4, and all labels in your inputfile will be used.\n"
			"The default value for calc is sobs if you have only read a list file or if you have selected only one group, and sharedsobs if you have multiple groups.\n"
			"The default available estimators for calc are sobs, chao and ace if you have only read a list file, and sharedsobs, sharedchao and sharedace if you have read a shared file.\n"
			"The nseqs parameter will output the number of sequences represented by the otus in the picture, default=F.\n"
			"If you have more than 4 groups, you can use the permute parameter to set the number of groups you would like mothur to divide the samples into to draw the venn diagrams for all possible combos. Default=4.\n"
			"The only estimators available four 4 groups are sharedsobs and sharedchao.\n"
			...
			"The venn command outputs a .svg file for each calculator you specify at each distance you choose.\n"
			"Note: No spaces between parameter labels (i.e. groups), '=' and parameters (i.e.yourGroups).\n";
		return helpString;
	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in VennCommand, getHelpString";
		exit(1);
	}
}
//**********************************************************************************************************************
...
//**********************************************************************************************************************
VennCommand::VennCommand(Settings& settings) : Command(settings) {
	try {
		...
		LOG(FATAL) << e.what() << " in VennCommand, VennCommand";
		exit(1);
	}
}
//**********************************************************************************************************************

VennCommand::VennCommand(Settings& settings, string option) : Command(settings, option) {
	...
	if (option == "help") { help(); abort = true; calledHelp = true; }
		...
		if ((sharedfile == "") && (listfile == "")) {
			...
			if (sharedfile != "") { inputfile = sharedfile; format = "sharedfile"; LOG(INFO) << "Using " + sharedfile + " as input file for the shared parameter." << '\n'; }
				...
				if (listfile != "") { inputfile = listfile; format = "list"; LOG(INFO) << "Using " + listfile + " as input file for the list parameter." << '\n'; }
					...
					LOG(INFO) << "No valid current files. You must provide a list or shared file." << '\n';
					abort = true;
				}
			}
		}
		...
		Utility::mothurConvert(temp, perm);
		if ((perm == 1) || (perm == 2) || (perm == 3) || (perm == 4)) {}
		else { LOG(LOGERROR) << "Not a valid permute value.  Valid values are 1, 2, 3, 4 and true." << '\n'; abort = true; }
...
int VennCommand::execute() {
	...
	if (vennCalculators.size() == 0) { LOG(INFO) << "No valid calculators given, please correct." << '\n'; return 0; }
