...
vector<string> SummaryCommand::setParameters() {
	try {
		...
		nkParameters.add(new MultipleParameter("calc", vector<string>{"sobs", "chao", "nseqs", "coverage", "ace", "jack", "shannon", "shannoneven", "npshannon", "heip", "smithwilson-simpson-simpsoneven-invsimpson-bootstrap-geometric-qstat-logseries-bergerparker-bstick-goodscoverage-efron-boneh-solow-shen"}, "sobs-chao-ace-jack-shannon-npshannon-simpson-shannonrange", true, false, true));
		...
		LOG(FATAL) << e.what() << " in SummaryCommand, setParameters";
		exit(1);
	}
}
//**********************************************************************************************************************
string SummaryCommand::getHelpString() {
	try {
		...
		ValidCalculators validCalculator
			"The summary.single command parameters are list, sabund, rabund, shared, subsample, iters, label, calc, abund and groupmode.  list, sabund, rabund or shared is required unless you have a valid current file.\n"
			"The summary.single command should be in the following format: \n"
			"summary.single(label=yourLabel, calc=yourEstimators).\n"
			"Example summary.single(label=unique-.01-.03, calc=sobs-chao-ace-jack-bootstrap-shannon-npshannon-simpson).\n";
		helpString += validCalculator.printCalc("summary")
			...
			"The default value calc is sobs-chao-ace-jack-shannon-npshannon-simpson\n"
			"If you are running summary.single with a shared file and would like your summary results collated in one file, set groupmode=t. (Default=true).\n"
		...
		LOG(FATAL) << e.what() << " in SummaryCommand, getHelpString";
		exit(1);
	}
}
//**********************************************************************************************************************
...
//**********************************************************************************************************************
SummaryCommand::SummaryCommand(Settings& settings) : Command(settings) {
	try {
		...
		LOG(FATAL) << e.what() << " in SummaryCommand, SummaryCommand";
		exit(1);
	}
}
//**********************************************************************************************************************

SummaryCommand::SummaryCommand(Settings& settings, string option) : Command(settings, option) {
	...
	if (option == "help") { help();  abort = true; calledHelp = true; }
		...
		if (subsample == false) { iters = 0; }
			...
			if ((subsampleSize == -1) && (format != "sharedfile")) { LOG(LOGERROR) << "If you want to subsample with a list, rabund or sabund file, you must provide the sample size.  You can do this by setting subsample=yourSampleSize.\n";  abort = true; }
		}
...
int SummaryCommand::execute() {
	...
	for (int p = 0; p < inputFileNames.size(); p++) {
		...
		if (sumCalculators.size() == 0) { for (int i = 0; i < outputNames.size(); i++) { File::remove(outputNames[i]); } return 0; }

		ofstream outputFileHandle;
		File::openOutputFile(fileNameRoot, outputFileHandle);
		outputFileHandle << "label";

		ofstream outAve;
		if (subsample) {
			File::openOutputFile(fileNameAve, outAve);
		...
		if (ctrlc_pressed) { outputFileHandle.close(); outAve.close(); for (int i = 0; i < outputNames.size(); i++) { File::remove(outputNames[i]); } for (int i = 0;i < sumCalculators.size();i++) { delete sumCalculators[i]; }  delete sabund;  delete input;  return 0; }
		...
		if (ctrlc_pressed) { outputFileHandle.close(); outAve.close();  for (int i = 0; i < outputNames.size(); i++) { File::remove(outputNames[i]); } for (int i = 0;i < sumCalculators.size();i++) { delete sumCalculators[i]; }   delete input;  return 0; }
		...
		if (ctrlc_pressed) { for (int i = 0; i < outputNames.size(); i++) { File::remove(outputNames[i]); } for (int i = 0;i < sumCalculators.size();i++) { delete sumCalculators[i]; }   delete input;  return 0; }
...
vector<string> SummaryCommand::parseSharedFile(string filename) {
	...
	if (subsample) {
		...
		if (lookup.size() < 1) { LOG(INFO) << "You have not provided enough valid groups.  I cannot run the command." << '\n'; ctrlc_pressed = true; delete input; return filenames; }
	}
...
vector<string> SummaryCommand::createGroupSummaryFile(int numLines, int numCols, vector<string>& outputNames, map<string, string> groupIndex) {
	...
	for (int i = 0; i < outputNames.size(); i++) {
		...
		string labelsLine = File::getline(temp);
