...
public:
	PairwiseSeqsCommand(Settings& settings, string option);
	PairwiseSeqsCommand(Settings& settings);
	~PairwiseSeqsCommand() {}
...
static DWORD WINAPI MyPairwiseSquareThreadFunction(LPVOID lpParam) {
	...
	try {
		...
		LOG(SCREENONLY) << toString(pDataArray->count) + "\t" + toString(time(NULL) - startTime) + "\n";
		...
		LOG(FATAL) << e.what() << " in PairwiseSeqsCommand, MyPairwiseSquareThreadFunction";
		exit(1);
	}
}
...
static DWORD WINAPI MyPairwiseThreadFunction(LPVOID lpParam) {
	...
	try {
		...
		LOG(SCREENONLY) << toString(pDataArray->end - 1) + "\t" + toString(time(NULL) - startTime) + "\n";
		...
		LOG(FATAL) << e.what() << " in PairwiseSeqsCommand, MyPairwiseThreadFunction";
		exit(1);
	}
}
