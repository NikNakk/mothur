...
vector<string> CorrAxesCommand::setParameters() {
	try {
		...
		nkParameters.add(new NumberParameter("numaxes", -INFINITY, INFINITY, 3, false, false));
		...
		LOG(FATAL) << e.what() << " in CorrAxesCommand, setParameters";
		exit(1);
	}
}
//**********************************************************************************************************************
string CorrAxesCommand::getHelpString() {
	try {
		string helpString = "The corr.axes command reads a shared, relabund or metadata file as well as an axes file and calculates the correlation coefficient.\n"
			"The corr.axes command parameters are shared, relabund, axes, metadata, groups, method, numaxes and label.  The shared, relabund or metadata and axes parameters are required.  If shared is given the relative abundance is calculated.\n"
			...
			"The corr.axes command should be in the following format: corr.axes(axes=yourPcoaFile, shared=yourSharedFile, method=yourMethod).\n"
			"Example corr.axes(axes=genus.pool.thetayc.genus.lt.pcoa, shared=genus.pool.shared, method=kendall).\n"
			"The corr.axes command outputs a .corr.axes file.\n"
			"Note: No spaces between parameter labels (i.e. groups), '=' and parameters (i.e.yourGroups).\n";
		return helpString;
	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in CorrAxesCommand, getHelpString";
		exit(1);
	}
}
//**********************************************************************************************************************
...
//**********************************************************************************************************************
CorrAxesCommand::CorrAxesCommand(Settings& settings) : Command(settings) {
	try {
		...
		LOG(FATAL) << e.what() << " in CorrAxesCommand, CorrAxesCommand";
		exit(1);
	}
}
//**********************************************************************************************************************
CorrAxesCommand::CorrAxesCommand(Settings& settings, string option) : Command(settings, option) {
	...
	if (option == "help") { help(); abort = true; calledHelp = true; }
		...
		else if (axesfile == "not found") { axesfile = ""; LOG(INFO) << "axes is a required parameter for the corr.axes command." << '\n'; abort = true; }
		...
		if ((relabundfile == "") && (sharedfile == "") && (metadatafile == "")) {
			...
			if (sharedfile != "") { inputFileName = sharedfile; LOG(INFO) << "Using " + sharedfile + " as input file for the shared parameter." << '\n'; }
				...
				if (relabundfile != "") { inputFileName = relabundfile;  LOG(INFO) << "Using " + relabundfile + " as input file for the relabund parameter." << '\n'; }
					...
					LOG(INFO) << "You must provide either a shared, relabund, or metadata file." << '\n'; abort = true;
				}
			}
		}

		if (metadatafile != "") {
			if ((relabundfile != "") || (sharedfile != "")) { LOG(INFO) << "You may only use one of the following : shared, relabund or metadata file." << '\n'; abort = true; }
		}
		else {
			if ((relabundfile != "") && (sharedfile != "")) { LOG(INFO) << "You may only use one of the following : shared, relabund or metadata file." << '\n'; abort = true; }
		}
		...
		Utility::mothurConvert(temp, numaxes);
...
int CorrAxesCommand::execute() {
	...
	if (sharedfile != "") {
		...
		if (ctrlc_pressed) { for (int i = 0; i < lookupFloat.size(); i++) { delete lookupFloat[i]; } return 0; }
		...
		if (ctrlc_pressed) { for (int i = 0; i < lookupFloat.size(); i++) { delete lookupFloat[i]; } return 0; }
		if (lookupFloat[0] == NULL) { LOG(INFO) << "[ERROR] reading metadata file." << '\n'; return 0; }

		if (pickedGroups) { eliminateZeroOTUS(lookupFloat); }

	}
	else { LOG(LOGERROR) << "no file given." << '\n'; return 0; }
	...
	if (axes.size() != lookupFloat.size()) {
		...
		for (int i = 0; i < lookupFloat.size(); i++) {
			...
			if (it == axes.end()) { LOG(INFO) << lookupFloat[i]->getGroup() + " is in your shared of relabund file but not in your axes file, please correct." << '\n'; }
		}
		ctrlc_pressed = true;
	}
	...
	else { LOG(LOGERROR) << "Invalid method." << '\n'; }
...
int CorrAxesCommand::calcPearson(map<string, vector<float> >& axes, ofstream& out) {
	try {
		...
		for (int i = 0; i < lookupFloat[0]->getNumBins(); i++) {
			...
			if (metadatafile == "") { out << settings.currentSharedBinLabels[i]; }
		...
		LOG(FATAL) << e.what() << " in CorrAxesCommand, calcPearson";
		exit(1);
	}
}
//**********************************************************************************************************************
...
map<string, vector<float> > CorrAxesCommand::readAxes() {
	...
	if (numaxes > count) { LOG(INFO) << "You requested " + toString(numaxes) + " axes, but your file only includes " + toString(count) + ". Using " + toString(count) + "." << '\n'; numaxes = count; }
	...
	while (!in.eof()) {
		...
		if (names.count(group) != 0) {
			...
			if (it == axes.end()) {
				...
				LOG(INFO) << group + " is already in your axes file, using first definition." << '\n';
			}
		}
...
int CorrAxesCommand::getMetadata() {
	...
	for (int i = 0; i < lookupFloat.size(); i++) {
		...
		if (util->isValidGroup(lookupFloat[i]->getGroup(), settings.getGroups()) == false) {
