...
string MakeLefseCommand::getHelpString() {
	string helpString = "The make.lefse command allows you to create a lefse formatted input file from mothur's output files.\n"
		"The make.lefse command parameters are: shared, relabund, constaxonomy, design, scale, groups and label.  The shared or relabund are required.\n"
		"The shared parameter is used to input your shared file, http://www.wiki.mothur.org/wiki/Shared_file.\n"
		"The relabund parameter is used to input your relabund file, http://www.wiki.mothur.org/wiki/Relabund_file.\n"
		"The design parameter is used to input your design file, http://www.wiki.mothur.org/wiki/Design_File.\n"
		"The constaxonomy parameter is used to input your taxonomy file. http://www.wiki.mothur.org/wiki/Constaxonomy_file. The contaxonomy file is the taxonomy file outputted by classify.otu(list=yourListfile, taxonomy=yourTaxonomyFile). Be SURE that the you are the constaxonomy file distance matches the shared file distance.  ie, for *.0.03.cons.taxonomy set label=0.03. Mothur is smart enough to handle shared files that have been subsampled. \n"
		...
		"The make.lefse command should be in the following format: make.lefse(shared=yourSharedFile)\n"
		"make.lefse(shared=final.an.shared)\n";
	return helpString;
}
//**********************************************************************************************************************
...
//**********************************************************************************************************************
MakeLefseCommand::MakeLefseCommand(Settings& settings) : Command(settings) {
	...
	LOG(FATAL) << e.what() << " in MakeLefseCommand, MakeLefseCommand";
	exit(1);
}
}
//**********************************************************************************************************************
MakeLefseCommand::MakeLefseCommand(Settings& settings, string option) : Command(settings, option) {
	try {
		...
		if (option == "help") { help(); abort = true; calledHelp = true; }
			...
			else { settings.setCurrent("relabund", relabundfile); }
			...
			if ((relabundfile == "") && (sharedfile == "")) {
				...
				if (sharedfile != "") { LOG(INFO) << "Using " + sharedfile + " as input file for the shared parameter." << '\n'; }
					...
					if (relabundfile != "") { LOG(INFO) << "Using " + relabundfile + " as input file for the relabund parameter." << '\n'; }
					else {
	...
	int MakeLefseCommand::runRelabund(map<int, consTax2>& consTax, vector<SharedRAbundFloatVector*>& lookup) {
		if (outputDir == "") { outputDir = File::getPath(inputFile); }
		...
		for (int i = 0; i < lookup[0]->getNumBins(); i++) { //process each otu
			...
			string nameOfOtu = settings.currentSharedBinLabels[i];

			if (constaxonomyfile != "") { //try to find the otuName in consTax to replace with consensus taxonomy
				int simpleLabel;
				Utility::mothurConvert(m->getSimpleLabel(nameOfOtu), simpleLabel);
				...
				if (it != consTax.end()) {
					...
					for (int j = 0; j < nameOfOtu.length() - 1; j++) {
						if (nameOfOtu[j] == ';') { fixedName += "_" + settings.currentSharedBinLabels[i] + '|'; }
						else { fixedName += nameOfOtu[j]; }
					}
					nameOfOtu = fixedName;
				}
				else {
					LOG(LOGERROR) << "can't find " + nameOfOtu + " in constaxonomy file. Do the distances match, did you forget to use the label parameter?\n"; ctrlc_pressed = true;
				}
	...
	vector<SharedRAbundFloatVector*> MakeLefseCommand::getSharedRelabund() {
		...
		if (label == "") { label = lastLabel; }
			...
			while ((templookup[0] != NULL) && (userLabels.size() != 0)) {
				if (ctrlc_pressed) { for (int i = 0; i < templookup.size(); i++) { delete templookup[i]; } return lookup; }
				...
				if ((Utility::anyLabelsToProcess(templookup[0]->getLabel(), userLabels, "") == true) && (processedLabels.count(lastLabel) != 1)) {
		...
		for (int i = 0; i < templookup.size(); i++) {
			for (int j = 0; j < templookup[i]->getNumBins(); j++) {
				...
				if (ctrlc_pressed) { for (int k = 0; k < templookup.size(); k++) { delete templookup[k]; } return lookup; }
	...
	vector<SharedRAbundFloatVector*> MakeLefseCommand::getRelabund() {
		...
		while ((lookupFloat[0] != NULL) && (userLabels.size() != 0)) {
			...
			if (ctrlc_pressed) { return lookupFloat; }
