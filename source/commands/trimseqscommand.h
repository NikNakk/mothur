...
public:
	TrimSeqsCommand(Settings& settings, string option);
	TrimSeqsCommand(Settings& settings);
	~TrimSeqsCommand() {}
...
static DWORD WINAPI MyTrimThreadFunction(LPVOID lpParam) {
	...
	try {
		...
		File::openOutputFile(pDataArray->trimFileName, trimFASTAFile);

		ofstream scrapFASTAFile;
		File::openOutputFile(pDataArray->scrapFileName, scrapFASTAFile);

		ofstream trimQualFile;
		ofstream scrapQualFile;
		if (pDataArray->qFileName != "") {
			File::openOutputFile(pDataArray->trimQFileName, trimQualFile);
			File::openOutputFile(pDataArray->scrapQFileName, scrapQualFile);
		}

		ofstream trimNameFile;
		ofstream scrapNameFile;
		if (pDataArray->nameFile != "") {
			File::openOutputFile(pDataArray->trimNFileName, trimNameFile);
			File::openOutputFile(pDataArray->scrapNFileName, scrapNameFile);
		}


		ofstream outGroupsFile;
		if ((pDataArray->createGroup) && (pDataArray->countfile == "")) { File::openOutputFile(pDataArray->groupFileName, outGroupsFile); }
			...
			for (int i = 0; i < pDataArray->fastaFileNames.size(); i++) { //clears old file
				for (int j = 0; j < pDataArray->fastaFileNames[i].size(); j++) { //clears old file
					if (pDataArray->fastaFileNames[i][j] != "") {
						ofstream temp;
						File::openOutputFile(pDataArray->fastaFileNames[i][j], temp);			temp.close();
						if (pDataArray->qFileName != "") {
							File::openOutputFile(pDataArray->qualFileNames[i][j], temp);			temp.close();
						}

						if (pDataArray->nameFile != "") {
							File::openOutputFile(pDataArray->nameFileNames[i][j], temp);			temp.close();
						}
					}
				}
			}
		...
		if (pDataArray->countfile != "") {
			File::openOutputFile(pDataArray->trimCFileName, trimCountFile);
			File::openOutputFile(pDataArray->scrapCFileName, scrapCountFile);
		...
		if ((pDataArray->lineStart == 0) || (pDataArray->lineStart == 1)) {
			...
			inFASTA.seekg(pDataArray->lineStart - 1); pDataArray->File::gobble(inFASTA);
		}

		ifstream qFile;
		if (pDataArray->qFileName != "") {
			File::openInputFile(pDataArray->qFileName, qFile);
			if ((pDataArray->qlineStart == 0) || (pDataArray->qlineStart == 1)) {
				qFile.seekg(0);
			}
			else { //this accounts for the difference in line endings. 
				qFile.seekg(pDataArray->qlineStart - 1); pDataArray->File::gobble(qFile);
			}
		}
		...
		for (int i = 0; i < pDataArray->lineEnd; i++) { //end is the number of sequences to process
			...
			if (pDataArray->qFileName != "") {
				currQual = QualityScores(qFile);  pDataArray->File::gobble(qFile);
			...
			if (origSeq != "") {
				...
				if (trashCode.length() == 0) {
					...
					if (pos == string::npos) {
						...
						if (pDataArray->allFiles) {
							ofstream output;
							File::openOutputFileAppend(pDataArray->fastaFileNames[barcodeIndex][primerIndex], output);
							currSeq.printSequence(output);
							output.close();

							if (pDataArray->qFileName != "") {
								File::openOutputFileAppend(pDataArray->qualFileNames[barcodeIndex][primerIndex], output);
							...
							if (pDataArray->nameFile != "") {
								map<string, string>::iterator itName = pDataArray->nameMap.find(currSeq.getName());
								if (itName != pDataArray->nameMap.end()) {
									File::openOutputFileAppend(pDataArray->nameFileNames[barcodeIndex][primerIndex], output);
		...
		LOG(FATAL) << e.what() << " in TrimSeqsCommand, MyTrimThreadFunction";
		exit(1);
	}
}
#endif
