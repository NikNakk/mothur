...
public:
	TrimFlowsCommand(Settings& settings, string option);
	TrimFlowsCommand(Settings& settings);
	~TrimFlowsCommand() {}
...
static DWORD WINAPI MyTrimFlowThreadFunction(LPVOID lpParam){
	...
	try {
		...
		File::openOutputFile(pDataArray->trimFlowFileName, trimFlowFile);
		trimFlowFile.setf(ios::fixed, ios::floatfield); trimFlowFile.setf(ios::showpoint);

		ofstream scrapFlowFile;
		File::openOutputFile(pDataArray->scrapFlowFileName, scrapFlowFile);
		scrapFlowFile.setf(ios::fixed, ios::floatfield); scrapFlowFile.setf(ios::showpoint);

		ofstream fastaFile;
		if(pDataArray->fasta){	File::openOutputFile(pDataArray->fastaFileName, fastaFile);	}

		ifstream flowFile;
		File::openInputFile(pDataArray->flowFileName, flowFile);

		flowFile.seekg(pDataArray->start);

		if(pDataArray->start == 0){
			flowFile >> pDataArray->numFlows; pDataArray->File::gobble(flowFile);
			...
			if(pDataArray->allFiles){
				for(int i=0;i<pDataArray->barcodePrimerComboFileNames.size();i++){
					for(int j=0;j<pDataArray->barcodePrimerComboFileNames[0].size();j++){
						ofstream temp;
						File::openOutputFile(pDataArray->barcodePrimerComboFileNames[i][j], temp);
		...
		for(int i = 0; i < pDataArray->end; i++){ //end is the number of sequences to process
			...
			if(trashCode.length() == 0){
				...
				if(pDataArray->allFiles){
					ofstream output;
					File::openOutputFileAppend(pDataArray->barcodePrimerComboFileNames[barcodeIndex][primerIndex], output);
		...
		LOG(FATAL) << e.what() << " in TrimFlowsCommand, MyTrimFlowsThreadFunction";
		exit(1);
	}
}
#endif
