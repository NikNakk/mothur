...
vector<string> RemoveLineageCommand::setParameters() {
	try {
		...
		LOG(FATAL) << e.what() << " in RemoveLineageCommand, setParameters";
		exit(1);
	}
}
//**********************************************************************************************************************
string RemoveLineageCommand::getHelpString() {
	try {
		string helpString = "The remove.lineage command reads a taxonomy or constaxonomy file and any of the following file types: fasta, name, group, count, list, shared or alignreport file. The constaxonomy can only be used with a shared or list file.\n"
		...
		LOG(FATAL) << e.what() << " in RemoveLineageCommand, getHelpString";
		exit(1);
	}
}
//**********************************************************************************************************************
...
//**********************************************************************************************************************
RemoveLineageCommand::RemoveLineageCommand(Settings& settings) : Command(settings) {
	try {
		...
		LOG(FATAL) << e.what() << " in RemoveLineageCommand, RemoveLineageCommand";
		exit(1);
	}
}
//**********************************************************************************************************************
RemoveLineageCommand::RemoveLineageCommand(Settings& settings, string option) : Command(settings, option) {
	...
	if (option == "help") { help(); abort = true; calledHelp = true; }
		...
		if ((usedDups != "") && (namefile == "")) { LOG(INFO) << "You may only use dups with the name option." << '\n';  abort = true; }
...
int RemoveLineageCommand::execute() {
	...
	LOG(FATAL) << e.what() << " in RemoveLineageCommand, execute";
	exit(1);
}
}
...
int RemoveLineageCommand::readFasta() {
	try {
		...
		if (wroteSomething == false) { LOG(INFO) << "Your fasta file contains only sequences from " + taxons + "." << '\n'; }
	...
	int RemoveLineageCommand::readList() {
		...
		if (wroteSomething == false) { LOG(INFO) << "Your list file contains only sequences from " + taxons + "." << '\n'; }
	...
	int RemoveLineageCommand::readName() {
		...
		if (wroteSomething == false) { LOG(INFO) << "Your name file contains only sequences from " + taxons + "." << '\n'; }
	...
	int RemoveLineageCommand::readCount() {
		...
		if (wroteSomething == false) { LOG(INFO) << "Your group file contains only sequences from " + taxons + "." << '\n'; }
	...
	int RemoveLineageCommand::readConsList() {
		...
		if (wroteSomething == false) { LOG(INFO) << "Your file only contains OTUs from " + taxons + "." << '\n'; }
		outputNames.push_back(outputFileName); outputTypes["list"].push_back(outputFileName);

		LOG(INFO) << "Removed " + toString(removedCount) + " OTUs from your list file." << '\n';
	...
	int RemoveLineageCommand::readShared() {
		...
		LOG(INFO) << "Removed " + toString(numRemoved) + " OTUs from your shared file." << '\n';
	...
	int RemoveLineageCommand::readTax() {
		...
		if (!wroteSomething) { LOG(INFO) << "Your taxonomy file contains only sequences from " + taxons + "." << '\n'; }
	...
	int RemoveLineageCommand::readConsTax() {
		...
		if (names.size() == 0) { LOG(INFO) << "Your constaxonomy file contains OTUs only from " + taxons + "." << '\n'; }
	...
	int RemoveLineageCommand::readAlign() {
		...
		if (wroteSomething == false) { LOG(INFO) << "Your align file contains only sequences from " + taxons + "." << '\n'; }
