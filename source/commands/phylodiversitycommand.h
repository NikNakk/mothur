...
public:
	PhyloDiversityCommand(Settings& settings, string option);
	PhyloDiversityCommand(Settings& settings);
...
static DWORD WINAPI MyPhyloDivThreadFunction(LPVOID lpParam) {
	...
	try {
		...
		vector<string> mGroups = pDataArray->settings.getGroups();
		...
		LOG(FATAL) << e.what() << " in PhyloDiversityCommand, MyPhyloDivThreadFunction";
		exit(1);
	}
}
#endif
