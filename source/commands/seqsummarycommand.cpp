...
vector<string> SeqSummaryCommand::setParameters() {
	try {
		...
		LOG(FATAL) << e.what() << " in SeqSummaryCommand, setParameters";
		exit(1);
	}
}
//**********************************************************************************************************************
string SeqSummaryCommand::getHelpString() {
	try {
		string helpString = "The summary.seqs command reads a fastafile and summarizes the sequences.\n"
		...
		LOG(FATAL) << e.what() << " in SeqSummaryCommand, getHelpString";
		exit(1);
	}
}
//**********************************************************************************************************************
...
//**********************************************************************************************************************
SeqSummaryCommand::SeqSummaryCommand(Settings& settings) : Command(settings) {
	try {
		...
		LOG(FATAL) << e.what() << " in SeqSummaryCommand, SeqSummaryCommand";
		exit(1);
	}
}
//***************************************************************************************************************

SeqSummaryCommand::SeqSummaryCommand(Settings& settings, string option) : Command(settings, option) {
...
int SeqSummaryCommand::execute() {
	try {
		...
		LOG(INFO) << "Minimum:\t" + toString(starts[0]) + "\t" + toString(ends[0]) + "\t" + toString(lengths[0]) + "\t" + toString(ambigs[0]) + "\t" + toString(homops[0]) + "\t" + toString(1) << '\n';
		LOG(INFO) << "2.5%-tile:\t" + toString(starts[1]) + "\t" + toString(ends[1]) + "\t" + toString(lengths[1]) + "\t" + toString(ambigs[1]) + "\t" + toString(homops[1]) + "\t" + toString(ptile0_25) << '\n';
		LOG(INFO) << "25%-tile:\t" + toString(starts[2]) + "\t" + toString(ends[2]) + "\t" + toString(lengths[2]) + "\t" + toString(ambigs[2]) + "\t" + toString(homops[2]) + "\t" + toString(ptile25) << '\n';
		LOG(INFO) << "Median: \t" + toString(starts[3]) + "\t" + toString(ends[3]) + "\t" + toString(lengths[3]) + "\t" + toString(ambigs[3]) + "\t" + toString(homops[3]) + "\t" + toString(ptile50) << '\n';
		LOG(INFO) << "75%-tile:\t" + toString(starts[4]) + "\t" + toString(ends[4]) + "\t" + toString(lengths[4]) + "\t" + toString(ambigs[4]) + "\t" + toString(homops[4]) + "\t" + toString(ptile75) << '\n';
		LOG(INFO) << "97.5%-tile:\t" + toString(starts[5]) + "\t" + toString(ends[5]) + "\t" + toString(lengths[5]) + "\t" + toString(ambigs[5]) + "\t" + toString(homops[5]) + "\t" + toString(ptile97_5) << '\n';
		LOG(INFO) << "Maximum:\t" + toString(starts[6]) + "\t" + toString(ends[6]) + "\t" + toString(lengths[6]) + "\t" + toString(ambigs[6]) + "\t" + toString(homops[6]) + "\t" + toString(ptile100) << '\n';
		LOG(INFO) << "Mean:\t" + toString(meanstartPosition) + "\t" + toString(meanendPosition) + "\t" + toString(meanseqLength) + "\t" + toString(meanambigBases) + "\t" + toString(meanlongHomoPolymer) << '\n';
		if ((namefile == "") && (countfile == "")) { LOG(INFO) << "# of Seqs:\t" + toString(numSeqs) << '\n'; }
		else { LOG(INFO) << "# of unique seqs:\t" + toString(numSeqs) << '\n'; LOG(INFO) << "total # of seqs:\t" + toString(size) << '\n'; }
		...
		if ((namefile == "") && (countfile == "")) { LOG(INFO) << "It took " + toString(time(NULL) - start) + " secs to summarize " + toString(numSeqs) + " sequences.\n"; }
		else { LOG(INFO) << "It took " + toString(time(NULL) - start) + " secs to summarize " + toString(size) + " sequences.\n"; }

		//set fasta file as new current fastafile
		string current = "";
		itTypes = outputTypes.find("summary");
		if (itTypes != outputTypes.end()) {
			if ((itTypes->second).size() != 0) { current = (itTypes->second)[0]; settings.setCurrent("summary", current); }
		}

		return 0;
	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in SeqSummaryCommand, execute";
		exit(1);
	}
}
/**************************************************************************************/
long long SeqSummaryCommand::driverCreateSummary(map<int, long long>& startPosition, map<int, long long>& endPosition, map<int, long long>& seqLength, map<int, long long>& ambigBases, map<int, long long>& longHomoPolymer, string filename, string sumFile, linePair* filePos) {
	...
	ofstream outSummary;
	File::openOutputFile(sumFile, outSummary);
	...
	while (!done) {
		...
		if (ctrlc_pressed) { in.close(); outSummary.close(); return 1; }

		if (app.isDebug) { LOG(DEBUG) << "count = " + toString(count) + "\n"; }

		Sequence current(in); File::gobble(in);

		if (current.getName() != "") {

			if (app.isDebug) { LOG(DEBUG) << "" + current.getName() + '\t' + toString(current.getNumBases()) + "\n"; }
			...
			if ((namefile != "") || (countfile != "")) {
				...
				if (it == nameMap.end()) { LOG(LOGERROR) << "'" + current.getName() + "' is not in your name or count file, please correct." << '\n'; ctrlc_pressed = true; }
				else { num = it->second; }
			}
			...
			if (app.isDebug) { LOG(DEBUG) << "" + current.getName() + '\t' + toString(num) + "\n"; }
		}
...
long long SeqSummaryCommand::createProcessesCreateSummary(map<int, long long>& startPosition, map<int, long long>& endPosition, map<int, long long>& seqLength, map<int, long long>& ambigBases, map<int, long long>& longHomoPolymer, string filename, string sumFile) {
	...
	while (process != processors) {
		...
		if (pid > 0) {
			...
			ctrlc_pressed = false;
			for (int i = 0;i < processIDS.size();i++) {
				File::remove(fastafile + (toString(processIDS[i]) + ".num.temp"));
				File::remove(sumFile + (toString(processIDS[i]) + ".temp"));
			}
			recalc = true;
			break;
		}
	...
	for (int i = 0; i < processIDS.size(); i++) {
		...
		long long  tempNum;
		in >> tempNum; File::gobble(in); num += tempNum;
		in >> tempNum; File::gobble(in);
		for (int k = 0; k < tempNum; k++) {
			long long first, second;
			in >> first; File::gobble(in); in >> second; File::gobble(in);
		...
		File::appendFiles((sumFile + toString(processIDS[i]) + ".temp"), sumFile);
		File::remove((sumFile + toString(processIDS[i]) + ".temp"));
	}
