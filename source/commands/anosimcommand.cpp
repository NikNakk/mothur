...
vector<string> AnosimCommand::setParameters() {
	try {
		...
		LOG(FATAL) << e.what() << " in AnosimCommand, setParameters";
		exit(1);
	}
}
//**********************************************************************************************************************
string AnosimCommand::getHelpString() {
	try {
		string helpString = "Referenced: Clarke, K. R. (1993). Non-parametric multivariate analysis of changes in community structure.   _Australian Journal of Ecology_ 18, 117-143.\n"
		...
		LOG(FATAL) << e.what() << " in AnosimCommand, getHelpString";
		exit(1);
	}
}
//**********************************************************************************************************************
...
//**********************************************************************************************************************
AnosimCommand::AnosimCommand(Settings& settings) : Command(settings) {
	try {
		...
		LOG(FATAL) << e.what() << " in AnosimCommand, AnosimCommand";
		exit(1);
	}
}
//**********************************************************************************************************************

AnosimCommand::AnosimCommand(Settings& settings, string option) : Command(settings, option) {
...
int AnosimCommand::execute() {
	...
	File::openOutputFile(ANOSIMFileName, ANOSIMFile);
...
double AnosimCommand::runANOSIM(ofstream& ANOSIMFile, vector<vector<double> > dMatrix, map<string, vector<int> > groupSampleMap, double alpha) {
	...
	LOG(INFO) << '\t' + toString(RValue) + '\t' + pString;
