...
public:
	UnifracWeightedCommand(Settings& settings, string option);
	UnifracWeightedCommand(Settings& settings);
...
static DWORD WINAPI MyWeightedRandomThreadFunction(LPVOID lpParam) {
	...
	try {
		...
		LOG(FATAL) << e.what() << " in Weighted, MyWeightedRandomThreadFunction";
		exit(1);
	}
}
#endif
