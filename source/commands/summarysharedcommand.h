...
public:
	SummarySharedCommand(Settings& settings, string option);
	SummarySharedCommand(Settings& settings);
...
static DWORD WINAPI MySummarySharedThreadFunction(LPVOID lpParam) {
	...
	try {
		...
		File::openOutputFile(pDataArray->sumFile, outputFileHandle);
		...
		for (int k = pDataArray->start; k < pDataArray->end; k++) { // pass cdd each set of groups to compare
			...
			for (int l = 0; l < k; l++) {
				...
				for (int i = 0;i < sumCalculators.size();i++) {
					...
					if (ctrlc_pressed) { for (int i = 0;i < sumCalculators.size();i++) { delete sumCalculators[i]; } outputFileHandle.close(); return 1; }
		...
		LOG(FATAL) << e.what() << " in SummarySharedCommand, MySummarySharedThreadFunction";
		exit(1);
	}
}
#endif
