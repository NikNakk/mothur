...
public:
	PcrSeqsCommand(Settings& settings, string option);
	PcrSeqsCommand(Settings& settings);
	~PcrSeqsCommand() {}
...
static DWORD WINAPI MyPcrThreadFunction(LPVOID lpParam) {
	...
	try {
		ofstream goodFile;
		File::openOutputFile(pDataArray->goodFasta, goodFile);

		ofstream badFile;
		File::openOutputFile(pDataArray->badFasta, badFile);

		ofstream locationsFile;
		File::openOutputFile(pDataArray->locationsName, locationsFile);
		...
		if ((pDataArray->fstart == 0) || (pDataArray->fstart == 1)) {
			...
			inFASTA.seekg(pDataArray->fstart - 1); pDataArray->File::gobble(inFASTA);
		}
		...
		for (int i = 0; i < pDataArray->fend; i++) { //end is the number of sequences to process
			...
			if (currSeq.getName() != "") {
				...
				if (pDataArray->oligosfile != "") {
					...
					if (!pDataArray->fileAligned) { LOG(LOGERROR) << "seqs are not aligned. When using start and end your sequences must be aligned.\n"; ctrlc_pressed = true; break; }
				   ...
				   //make sure the seqs are aligned
					if (!pDataArray->fileAligned) { LOG(LOGERROR) << "seqs are not aligned. When using start and end your sequences must be aligned.\n"; ctrlc_pressed = true; break; }
						...
						if (pDataArray->end != -1) {
							if (pDataArray->end > currSeq.getAligned().length()) { LOG(LOGERROR) << "end is longer than your sequence length, aborting.\n"; ctrlc_pressed = true; break; }
							else {
		...
		//report progress
		if ((pDataArray->count) % 100 != 0) { LOG(SCREENONLY) << "Thread Processing sequence: " + toString(pDataArray->count) + "\n"; }
		...
		if (app.isDebug) { LOG(DEBUG) << "fileAligned = " + toString(pDataArray->fileAligned) + '\n'; }
		...
		LOG(FATAL) << e.what() << " in PcrSeqsCommand, MyPcrThreadFunction";
		exit(1);
	}
}
