...
vector<string> MGClusterCommand::setParameters() {
	try {
		...
		nkParameters.add(new NumberParameter("length", -INFINITY, INFINITY, 5, false, false));
		nkParameters.add(new NumberParameter("penalty", -INFINITY, INFINITY, 0.10, false, false));
		...
		nkParameters.add(new BooleanParameter("min", true, false, false));
		nkParameters.add(new BooleanParameter("merge", true, false, false));
		nkParameters.add(new StringParameter("adjust", "F", false, false));
		nkParameters.add(new BooleanParameter("hcluster", false, false, false));
		...
		LOG(FATAL) << e.what() << " in MGClusterCommand, setParameters";
		exit(1);
	}
}
//**********************************************************************************************************************
string MGClusterCommand::getHelpString() {
	try {
		string helpString = "The mgcluster command parameter options are blast, name, cutoff, precision, hard,  method, merge, min, length, penalty, adjust and hcluster. The blast parameter is required.\n"
			"The mgcluster command reads a blast and name file and clusters the sequences into OPF units similar to the OTUs.\n"
			"This command outputs a .list, .rabund and .sabund file that can be used with mothur other commands to estimate richness.\n"
			"The cutoff parameter is used to specify the maximum distance you would like to cluster to. The default is 0.70.\n"
			"The precision parameter's default value is 100. \n"
			"The acceptable mgcluster methods are furthest, nearest and average.  If no method is provided then average is assumed.\n";
		helpString += "The min parameter allows you to specify is you want the minimum or maximum blast score ratio used in calculating the distance. The default is true, meaning you want the minimum.\n"
			"The length parameter is used to specify the minimum overlap required.  The default is 5.\n"
			"The adjust parameter is used to handle missing distances.  If you set a cutoff, adjust=f by default.  If not, adjust=t by default. Adjust=f, means ignore missing distances and adjust cutoff as needed with the average neighbor method.  Adjust=t, will treat missing distances as 1.0. You can also set the value the missing distances should be set to, adjust=0.5 would give missing distances a value of 0.5.\n"
			"The penalty parameter is used to adjust the error rate.  The default is 0.10.\n"
			"The merge parameter allows you to shut off merging based on overlaps and just cluster.  By default merge is true, meaning you want to merge.\n"
			"The hcluster parameter allows you to use the hcluster algorithm when clustering.  This may be necessary if your file is too large to fit into RAM. The default is false.\n"
			"The mgcluster command should be in the following format: \n"
			"mgcluster(blast=yourBlastfile, name=yourNameFile, cutoff=yourCutOff).\n"
			"Note: No spaces between parameter labels (i.e. balst), '=' and parameters (i.e.yourBlastfile).\n";
		return helpString;
	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in MGClusterCommand, getHelpString";
		exit(1);
	}
}
//**********************************************************************************************************************
...
//*******************************************************************************************************************
MGClusterCommand::MGClusterCommand(Settings& settings) : Command(settings) {
	try {
		...
		LOG(FATAL) << e.what() << " in MGClusterCommand, MGClusterCommand";
		exit(1);
	}
}
//**********************************************************************************************************************
MGClusterCommand::MGClusterCommand(Settings& settings, string option) : Command(settings, option) {
	...
	if (option == "help") { help(); abort = true; calledHelp = true; }
		...
		outputDir = validParameter.validFile(parameters, "outputdir", false);		if (outputDir == "not found") {
			outputDir = "";
			outputDir += File::getPath(blastfile); //if user entered a file with a path then preserve it	
		}
		...
		if (countfile != "" && namefile != "") { LOG(LOGERROR) << "Cannot have both a name file and count file. Please use one or the other." << '\n'; abort = true; }

		if ((blastfile == "")) { LOG(INFO) << "When executing a mgcluster command you must provide a blastfile." << '\n'; abort = true; }
		...
		else { LOG(INFO) << "Not a valid clustering method.  Valid clustering algorithms are furthest, nearest or average." << '\n'; abort = true; }
		...
		Utility::mothurConvert(temp, penalty);
...
int MGClusterCommand::execute() {
	...
	string fileroot = outputDir + File::getRootName(File::getSimpleName(blastfile));
	string tag = "";
	time_t start;
	...
	if (ctrlc_pressed) { outputTypes.clear(); delete nameMap; delete read; delete list; delete rabund; return 0; }
	...
	if (ctrlc_pressed) {
		...
		listFile.close(); if (countfile == "") { rabundFile.close(); sabundFile.close();  File::remove((fileroot + tag + ".rabund")); File::remove((fileroot + tag + ".sabund")); } File::remove((fileroot + tag + ".list"));
		outputTypes.clear();
		return 0;
	}
	...
	if (!hclusterWanted) {
		...
		while (distMatrix->getSmallDist() < cutoff && distMatrix->getNNodes() > 0) {
			...
			if (app.isDebug) { cout << "numNodes=" << distMatrix->getNNodes() << " smallDist = " << distMatrix->getSmallDist() << endl; }
	   ...
	   //get distmatrix and overlap
		...
		//ifstream inHcluster;
		//File::openInputFile(distFile, inHcluster);
		...
		while (seqs.size() != 0) {
			...
			if (ctrlc_pressed) {
				delete nameMap;  delete list; delete rabund; delete hcluster;
				listFile.close(); if (countfile == "") { rabundFile.close(); sabundFile.close();  File::remove((fileroot + tag + ".rabund")); File::remove((fileroot + tag + ".sabund")); } File::remove((fileroot + tag + ".list"));
				File::remove(distFile);
				File::remove(overlapFile);
				outputTypes.clear();
				return 0;
			}
	...
	if (countfile == "") {
		LOG(INFO) << rabundFileName << '\n';	outputNames.push_back(rabundFileName); outputTypes["rabund"].push_back(rabundFileName);
		LOG(INFO) << sabundFileName << '\n';	outputNames.push_back(sabundFileName); outputTypes["sabund"].push_back(sabundFileName);
	}
	LOG(INFO) << "";
	...
	LOG(INFO) << "It took " + toString(time(NULL) - start) + " seconds to cluster." << '\n';
...
ListVector* MGClusterCommand::mergeOPFs(map<string, int> binInfo, float dist) {
	...
	if (hclusterWanted) {
		File::openInputFile(overlapFile, inOverlap);
	...
	while (!done) {
		...
		if (!hclusterWanted) {
			...
			if (!inOverlap.eof()) {
				...
				inOverlap >> firstName >> secondName >> overlapDistance; File::gobble(inOverlap);
...
void MGClusterCommand::sortHclusterFiles(string unsortedDist, string unsortedOverlap) {
	...
	File::remove(unsortedDist);  //delete unsorted file
	distFile = sortedDistFile;

	//sort overlap file
	string sortedOverlapFile = m->sortFile(unsortedOverlap, outputDir);
	File::remove(unsortedOverlap);  //delete unsorted file
