...
string BiomInfoCommand::getHelpString() {
	string helpString = "The biom.info command reads a biom file creates a shared file. If your biom file contains metadata mothur will also create taxonomy or constaxonomy along with tax.summary files.\n"
		"The biom.info command parameters are biom, label and relabund. The biom parameter is required.\n"
		"The label parameter allows you to enter a distance label to be used in the shared file created from your biom file.\n"
		...
		"For example consider the following basis=sequence could give Clostridiales	3	105, where 105 is the total number of sequences whose otu classified to Clostridiales.\n"
		"Now for basis=otu could give Clostridiales	3	7, where 7 is the number of otus that classified to Clostridiales.\n"
		"The biom.info command should be in the following format: biom.info(biom=test.biom, label=0.03).\n"
		"Note: No spaces between parameter labels (i.e. label), '=' and parameters (i.e. 0.03).\n";
	return helpString;
}
//**********************************************************************************************************************
...
//**********************************************************************************************************************
BiomInfoCommand::BiomInfoCommand(Settings& settings) : Command(settings) {
...
//**********************************************************************************************************************
BiomInfoCommand::BiomInfoCommand(Settings& settings, string option) : Command(settings, option) {
	...
	if (option == "help") { help(); abort = true; calledHelp = true; }
		...
		else if (biomfile == "not found") { biomfile = ""; LOG(LOGERROR) << "You must provide a biom file, please correct.\n";  abort = true; }
...
int BiomInfoCommand::execute() {
	...
	if (ctrlc_pressed) { for (int i = 0; i < outputNames.size(); i++) { File::remove(outputNames[i]); } }
...
int BiomInfoCommand::createFilesFromBiom() {
	...
	ofstream out;
	File::openOutputFile(sharedFilename, out);
	...
	if (ctrlc_pressed) { out.close(); for (int j = 0; j < outputNames.size(); j++) { File::remove(outputNames[j]); } return 0; }
	...
	if (it == fileLines.end()) { LOG(LOGERROR) << "you file does not have a columns provided.\n"; }
		...
		if (hasTaxonomy) {
			...
			ofstream outTax;
			File::openOutputFile(taxFilename, outTax);
			...
			ofstream outTaxSum;
			File::openOutputFile(taxSumFilename, outTaxSum);
...
string BiomInfoCommand::addUnclassifieds(string tax) {
	try {
		...
		LOG(FATAL) << e.what() << " in BiomInfoCommand, addUnclassifieds";
		exit(1);
	}
}
