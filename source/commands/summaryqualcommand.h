...
public:
	SummaryQualCommand(Settings& settings, string option);
	SummaryQualCommand(Settings& settings);
	~SummaryQualCommand() {}
...
static DWORD WINAPI MySeqSumQualThreadFunction(LPVOID lpParam) {
	...
	try {
		...
		for (int i = 0; i < pDataArray->end; i++) { //end is the number of sequences to process
			...
			if (ctrlc_pressed) { in.close(); pDataArray->count = 1; return 1; }

			QualityScores current(in); pDataArray->File::gobble(in);
			...
			if (current.getName() != "") {
				...
				if (pDataArray->hasNameMap) {
					...
					if (it == pDataArray->nameMap.end()) { LOG(LOGERROR) << "" + current.getName() + " is not in your namefile, please correct." << '\n'; ctrlc_pressed = true; }
					else { num = it->second; }
				}
		...
		LOG(FATAL) << e.what() << " in SummaryQualCommand, MySeqSumQualThreadFunction";
		exit(1);
	}
}
#endif
