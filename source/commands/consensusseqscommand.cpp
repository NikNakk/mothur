...
vector<string> ConsensusSeqsCommand::setParameters() {
	try {
		...
		LOG(FATAL) << e.what() << " in ConsensusSeqsCommand, setParameters";
		exit(1);
	}
}
//**********************************************************************************************************************
string ConsensusSeqsCommand::getHelpString() {
	try {
		string helpString = "The consensus.seqs command can be used in 2 ways: create a consensus sequence from a fastafile, or with a listfile create a consensus sequence for each otu. Sequences must be aligned.\n"
			"The consensus.seqs command parameters are fasta, list, name, count, cutoff and label.\n"
			...
			"The list parameter allows you to enter a your list file. \n"
			"The name parameter allows you to enter a names file associated with the fasta file. \n"
			...
			"The consensus.seqs command should be in the following format: \n"
			"consensus.seqs(fasta=yourFastaFile, list=yourListFile) \n";
		...
		LOG(FATAL) << e.what() << " in ConsensusSeqsCommand, getHelpString";
		exit(1);
	}
}
//**********************************************************************************************************************
...
//**********************************************************************************************************************
ConsensusSeqsCommand::ConsensusSeqsCommand(Settings& settings) : Command(settings) {
	try {
		...
		LOG(FATAL) << e.what() << " in ConsensusSeqsCommand, ConsensusSeqsCommand";
		exit(1);
	}
}
//***************************************************************************************************************
ConsensusSeqsCommand::ConsensusSeqsCommand(Settings& settings, string option) : Command(settings, option) {
...
int ConsensusSeqsCommand::execute() {
	try {
		...
		if (listfile == "") {
			...
			File::openOutputFile(outputSummaryFile, outSummary);
			...
			File::openOutputFile(outputFastaFile, outFasta);
			...
			for (int j = 0; j < seqLength; j++) {
				...
				if (ctrlc_pressed) { outSummary.close(); outFasta.close(); for (int i = 0; i < outputNames.size(); i++) { File::remove(outputNames[i]); } return 0; }
				...
				for (map<string, string>::iterator it = fastaMap.begin(); it != fastaMap.end(); it++) {
					...
					if (countfile != "") { size = ct.getNumSeqs(it->first); }
						...
						else { LOG(LOGERROR) << "file mismatch, aborting.\n"; ctrlc_pressed = true; break; }
					}
			...
			while ((list != NULL) && ((allLines == 1) || (userLabels.size() != 0))) {
				...
				if (ctrlc_pressed) { for (int i = 0; i < outputNames.size(); i++) { File::remove(outputNames[i]); } delete list; delete input;  return 0; }
			...
			if (ctrlc_pressed) { for (int i = 0; i < outputNames.size(); i++) { File::remove(outputNames[i]); } if (list != NULL) { delete list; } delete input; return 0; }
		...
		LOG(INFO) << "It took " + toString(time(NULL) - start) + " secs to find the consensus sequences.";
		...
		LOG(FATAL) << e.what() << " in ConsensusSeqsCommand, execute";
		exit(1);
	}
}
//***************************************************************************************************************
...
int ConsensusSeqsCommand::processList(ListVector*& list) {
	try {
		...
		File::openOutputFile(outputNameFile, outName);
		...
		for (int i = 0; i < list->getNumBins(); i++) {
			...
			if (ctrlc_pressed) { outSummary.close(); outName.close(); outFasta.close(); return 0; }
		...
		LOG(FATAL) << e.what() << " in ConsensusSeqsCommand, processList";
		exit(1);
	}
}
	...
	try {
		...
		if (countfile != "") {
			...
			for (int j = 0; j < seqLength; j++) {
				...
				if (ctrlc_pressed) { return consSeq; }
				...
				for (int i = 0; i < binNames.size(); i++) {
					...
					else { LOG(LOGERROR) << "" + binNames[i] + " is not in your fasta file, please correct." << '\n'; ctrlc_pressed = true; }
					...
					else { LOG(LOGERROR) << "" + binNames[i] + " is not in your count file, please correct." << '\n'; ctrlc_pressed = true; }
				}
				char conBase = '.';
			...
			for (int i = 0; i < binNames.size(); i++) {
				...
				if (it == nameMap.end()) {
					if (namefile == "") { LOG(LOGERROR) << "" + binNames[i] + " is not in your fasta file, please correct." << '\n'; error = true; }
					else { LOG(LOGERROR) << "" + binNames[i] + " is not in your fasta or name file, please correct." << '\n'; error = true; }
					break;
				}
				else {
			...
			if (error) { ctrlc_pressed = true; return consSeq; }
			totalSize = seqs.size();
			//get counts
			for (int j = 0; j < seqLength; j++) {
		...
		LOG(FATAL) << e.what() << " in ConsensusSeqsCommand, getConsSeq";
		exit(1);
	}
}
//***************************************************************************************************************
...
char ConsensusSeqsCommand::getBase(vector<int> counts, int size) {  //A,T,G,C,Gap
	try {
		...
		* N = A G C T (any) */
		...
		LOG(FATAL) << e.what() << " in ConsensusSeqsCommand, getBase";
		exit(1);
	}
}
	...
	try {
		...
		while (!in.eof()) {
			...
			if (name != "") {
				...
				else if (seqLength != seq.getAligned().length()) { LOG(LOGERROR) << "sequence are not the same length, please correct." << '\n'; ctrlc_pressed = true; break; }
			}
		}
		...
		LOG(FATAL) << e.what() << " in ConsensusSeqsCommand, readFasta";
		exit(1);
	}
}
//***************************************************************************************************************
...
int ConsensusSeqsCommand::readNames() {
	try {
		...
		for (map<string, string>::iterator itTemp = temp.begin(); itTemp != temp.end(); itTemp++) {
			...
			else { LOG(LOGERROR) << "" + thisname + " is not in the fasta file, please correct." << '\n'; error = true; }
		}
		...
		LOG(FATAL) << e.what() << " in ConsensusSeqsCommand, readNames";
		exit(1);
	}
}
