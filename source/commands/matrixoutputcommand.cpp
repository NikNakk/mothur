...
vector<string> MatrixOutputCommand::setParameters() {
	try {
		...
		LOG(FATAL) << e.what() << " in MatrixOutputCommand, setParameters";
		exit(1);
	}
}
//**********************************************************************************************************************
string MatrixOutputCommand::getHelpString() {
	try {
		...
		ValidCalculators validCalculator
			"The dist.shared command parameters are shared, groups, calc, output, processors, subsample, iters, mode, and label.  shared is a required, unless you have a valid current file.\n"
			"The groups parameter allows you to specify which of the groups in your groupfile you would like included used.\n"
			"The group names are separated by dashes. The label parameter allows you to select what distance levels you would like distance matrices created for, and is also separated by dashes.\n"
			...
			"The dist.shared command should be in the following format: dist.shared(groups=yourGroups, calc=yourCalcs, label=yourLabels).\n"
			"The output parameter allows you to specify format of your distance matrix. Options are lt, column and square. The default is lt.\n"
			"The mode parameter allows you to specify if you want the average or the median values reported when subsampling. Options are average, and median. The default is average.\n"
			"Example dist.shared(groups=A-B-C, calc=jabund-sorabund).\n"
			"The default value for groups is all the groups in your groupfile.\n"
			"The default value for calc is jclass and thetayc.\n";
		helpString += validCalculator.printCalc("matrix")
			"The dist.shared command outputs a .dist file for each calculator you specify at each distance you choose.\n"
			"Note: No spaces between parameter labels (i.e. groups), '=' and parameters (i.e.yourGroups).\n";
		return helpString;
	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in MatrixOutputCommand, getHelpString";
		exit(1);
	}
}
//**********************************************************************************************************************
...
//**********************************************************************************************************************
MatrixOutputCommand::MatrixOutputCommand(Settings& settings) : Command(settings) {
	try {
		...
		LOG(FATAL) << e.what() << " in MatrixOutputCommand, MatrixOutputCommand";
		exit(1);
	}
}
//**********************************************************************************************************************

MatrixOutputCommand::MatrixOutputCommand(Settings& settings, string option) : Command(settings, option) {
	...
	if (option == "help") { help(); abort = true; calledHelp = true; }
		...
		if ((output != "lt") && (output != "square") && (output != "column")) { LOG(INFO) << output + " is not a valid output form. Options are lt, column and square. I will use lt." << '\n'; output = "lt"; }

		mode = validParameter.validFile(parameters, "mode", false);		if (mode == "not found") { mode = "average"; }
		if ((mode != "average") && (mode != "median")) { LOG(INFO) << mode + " is not a valid mode. Options are average and medina. I will use average." << '\n'; output = "average"; }
...
int MatrixOutputCommand::execute() {
	...
	if (matrixCalculators.size() == 0) { LOG(INFO) << "No valid calculators." << '\n';  return 0; }
	...
	while ((lookup[0] != NULL) && ((allLines == 1) || (userLabels.size() != 0))) {
		...
		if (ctrlc_pressed) { outputTypes.clear(); delete input; for (int i = 0; i < lookup.size(); i++) { delete lookup[i]; } for (int i = 0; i < outputNames.size(); i++) { File::remove(outputNames[i]); } m->clearGroups(); return 0; }
	...
	if (ctrlc_pressed) { outputTypes.clear(); delete input; for (int i = 0; i < outputNames.size(); i++) { File::remove(outputNames[i]); } m->clearGroups(); return 0; }
	...
	if (itTypes != outputTypes.end()) {
		if ((itTypes->second).size() != 0) { current = (itTypes->second)[0]; if (!subsample) { settings.setCurrent("phylip", current); } }
	}
...
int MatrixOutputCommand::process(vector<SharedRAbundVector*> thisLookup) {
		...
		if (processors == 1) {
			...
			while (process != processors) {
				...
				if (pid > 0) {
					...
					ctrlc_pressed = false;
					for (int i = 0;i < processIDS.size();i++) {
						File::remove(File::getRootName(File::getSimpleName(sharedfile)) + m->mothurGetpid(process) + ".dist");
					}
					recalc = true;
					break;
				}
			...
			if (recalc) {
				...
				//for (int i = 0; i < processIDS.size(); i++) { kill (processIDS[i], SIGINT); } for (int i=0;i<processIDS.size();i++) { int temp = processIDS[i]; wait(&temp); } ctrlc_pressed = false;  for (int i=0;i<processIDS.size();i++) {File::remove(File::getRootName(File::getSimpleName(sharedfile)) + m->mothurGetpid(process) + ".dist");}processors=3; LOG(LOGERROR) << "unable to spawn the number of processes you requested, reducing number to " + toString(processors) + "\n";
	...
	if (iters != 0) {
		...
		for (int i = 0; i < calcDists.size(); i++) {
			...
			settings.setCurrent("phylip", distFileName);
			ofstream outAve;
...
int MatrixOutputCommand::driver(vector<SharedRAbundVector*> thisLookup, int start, int end, vector< vector<seqDist> >& calcDists) {
	try {
		...
		LOG(FATAL) << e.what() << " in MatrixOutputCommand, driver";
		exit(1);
	}
}
/***********************************************************/
