...
vector<string> LibShuffCommand::setParameters() {
	try {
		...
		nkParameters.add(new NumberParameter("iters", -INFINITY, INFINITY, 10000, false, false));
		nkParameters.add(new NumberParameter("step", -INFINITY, INFINITY, 0.01, false, false));
		nkParameters.add(new NumberParameter("cutoff", -INFINITY, INFINITY, 1.0, false, false));
		...
		LOG(FATAL) << e.what() << " in LibShuffCommand, setParameters";
		exit(1);
	}
}
//**********************************************************************************************************************
string LibShuffCommand::getHelpString() {
	try {
		string helpString = "The libshuff command parameters are phylip, group, sim, groups, iters, step, form and cutoff.  phylip and group parameters are required, unless you have valid current files.\n"
			...
			"The group names are separated by dashes.  The iters parameter allows you to specify how many random matrices you would like compared to your matrix.\n"
			"The step parameter allows you to specify change in distance you would like between each output if you are using the discrete form.\n"
			"The form parameter allows you to specify if you would like to analyze your matrix using the discrete or integral form. Your options are integral or discrete.\n"
			"The libshuff command should be in the following format: libshuff(groups=yourGroups, iters=yourIters, cutOff=yourCutOff, form=yourForm, step=yourStep).\n"
			"Example libshuff(groups=A-B-C, iters=500, form=discrete, step=0.01, cutOff=2.0).\n"
			"The default value for groups is all the groups in your groupfile, iters is 10000, cutoff is 1.0, form is integral and step is 0.01.\n"
			"The libshuff command output two files: .coverage and .slsummary their descriptions are in the manual.\n"
			"Note: No spaces between parameter labels (i.e. iters), '=' and parameters (i.e.yourIters).\n";
		return helpString;
	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in LibShuffCommand, getHelpString";
		exit(1);
	}
}
//**********************************************************************************************************************
...
//**********************************************************************************************************************
LibShuffCommand::LibShuffCommand(Settings& settings) : Command(settings) {
	try {
		...
		LOG(FATAL) << e.what() << " in LibShuffCommand, LibShuffCommand";
		exit(1);
	}
}

//**********************************************************************************************************************
LibShuffCommand::LibShuffCommand(Settings& settings, string option) : Command(settings, option) {
	...
	if (option == "help") { help(); abort = true; calledHelp = true; }
		...
		if (phylipfile == "not open") { phylipfile = ""; abort = true; }
			...
			if (phylipfile != "") { LOG(INFO) << "Using " + phylipfile + " as input file for the phylip parameter." << '\n'; }
				...
				LOG(INFO) << "You must provide a phylip file." << '\n';
				abort = true;
			}
		}
		...
		if (groupfile == "not open") { groupfile = ""; abort = true; }
			...
			if (groupfile != "") { LOG(INFO) << "Using " + groupfile + " as input file for the group parameter." << '\n'; }
				...
				LOG(INFO) << "You must provide a group file." << '\n';
				abort = true;
			}
		}
		...
		Utility::mothurConvert(temp, step);
...
int LibShuffCommand::execute() {
	...
	if (ctrlc_pressed) { delete groupMap; delete matrix; return 0; }

	//if files don't match...
	if (matrix->getNumSeqs() < groupMap->getNumSeqs()) {
		LOG(INFO) << "Your distance file contains " + toString(matrix->getNumSeqs()) + " sequences, and your group file contains " + toString(groupMap->getNumSeqs()) + " sequences." << '\n';
		//create new group file
		if (outputDir == "") { outputDir += File::getPath(groupfile); }

		string newGroupFile = outputDir + File::getRootName(File::getSimpleName(groupfile)) + "editted.groups";
		outputNames.push_back(newGroupFile);
		ofstream outGroups;
		File::openOutputFile(newGroupFile, outGroups);

		for (int i = 0; i < matrix->getNumSeqs(); i++) {
			if (ctrlc_pressed) { delete groupMap; delete matrix; outGroups.close(); File::remove(newGroupFile); return 0; }
		...
		LOG(INFO) << newGroupFile + " is a new group file containing only the sequence that are in your distance file. I will read this file instead." << '\n';
		...
		if (ctrlc_pressed) { delete groupMap; delete matrix; File::remove(newGroupFile); return 0; }
	}


	setGroups();								//set the groups to be analyzed and sorts them

	if (numGroups < 2) { LOG(LOGERROR) << "libshuff requires at least 2 groups, you only have " + toString(numGroups) + ", aborting." << '\n'; ctrlc_pressed = true; }
	...
	if (ctrlc_pressed) { delete form; m->clearGroups(); delete matrix; delete groupMap; return 0; }
	...
	if (ctrlc_pressed) { outputTypes.clear(); delete form; m->clearGroups(); delete matrix; delete groupMap; return 0; }
	...
	for (int i = 0;i < numGroups - 1;i++) {
		for (int j = i + 1;j < numGroups;j++) {
			...
			if (ctrlc_pressed) { outputTypes.clear();  delete form; m->clearGroups(); delete matrix; delete groupMap; delete reading; return 0; }
...
int LibShuffCommand::printCoverageFile() {
	...
	File::openOutputFile(summaryFile, outCov);
	...
	for (int i = 0;i < numGroups;i++) {
		...
		for (int j = 0;j < numGroups;j++) {
			...
			for (int k = 0;k < savedMinValues[spoti][spotj].size();k++) {
				...
				if (ctrlc_pressed) { outCov.close(); return 0; }
...
int LibShuffCommand::printSummaryFile() {
	...
	File::openOutputFile(summaryFile, outSum);
	...
	for (int i = 0;i < numGroups;i++) {
		for (int j = i + 1;j < numGroups;j++) {
			...
			if (pValueCounts[i][j]) {
				...
				LOG(FILEONLY) << groupNames[i] + "-" + groupNames[j] + "\t" + toString(savedDXYValues[spoti][spotj]) + "\t" + toString((pValueCounts[i][j] / (float)iters)) << '\n';
				...
				LOG(FILEONLY) << groupNames[i] + "-" + groupNames[j] + "\t" + toString(savedDXYValues[spoti][spotj]) + "\t" + toString((1 / (float)iters)) << '\n';
			...
			if (pValueCounts[j][i]) {
				...
				LOG(FILEONLY) << groupNames[j] + "-" + groupNames[i] + "\t" + toString(savedDXYValues[spotj][spoti]) + "\t" + toString((pValueCounts[j][i] / (float)iters)) << '\n';
				...
				LOG(FILEONLY) << groupNames[j] + "-" + groupNames[i] + "\t" + toString(savedDXYValues[spotj][spoti]) + "\t" + toString((1 / (float)iters)) << '\n';
...
void LibShuffCommand::setGroups() {
	vector<string> myGroups = settings.getGroups();
		...
		if (savegroups != "all") {
			...
			for (int i = 0; i < myGroups.size(); i++) {
				if (groupMap->isValidGroup(myGroups[i]) != true) {
					LOG(INFO) << myGroups[i] + " is not a valid group, and will be disregarded." << '\n';
			...
			if ((myGroups.size() == 0) || (myGroups.size() == 1)) {
				...
				LOG(INFO) << "When using the groups parameter you must have at least 2 valid groups. I will run the command using all the groups in your groupfile." << '\n';
			}
