...
vector<string> GetLineageCommand::setParameters() {
	try {
		...
		LOG(FATAL) << e.what() << " in GetLineageCommand, setParameters";
		exit(1);
	}
}
//**********************************************************************************************************************
string GetLineageCommand::getHelpString() {
	try {
		string helpString = "The get.lineage command reads a taxonomy or constaxonomy file and any of the following file types: fasta, name, group, count, list, shared or alignreport file. The constaxonomy can only be used with a shared or list file.\n"
			"It outputs a file containing only the sequences from the taxonomy file that are from the taxon requested.\n"
			"The get.lineage command parameters are taxon, fasta, name, group, count, list, shared, taxonomy, alignreport, label and dups.  You must provide taxonomy or constaxonomy unless you have a valid current taxonomy file.\n"
			"The dups parameter allows you to add the entire line from a name file if you add any name from the line. default=false. \n"
			"The taxon parameter allows you to select the taxons you would like to get and is required.\n"
			"You may enter your taxons with confidence scores, doing so will get only those sequences that belong to the taxonomy and whose cofidence scores is above the scores you give.\n"
			"If they belong to the taxonomy and have confidences below those you provide the sequence will not be selected.\n"
			"The label parameter is used to analyze specific labels in your input. \n"
			"The get.lineage command should be in the following format: get.lineage(taxonomy=yourTaxonomyFile, taxon=yourTaxons).\n"
			"Example get.lineage(taxonomy=amazon.silva.taxonomy, taxon=Bacteria;Firmicutes;Bacilli;Lactobacillales;).\n"
		...
		LOG(FATAL) << e.what() << " in GetLineageCommand, getHelpString";
		exit(1);
	}
}
//**********************************************************************************************************************
...
//**********************************************************************************************************************
GetLineageCommand::GetLineageCommand(Settings& settings) : Command(settings) {
	try {
		...
		LOG(FATAL) << e.what() << " in GetLineageCommand, GetLineageCommand";
		exit(1);
	}
}
//**********************************************************************************************************************
GetLineageCommand::GetLineageCommand(Settings& settings, string option) : Command(settings, option) {
	...
	if (option == "help") { help(); abort = true; calledHelp = true; }
		...
		if ((constaxonomy == "") && (taxfile == "")) {
			...
			if (taxfile != "") { LOG(INFO) << "Using " + taxfile + " as input file for the taxonomy parameter." << '\n'; }
				...
				LOG(INFO) << "You have no current taxonomy file and did not provide a constaxonomy file. The taxonomy or constaxonomy parameter is required." << '\n'; abort = true;
			}
		}
		...
		if (taxons == "not found") { taxons = "";  LOG(INFO) << "No taxons given, please correct." << '\n';  abort = true; }
		else {
			//rip off quotes
		...
		if ((fastafile == "") && (constaxonomy == "") && (namefile == "") && (groupfile == "") && (alignfile == "") && (listfile == "") && (taxfile == "") && (countfile == "")) { LOG(INFO) << "You must provide one of the following: fasta, name, group, count, alignreport, taxonomy, constaxonomy, shared or listfile." << '\n'; abort = true; }

		if ((constaxonomy != "") && ((fastafile != "") || (namefile != "") || (groupfile != "") || (alignfile != "") || (taxfile != "") || (countfile != ""))) {
			LOG(LOGERROR) << "can only use constaxonomy file with a list or shared file, aborting.\n";  abort = true;
		}

		if ((constaxonomy != "") && (taxfile != "")) {
			LOG(LOGERROR) << "Choose only one: taxonomy or constaxonomy, aborting.\n"; abort = true;
		}

		if ((sharedfile != "") && (taxfile != "")) {
			LOG(LOGERROR) << "sharedfile can only be used with constaxonomy file, aborting.\n"; abort = true;
		}

		if ((sharedfile != "") || (listfile != "")) {
			label = validParameter.validFile(parameters, "label", false);
			if (label == "not found") { label = ""; LOG(WARNING) << "You did not provide a label, I will use the first label in your inputfile." << '\n'; }
		}
...
int GetLineageCommand::execute() {
	...
	LOG(FATAL) << e.what() << " in GetLineageCommand, execute";
	exit(1);
}
}
...
int GetLineageCommand::readFasta() {
	try {
		...
		if (wroteSomething == false) { LOG(INFO) << "Your file contains does not contain any sequences from " + taxons + "." << '\n'; }
	...
	int GetLineageCommand::readConsList() {
		...
		if (wroteSomething == false) { LOG(INFO) << "Your file does not contain OTUs from " + taxons + "." << '\n'; }
	...
	int GetLineageCommand::readShared() {
		...
		for (int i = 0; i < lookup[0]->getNumBins(); i++) {
			...
			if (names.count(m->getSimpleLabel(settings.currentSharedBinLabels[i])) != 0) {
	...
	int GetLineageCommand::readTax() {
		...
		if (names.size() == 0) { LOG(INFO) << "Your taxonomy file does not contain any sequences from " + taxons + "." << '\n'; }
	...
	int GetLineageCommand::readConsTax() {
		string thisOutputDir = outputDir;
		if (outputDir == "") { thisOutputDir += File::getPath(constaxonomy); }
		map<string, string> variables;
		variables["[filename]"] = thisOutputDir + File::getRootName(File::getSimpleName(constaxonomy));
		...
		ifstream in;
		File::openInputFile(constaxonomy, in);
		string otuLabel, tax;
		int numReps;
		...
		while (!in.eof()) {
			...
			in >> otuLabel;	 		File::gobble(in);
			in >> numReps;          File::gobble(in);
		...
		if (names.size() == 0) { LOG(INFO) << "Your taxonomy file does not contain any OTUs from " + taxons + "." << '\n'; }
