...
public:
	MatrixOutputCommand(Settings& settings, string option);
	MatrixOutputCommand(Settings& settings);
	~MatrixOutputCommand();
...
static DWORD WINAPI MyDistSharedThreadFunction(LPVOID lpParam) {
	...
	try {
		...
		LOG(FATAL) << e.what() << " in MatrixOutputCommand, MyDistSharedThreadFunction";
		exit(1);
	}
}
#endif
