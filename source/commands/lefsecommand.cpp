...
vector<string> LefseCommand::setParameters() {
	...
	nkParameters.add(new StringParameter("subclass", "", false, false));
	nkParameters.add(new StringParameter("label", "", false, false));
	//nkParameters.add(new StringParameter("classes", "", false, false));
	nkParameters.add(new NumberParameter("aalpha", -INFINITY, INFINITY, 0.05, false, false));
	nkParameters.add(new NumberParameter("walpha", -INFINITY, INFINITY, 0.05, false, false));

	nkParameters.add(new NumberParameter("lda", -INFINITY, INFINITY, 2.0, false, false));
	nkParameters.add(new BooleanParameter("wilc", true, false, false));
	nkParameters.add(new BooleanParameter("norm", true, false, false));
	nkParameters.add(new NumberParameter("iters", -INFINITY, INFINITY, 30, false, false));
	//nkParameters.add(new BooleanParameter("wilcsamename", false, false, false));
	nkParameters.add(new BooleanParameter("curv", false, false, false));
	nkParameters.add(new NumberParameter("fboots", -INFINITY, INFINITY, 0.67, false, false));
	CommandParameter pstrict("strict", "Multiple", "0-1-2", "0", "", "", "", "", false, false); parameters.push_back(pstrict);
	nkParameters.add(new NumberParameter("minc", -INFINITY, INFINITY, 10, false, false));
	...
	//nkParameters.add(new BooleanParameter("subject", false, false, false));


	//not used in their current code, but in parameters
	//nkParameters.add(new NumberParameter("nlogs", -INFINITY, INFINITY, 3, false, false));
	...
	//nkParameters.add(new BooleanParameter("svmnorm", true, false, false)); //not used because svm not implemented yet.
...
string LefseCommand::getHelpString() {
	string helpString = "The lefse command allows you to ....\n"
		"The lefse command parameters are: shared, design, class, subclass, label, walpha, aalpha, lda, wilc, iters, curv, fboots, strict, minc, multiclass and norm.\n"
		...
		"The subclass parameter is used to indicate the .....If none is provided, second category is used, or if only one category subclass is ignored. \n"
	...
	"The label parameter is used to indicate which distances in the shared file you would like to use. labels are separated by dashes.\n"
		"The lefse command should be in the following format: lefse(shared=final.an.shared, design=final.design, class=treatment, subclass=age).\n";
	return helpString;
}
//**********************************************************************************************************************
...
//**********************************************************************************************************************
LefseCommand::LefseCommand(Settings& settings) : Command(settings) {
...
//**********************************************************************************************************************
LefseCommand::LefseCommand(Settings& settings, string option) : Command(settings, option) {
	...
	if (option == "help") { help(); abort = true; calledHelp = true; }
		...
		Utility::mothurConvert(temp, anovaAlpha);

		temp = validParameter.validFile(parameters, "walpha", false);
		if (temp == "not found") { temp = "0.05"; }
		Utility::mothurConvert(temp, wilcoxonAlpha);
		...
		Utility::mothurConvert(temp, ldaThreshold);
		...
		Utility::mothurConvert(temp, fBoots);
		...
		if ((temp != "0") && (temp != "1") && (temp != "2")) { LOG(INFO) << "Invalid strict option: choices are 0, 1 or 2." << '\n'; abort = true; }
		else { Utility::mothurConvert(temp, strict); }

		temp = validParameter.validFile(parameters, "minc", false);
		if (temp == "not found") { temp = "10"; }
		Utility::mothurConvert(temp, minC);

		multiClassStrat = validParameter.validFile(parameters, "multiclass", false);
		if (multiClassStrat == "not found") { multiClassStrat = "onevall"; }
		if ((multiClassStrat != "onevall") && (multiClassStrat != "onevone")) { LOG(INFO) << "Invalid multiclass option: choices are onevone or onevall." << '\n'; abort = true; }
	}
...
int LefseCommand::process(vector<SharedRAbundFloatVector*>& lookup, DesignMap& designMap) {
	...
	for (int j = 0; j < lookup.size(); j++) {
		...
		if (it == subclass2Class.end()) {
			...
			if (it->second != treatment) {
				//LOG(WARNING) << "subclass " + thisSub + " has members in " + it->second + " and " + treatment + ". Subclass members must be from the same class for Wilcoxon. Changing " + thisSub + " to " + treatment + "_" + thisSub + ".\n";
	...
	if (app.isDebug) { LOG(DEBUG) << "completed Kruskal Wallis\n"; }
	...
	if (app.isDebug) { LOG(DEBUG) << "completed Wilcoxon\n"; }
	...
	if (numSigAfterWilcox > 0) {
		...
		LOG(INFO) << "Number of discriminative features with abs LDA score > " + toString(ldaThreshold) + " : " + toString(significantOtuLabels.size()) + ".\n";
	}
	else { LOG(INFO) << "No features with significant differences between the classes.\n"; }

	if (app.isDebug) { LOG(DEBUG) << "completed lda\n"; }
...
map<int, double> LefseCommand::runKruskalWallis(vector<SharedRAbundFloatVector*>& lookup, DesignMap& designMap) {
	try {
		...
		LOG(FATAL) << e.what() << " in LefseCommand, runKruskalWallis";
		exit(1);
	}
}
//**********************************************************************************************************************
		...
		for (itB;itB != class2SubClasses.end();itB++) {
			...
			for (set<string>::iterator itClass1 = (it->second).begin(); itClass1 != (it->second).end(); itClass1++) {
				...
				for (set<string>::iterator itClass2 = (itB->second).begin(); itClass2 != (itB->second).end(); itClass2++) {
					...
					if (app.isDebug) { LOG(INFO) << "[DEBUG comparing " + it->first + "-" + *itClass1 + " to " + itB->first + "-" + *itClass2 + "\n"; }
					...
					if (strict == 2) {}
					else { alphaMtc = 1.0 - pow((1.0 - wilcoxonAlpha), (double)(numSubs1 * numSubs2)); }
...
map<int, double> LefseCommand::testLDA(vector<SharedRAbundFloatVector*>& lookup, map<int, double> bins, map<string, vector<int> >& class2GroupIndex, map<string, vector<int> >& subClass2GroupIndex) {
	...
	for (int i = 0; i < numBins; i++) {
		...
		if (app.isDebug) { LOG(DEBUG) << "bin = " + toString(i) + "\n."; }

		it = bins.find(i);
		if (it != bins.end()) { //flagged in Kruskal Wallis and Wilcoxon(if we ran it)

			if (app.isDebug) { LOG(INFO) << "[DEBUG]:flagged bin = " + toString(i) + "\n."; }
			...
			for (map<string, vector<int> >::iterator it = class2GroupIndex.begin(); it != class2GroupIndex.end(); it++) {
				...
				if (app.isDebug) { LOG(DEBUG) << "class = " + it->first + "\n."; }
	...
	if (app.isDebug) { LOG(DEBUG) << "about to start iters. \n."; }

	vector< vector< vector<double> > > results;//[iters][numComparison][numOTUs]
	for (int j = 0; j < iters; j++) {
		if (ctrlc_pressed) { return sigOTUS; }

		if (app.isDebug) { LOG(DEBUG) << "iter = " + toString(j) + "\n."; }
		...
		if (app.isDebug) { LOG(DEBUG) << "after 1000. \n."; }
		...
		if (save < 1000) { LOG(WARNING) << "Skipping iter " + toString(j + 1) + " in LDA test. This can be caused by too few groups per class or not enough contrast within the classes. \n"; }
		else {
			...
			if (app.isDebug) { LOG(DEBUG) << "after lda. \n."; }
		}
	}
...
vector< vector<double> > LefseCommand::lda(vector< vector<double> >& adjustedLookup, vector<int> rand_s, map<int, string>& indexToClass, vector<string> classes) {
	...
	if (ctrlc_pressed) { return scaling; }
...
bool LefseCommand::contastWithinClassesOrFewPerClass(vector< vector<double> >& lookup, vector<int> rands, int minCl, map<string, vector<int> > class2GroupIndex, map<int, string> indexToClass) {
	...
	//sanity check
	if (rands.size() != countFound) { LOG(INFO) << "oops, should never get here, missing something.\n"; }
...
int LefseCommand::printResults(vector< vector<double> > means, map<int, double> sigKW, map<int, double> sigLDA, string label, vector<string> classes) {
	...
	for (int i = 0; i < means.size(); i++) { //[numOTUs][classes]
		...
		out << settings.currentSharedBinLabels[i] << '\t' << logMaxMean << '\t';
		if (app.isDebug) { temp = settings.currentSharedBinLabels[i] + '\t' + toString(logMaxMean) + '\t'; }
		...
		if (it != sigLDA.end()) {
			...
			if (app.isDebug) { temp += maxClass + '\t' + toString(it->second) + '\t' + toString(sigKW[i]) + '\n'; LOG(INFO) << temp; temp = ""; }
		}
		else { out << '-' << endl; }
	}
...
bool LefseCommand::printToCoutForRTesting(vector< vector<double> >& adjustedLookup, vector<int> rand_s, map<string, vector<int> >& class2GroupIndex, map<int, double> bins, map<string, vector<int> >& subClass2GroupIndex, vector<string> groups) {
	...
	for (map<int, double>::iterator it = bins.begin(); it != bins.end(); it++) {
		...
		cout << settings.currentSharedBinLabels[it->first] << " <- c(";
   ...
   //print data frame
	...
	for (map<int, double>::iterator it = bins.begin(); it != bins.end(); it++) {
		...
		tempOutput += "\"" + settings.currentSharedBinLabels[it->first] + "\"=" + settings.currentSharedBinLabels[it->first] + ",";
	}
	//tempOutput = tempOutput.substr(0, tempOutput.length()-1);
	...
	for (map<int, double>::iterator it = bins.begin(); it != bins.end(); it++) {
		...
		tempOutput += settings.currentSharedBinLabels[it->first] + "+";
	}
...
int LefseCommand::makeShared(int numDesignLines) {
	ifstream in;
	File::openInputFile(sharedfile, in);
	...
	ofstream out;
	File::openOutputFile(sharedfile + ".design", out); out << "group";
	...
	while (!in.eof()) {
		...
		for (int i = 1; i < pieces.size(); i++) {
			float value; Utility::mothurConvert(pieces[i], value);
			sum += value;
		}
		...
		if (sum != 0.0) {
			...
			settings.currentSharedBinLabels.push_back(toString(count));
