...
vector<string> MakeContigsCommand::setParameters() {
	try {
		...
		nkParameters.add(new BooleanParameter("trimoverlap", false, false, false));
		...
		nkParameters.add(new NumberParameter("insert", -INFINITY, INFINITY, 20, false, false));
		nkParameters.add(new NumberParameter("deltaq", -INFINITY, INFINITY, 6, false, false));
		nkParameters.add(new ProcessorsParameter(settings));
		nkParameters.add(new MultipleParameter("format", vector<string>{"sanger", "illumina", "solexa", "illumina1.8+"}, "illumina1.8+", false, false, true));
		...
		LOG(FATAL) << e.what() << " in MakeContigsCommand, setParameters";
		exit(1);
	}
}
//**********************************************************************************************************************
string MakeContigsCommand::getHelpString() {
	try {
		string helpString = "The make.contigs command reads a file, forward fastq file and a reverse fastq file or forward fasta and reverse fasta files and outputs new fasta. \n"
			"If an oligos file is provided barcodes and primers will be trimmed, and a group file will be created.\n"
			"If a forward index or reverse index file is provided barcodes be trimmed, and a group file will be created. The oligos parameter is required if an index file is given.\n"
			"The make.contigs command parameters are file, ffastq, rfastq, ffasta, rfasta, fqfile, rqfile, oligos, findex, rindex, format, tdiffs, bdiffs, pdiffs, align, match, mismatch, gapopen, gapextend, insert, deltaq, allfiles and processors.\n"
			"The ffastq and rfastq, file, or ffasta and rfasta parameters are required.\n"
			...
			"The findex and rindex parameters are used to provide a forward index and reverse index files to process.  \n"
			"The align parameter allows you to specify the alignment method to use.  Your options are: kmer, gotoh and needleman. The default is needleman.\n"
			"The ksize parameter allows you to set the kmer size if you are doing align=kmer. Default=8.\n"
		...
		"The match parameter allows you to specify the bonus for having the same base. The default is 1.0.\n"
			...
			"The insert parameter allows you to set a quality scores threshold. In the case where we are trying to decide whether to keep a base or remove it because the base is compared to a gap in the other fragment, if the base has a quality score equal to or below the threshold we eliminate it. Default=20.\n"
			...
			"The trimoverlap parameter allows you to trim the sequences to only the overlapping section. The default is F.\n"
			"The make.contigs command should be in the following format: \n"
			"make.contigs(ffastq=yourForwardFastqFile, rfastq=yourReverseFastqFile, align=yourAlignmentMethod) \n"
			"Note: No spaces between parameter labels (i.e. ffastq), '=' and parameters (i.e.yourForwardFastqFile).\n";
		return helpString;
	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in MakeContigsCommand, getHelpString";
		exit(1);
	}
}
//**********************************************************************************************************************
...
//**********************************************************************************************************************
MakeContigsCommand::MakeContigsCommand(Settings& settings) : Command(settings) {
	try {
		...
		LOG(FATAL) << e.what() << " in MakeContigsCommand, MakeContigsCommand";
		exit(1);
	}
}
//**********************************************************************************************************************
MakeContigsCommand::MakeContigsCommand(Settings& settings, string option) : Command(settings, option) {
	...
	if (option == "help") { help(); abort = true; calledHelp = true; }
		...
		//provide at least
		if ((file == "") && (ffastafile == "") && (ffastqfile == "")) { abort = true; LOG(LOGERROR) << "The file, ffastq and rfastq or ffasta and rfasta parameters are required.\n"; }
		if ((file != "") && ((ffastafile != "") || (ffastqfile != ""))) { abort = true; LOG(LOGERROR) << "The file, ffastq and rfastq or ffasta and rfasta parameters are required.\n"; }
		if ((ffastqfile != "") && (rfastqfile == "")) { abort = true; LOG(LOGERROR) << "If you provide use the ffastq, you must provide a rfastq file.\n"; }
		if ((ffastqfile == "") && (rfastqfile != "")) { abort = true; LOG(LOGERROR) << "If you provide use the rfastq, you must provide a ffastq file.\n"; }
		if ((ffastafile != "") && (rfastafile == "")) { abort = true; LOG(LOGERROR) << "If you provide use the ffasta, you must provide a rfasta file.\n"; }
		if ((ffastafile == "") && (rfastafile != "")) { abort = true; LOG(LOGERROR) << "If you provide use the rfasta, you must provide a ffasta file.\n"; }
		if ((fqualfile != "") && (rqualfile == "")) { abort = true; LOG(LOGERROR) << "If you provide use the fqfile, you must provide a rqfile file.\n"; }
		if ((fqualfile == "") && (rqualfile != "")) { abort = true; LOG(LOGERROR) << "If you provide use the rqfile, you must provide a fqfile file.\n"; }
		if (((fqualfile != "") || (rqualfile != "")) && ((ffastafile == "") || (rfastafile == ""))) {
			abort = true; LOG(LOGERROR) << "If you provide use the rqfile or fqfile file, you must provide the ffasta and rfasta parameters.\n";
		}

		oligosfile = validParameter.validFile(parameters, "oligos", true);
		if (oligosfile == "not found") { oligosfile = ""; }
		else if (oligosfile == "not open") { abort = true; }
		else { settings.setCurrent("oligos", oligosfile); }
		...
		if ((rindexfile != "") || (findexfile != "")) {
			if (oligosfile == "") {
				oligosfile = settings.getCurrent("oligos");
				if (oligosfile != "") { LOG(INFO) << "Using " + oligosfile + " as input file for the oligos parameter.\n"; }
				else {
					LOG(INFO) << "You need to provide an oligos file if you are going to use an index file.\n"; abort = true;
				}
			}

			//can only use an index file with the fastq parameters not fasta and qual
			if ((ffastafile != "") || (rfastafile != "")) {
				LOG(LOGERROR) << "You can only use an index file with the fastq parameters or the file option.\n"; abort = true;
			}
		}
		...
		Utility::mothurConvert(temp, insert);
		if ((insert < 0) || (insert > 40)) { LOG(LOGERROR) << "insert must be between 0 and 40.\n"; abort = true; }

		temp = validParameter.validFile(parameters, "deltaq", false);	if (temp == "not found") { temp = "6"; }
		Utility::mothurConvert(temp, deltaq);
	  ...
	  //			Utility::mothurConvert(temp, ldiffs);
		...
		if ((align != "needleman") && (align != "gotoh") && (align != "kmer")) { LOG(INFO) << align + " is not a valid alignment method. Options are kmer, needleman or gotoh. I will use needleman." << '\n'; align = "needleman"; }
...
int MakeContigsCommand::execute() {
	...
	LOG(INFO) << "It took " + toString(time(NULL) - start) + " secs to process " + toString(numReads) + " sequences.\n";
	...
	LOG(INFO) << "";
	int total = 0;
	if (totalGroupCounts.size() != 0) { LOG(INFO) << "Group count: \n"; }
	...
	if (itTypes != outputTypes.end()) {
		if ((itTypes->second).size() != 0) { currentGroup = (itTypes->second)[0]; settings.setCurrent("group", currentGroup); }
	}
...
unsigned long long MakeContigsCommand::processSingleFileOption(map<string, int>& totalGroupCounts) {
	...
	if (ffastafile != "") {
		...
		if (outputDir == "") { thisOutputDir = File::getPath(inputFile); }
		...
		if (fqualfile != "") {
			...
			variables["[filename]"] = thisOutputDir + File::getRootName(File::getSimpleName(fqualfile));
			variables["[tag]"] = "trim";
		...
		variables["[filename]"] = thisOutputDir + File::getRootName(File::getSimpleName(inputFile));
		delim = '>';
	}
	else { //ffastqfile
		hasQual = true;
	...
	if (qualOrIndexInputs.size() != 0) {
		...
		if (!allGZ && !allPlainTxt) { //mixed bag of files, uh oh...
			LOG(LOGERROR) << "Your files must all be in compressed .gz form or all in plain text form.  Please correct. \n"; ctrlc_pressed = true;
		}
	}
#else
#if defined (UNIX)
	...
	if (extension == "gz") { LOG(LOGERROR) << "You cannot use compressed .gz files as input with our windows version of mothur. \n"; ctrlc_pressed = true; }
	...
	if (ctrlc_pressed) { for (int i = 0; i < outputNames.size(); i++) { File::remove(outputNames[i]); }  delete oligos; return 0; }
	...
	if (allFiles) {
		...
		for (it = uniqueFastaNames.begin(); it != uniqueFastaNames.end(); it++) {
			...
			ofstream out;
			variables["[filename]"] = thisOutputDir + File::getRootName(File::getSimpleName(it->first));
...
unsigned long long MakeContigsCommand::processMultipleFileOption(map<string, int>& totalGroupCounts) {
	...
	if (outputDir == "") { compOutputDir = File::getPath(file); }
	cvars["[filename]"] = compOutputDir + File::getRootName(File::getSimpleName(file));
	cvars["[tag]"] = "";
	...
	File::openOutputFile(compositeFastaFile, outCTFasta); outCTFasta.close();
	File::openOutputFile(compositeQualFile, outCTQual); outCTQual.close();
	File::openOutputFile(compositeScrapFastaFile, outCSFasta); outCSFasta.close();
	File::openOutputFile(compositeScrapQualFile, outCSQual); outCSQual.close();
	File::openOutputFile(compositeMisMatchFile, outCMisMatch); outCMisMatch.close();
	...
	if (gz) {
		...
		for (int l = 0; l < fileInputs.size(); l++) {
			...
			if (createFileGroup || createOligosGroup) {
				if (l == 0) {
					ofstream outCGroup;
					File::openOutputFile(compositeGroupFile, outCGroup); outCGroup.close();
					outputNames.push_back(compositeGroupFile); outputTypes["group"].push_back(compositeGroupFile);
				}
				File::appendFiles(outputGroupFileName, compositeGroupFile);
				if (!allFiles) { File::remove(outputGroupFileName); }
			...
			if (l == 0) { File::appendFiles(outMisMatchFile, compositeMisMatchFile); }
			else { File::appendFilesWithoutHeaders(outMisMatchFile, compositeMisMatchFile); }
			File::appendFiles(outFastaFile, compositeFastaFile);
			File::appendFiles(outScrapFastaFile, compositeScrapFastaFile);
			File::appendFiles(outQualFile, compositeQualFile);
			File::appendFiles(outScrapQualFile, compositeScrapQualFile);
			if (!allFiles) {
				File::remove(outMisMatchFile);
				File::remove(outFastaFile);
				File::remove(outScrapFastaFile);
				File::remove(outQualFile);
				File::remove(outScrapQualFile);
			}
			else {
				outputNames.push_back(outFastaFile); outputTypes["fasta"].push_back(outFastaFile);
				outputNames.push_back(outScrapFastaFile); outputTypes["fasta"].push_back(outScrapFastaFile);
...
unsigned long long MakeContigsCommand::createProcessesGroups(vector< vector<string> > fileInputs, string compositeGroupFile, string compositeFastaFile, string compositeScrapFastaFile, string compositeQualFile, string compositeScrapQualFile, string compositeMisMatchFile, map<string, int>& totalGroupCounts) {
	...
	while (process != processors) {
		...
		if (pid > 0) {
			...
			ctrlc_pressed = false;
			for (int i = 0;i < processIDS.size();i++) {
				File::remove((toString(processIDS[i]) + ".num.temp"));
			}
			recalc = true;
			break;
		}
	...
	for (int i = 0; i < processIDS.size(); i++) {
		...
		int tempNum;
		in >> tempNum; num += tempNum; File::gobble(in);
	...
	for (int i = 0; i < processIDS.size(); i++) {
		File::appendFiles((compositeGroupFile + toString(processIDS[i]) + ".temp"), compositeGroupFile);
		File::remove((compositeGroupFile + toString(processIDS[i]) + ".temp"));

		File::appendFiles((compositeFastaFile + toString(processIDS[i]) + ".temp"), compositeFastaFile);
		File::remove((compositeFastaFile + toString(processIDS[i]) + ".temp"));

		File::appendFiles((compositeScrapFastaFile + toString(processIDS[i]) + ".temp"), compositeScrapFastaFile);
		File::remove((compositeScrapFastaFile + toString(processIDS[i]) + ".temp"));

		File::appendFiles((compositeQualFile + toString(processIDS[i]) + ".temp"), compositeQualFile);
		File::remove((compositeQualFile + toString(processIDS[i]) + ".temp"));

		File::appendFiles((compositeScrapQualFile + toString(processIDS[i]) + ".temp"), compositeScrapQualFile);
		File::remove((compositeScrapQualFile + toString(processIDS[i]) + ".temp"));

		File::appendFiles((compositeMisMatchFile + toString(processIDS[i]) + ".temp"), compositeMisMatchFile);
		File::remove((compositeMisMatchFile + toString(processIDS[i]) + ".temp"));
	}
...
unsigned long long MakeContigsCommand::driverGroups(vector< vector<string> > fileInputs, int start, int end, string compositeGroupFile, string compositeFastaFile, string compositeScrapFastaFile, string compositeQualFile, string compositeScrapQualFile, string compositeMisMatchFile, map<string, int>& totalGroupCounts) {
	...
	for (int l = start; l < end; l++) {
		...
		ofstream temp;
		File::openOutputFile(outFastaFile, temp);		temp.close();
		File::openOutputFile(outScrapFastaFile, temp);		temp.close();
		File::openOutputFile(outQualFile, temp);		temp.close();
		File::openOutputFile(outScrapQualFile, temp);		temp.close();
		...
		if (createFileGroup || createOligosGroup) {
			ofstream outCGroup;
			if (l == 0) { File::openOutputFile(compositeGroupFile, outCGroup);  outputNames.push_back(compositeGroupFile); outputTypes["group"].push_back(compositeGroupFile); }
			else { File::openOutputFileAppend(compositeGroupFile, outCGroup); }


			ofstream outGroup;
			File::openOutputFile(thisOutputGroupFileName, outGroup);
			for (map<string, string>::iterator itGroup = groupMap.begin(); itGroup != groupMap.end(); itGroup++) {
			...
			if (!allFiles) { File::remove(thisOutputGroupFileName); }
...
unsigned long long MakeContigsCommand::createProcesses(vector<string> fileInputs, vector<string> qualOrIndexFiles, string outputFasta, string outputScrapFasta, string outputQual, string outputScrapQual, string outputMisMatches, vector<vector<string> > fastaFileNames, vector<vector<string> > qualFileNames, vector<linePair> lines, vector<linePair> qLines, string group) {
				...
				for (int i = 0;i < tempFASTAFileNames.size();i++) {
					for (int j = 0;j < tempFASTAFileNames[i].size();j++) {
						...
						if (tempQUALFileNames[i][j] != "") {
							...
							File::openOutputFile(tempQUALFileNames[i][j], temp);			temp.close();
						}
					}
				}
			}
	...
	ofstream temp;
	File::openOutputFile(outputFasta, temp);		temp.close();
	File::openOutputFile(outputScrapFasta, temp);		temp.close();
	if (outputQual != "") {
		File::openOutputFile(outputQual, temp);		temp.close();
		File::openOutputFile(outputScrapQual, temp);		temp.close();
	}
	//do my part
	int spot = 0;
	...
	File::openOutputFile(outputFasta, temp);		temp.close();
	File::openOutputFile(outputScrapFasta, temp2);		temp2.close();
	if (outputQual != "") {
		File::openOutputFile(outputQual, temp3);		temp3.close();
		File::openOutputFile(outputScrapQual, temp4);		temp4.close();
	}
	//do my part
	...
	for (int i = 0; i < pDataArray.size(); i++) {
		...
		if (!pDataArray[i]->done) {
			LOG(LOGERROR) << "process " + toString(i) + " only processed " + toString(pDataArray[i]->count) + " of sequences assigned to it, quitting. \n"; ctrlc_pressed = true;
		}
	...
	for (int i = 0; i < processIDS.size(); i++) {
		File::appendFiles((outputFasta + toString(processIDS[i]) + ".temp"), outputFasta);
		File::remove((outputFasta + toString(processIDS[i]) + ".temp"));

		File::appendFiles((outputScrapFasta + toString(processIDS[i]) + ".temp"), outputScrapFasta);
		File::remove((outputScrapFasta + toString(processIDS[i]) + ".temp"));

		if (outputQual != "") {
			File::appendFiles((outputQual + toString(processIDS[i]) + ".temp"), outputQual);
			File::remove((outputQual + toString(processIDS[i]) + ".temp"));

			File::appendFiles((outputScrapQual + toString(processIDS[i]) + ".temp"), outputScrapQual);
			File::remove((outputScrapQual + toString(processIDS[i]) + ".temp"));
		}

		File::appendFilesWithoutHeaders((outputMisMatches + toString(processIDS[i]) + ".temp"), outputMisMatches);
		File::remove((outputMisMatches + toString(processIDS[i]) + ".temp"));
...
unsigned long long MakeContigsCommand::driver(vector<string> inputFiles, vector<string> qualOrIndexFiles, string outputFasta, string outputScrapFasta, string outputQual, string outputScrapQual, string outputMisMatches, vector<vector<string> > fastaFileNames, vector<vector<string> > qualFileNames, linePair linesInput, linePair linesInputReverse, linePair qlinesInput, linePair qlinesInputReverse, string group) {
	...
	if (!gz) { //plain text files
		File::openInputFile(thisffastafile, inFFasta);
		File::openInputFile(thisrfastafile, inRFasta);
...
#ifdef USE_BOOST
		File::openInputFileBinary(thisffastafile, inFFasta, inFF);
		File::openInputFileBinary(thisrfastafile, inRFasta, inRF);
#endif
	}
	...
	if (thisfqualindexfile != "") {
		if (thisfqualindexfile != "NONE") {
			if (!gz) { //plain text files
				File::openInputFile(thisfqualindexfile, inFQualIndex);
				inFQualIndex.seekg(qlinesInput.start);
			}
			else {
#ifdef USE_BOOST
				File::openInputFileBinary(thisfqualindexfile, inFQualIndex, inFQ);
#endif
		...
		if (thisrqualindexfile != "NONE") {
			if (!gz) { //plain text files
				File::openInputFile(thisrqualindexfile, inRQualIndex);
				inRQualIndex.seekg(qlinesInputReverse.start);
			}
			else {
#ifdef USE_BOOST
				File::openInputFileBinary(thisrqualindexfile, inRQualIndex, inRQ);
#endif
			} //compressed files - no need to seekg because compressed files divide workload differently
		}
	...
	File::openOutputFile(outputFasta, outFasta);
	File::openOutputFile(outputScrapFasta, outScrapFasta);
	File::openOutputFile(outputMisMatches, outMisMatch);
	...
	if (delim == '@') { //fastq files so make an output quality
		File::openOutputFile(outputQual, outQual);
		File::openOutputFile(outputScrapQual, outScrapQual);
		hasQuality = true;
		if (thisfqualindexfile != "") {
	...
	if (app.isDebug) { if (hasQuality) { LOG(DEBUG) << "hasQuality = true\n"; } else { LOG(DEBUG) << "hasQuality = false\n"; } }
	...
	while (good) {
		...
		if (!ignore) {
			...
			if (trashCode.length() == 0) {
				...
				if (app.isDebug) { LOG(DEBUG) << "" + fSeq.getName(); }
				...
				if (!ignore) {
					...
					if (allFiles) {
						...
						if (hasQuality) {
							ofstream output2;
							File::openOutputFileAppend(qualFileNames[barcodeIndex][primerIndex], output2);
		...
		//report progress
		if ((num) % 1000 == 0) { LOG(SCREENONLY) << toString(num) + "\n"; }
	}
	...
	if (ctrlc_pressed) {
		File::remove(outputFasta); File::remove(outputScrapFasta); File::remove(outputMisMatches);
		if (hasQuality) { File::remove(outputQual); File::remove(outputScrapQual); }
	}
...
vector<int> MakeContigsCommand::assembleFragments(vector< vector<double> >&qual_match_simple_bayesian, vector< vector<double> >& qual_mismatch_simple_bayesian, Sequence& fSeq, Sequence& rSeq, QualityScores*& fQual, QualityScores*& rQual, QualityScores*& savedFQual, QualityScores*& savedRQual, bool hasQuality, Alignment*& alignment, string& contig, string& trashCode, int& oend, int& oStart, int& numMismatches) {
	...
	for (int i = overlapStart; i < overlapEnd; i++) {
		...
		if (seq1[i] == seq2[i]) {
			...
			LOG(LOGERROR) << "case I didn't think of seq1 = " + toString(seq1[i]) + " and seq2 = " + toString(seq2[i]) + "\n";
		}
	...
	if (seq1End < seq2End) { //seq1 ends before seq2 so take from overlap to length from seq2
		for (int i = overlapEnd; i < length; i++) {
			contig += seq2[i];  if (hasQuality) { if (((seq2[i] != '-') && (seq2[i] != '.'))) { contigScores.push_back(scores2[BBaseMap[i]]); } }
		}
	}
	else { //seq2 ends before seq1 so take from overlap to length from seq1
		for (int i = overlapEnd; i < length; i++) {
			contig += seq1[i];  if (hasQuality) { if (((seq1[i] != '-') && (seq1[i] != '.'))) { contigScores.push_back(scores1[ABaseMap[i]]); } }
		}
	}
	//cout << contig << endl;
	...
	if (delim == '@') { //fastq files
		...
		if (!checkName(fread, rread)) {
			...
			if (!checkName(f2read, rread)) {
				...
				if (!checkName(fread, r2read)) {
					LOG(WARNING) << "name mismatch in forward and reverse fastq file. Ignoring, " + fread.getName() + ".\n"; ignore = true;
				}
				else { rread = r2read; }
			}
			else { fread = f2read; }
		...
		if (thisfqualindexfile != "") { //forward index file
			...
			if (!checkName(fread, firead)) {
				...
				if (!checkName(fread, f2iread)) {
					LOG(WARNING) << "name mismatch in forward index file. Ignoring, " + fread.getName() + ".\n"; ignore = true;
				}
		...
		if (thisrqualindexfile != "") { //reverse index file
			...
			if (!checkName(fread, riread)) {
				FastqRead r2iread(inRQ, tignore, format); File::gobble(inRQ);
				if (tignore) { ignore = true; }
				if (!checkName(fread, r2iread)) {
					LOG(WARNING) << "name mismatch in reverse index file. Ignoring, " + fread.getName() + ".\n"; ignore = true;
				}
		...
		if (!checkName(tfSeq, trSeq)) {
			...
			if (!checkName(t2fSeq, trSeq)) {
				...
				if (!checkName(tfSeq, t2rSeq)) {
					LOG(WARNING) << "name mismatch in forward and reverse fasta file. Ignoring, " + tfSeq.getName() + ".\n"; ignore = true;
				}
				else { trSeq = t2fSeq; }
			}
			else { tfSeq = t2fSeq; }
		...
		if (thisfqualindexfile != "") {
			fQual = new QualityScores(inFQ); File::gobble(inFQ);
			rQual = new QualityScores(inRQ); File::gobble(inRQ);
			if (!checkName(*fQual, *rQual)) {
				LOG(WARNING) << "name mismatch in forward and reverse qual file. Ignoring, " + fQual->getName() + ".\n"; ignore = true;
			}
			savedFQual = new QualityScores(fQual->getName(), fQual->getQualityScores());
			savedRQual = new QualityScores(rQual->getName(), rQual->getQualityScores());
			if (fQual->getName() != tfSeq.getName()) { LOG(WARNING) << "name mismatch in forward quality file. Ignoring, " + tfSeq.getName() + ".\n"; ignore = true; }
			if (rQual->getName() != trSeq.getName()) { LOG(WARNING) << "name mismatch in reverse quality file. Ignoring, " + trSeq.getName() + ".\n"; ignore = true; }
		}
		if (tfSeq.getName() != trSeq.getName()) { LOG(WARNING) << "name mismatch in forward and reverse fasta file. Ignoring, " + tfSeq.getName() + ".\n"; ignore = true; }
	}
...
bool MakeContigsCommand::read(Sequence& fSeq, Sequence& rSeq, QualityScores*& fQual, QualityScores*& rQual, QualityScores*& savedFQual, QualityScores*& savedRQual, Sequence& findexBarcode, Sequence& rindexBarcode, char delim, ifstream& inFFasta, ifstream& inRFasta, ifstream& inFQualIndex, ifstream& inRQualIndex, string thisfqualindexfile, string thisrqualindexfile) {
	...
	if (delim == '@') { //fastq files
		bool tignore;
		FastqRead fread(inFFasta, tignore, format); File::gobble(inFFasta);
		FastqRead rread(inRFasta, ignore, format); File::gobble(inRFasta);
		if (!checkName(fread, rread)) {
			FastqRead f2read(inFFasta, tignore, format); File::gobble(inFFasta);
			if (!checkName(f2read, rread)) {
				FastqRead r2read(inRFasta, ignore, format); File::gobble(inRFasta);
				if (!checkName(fread, r2read)) {
					LOG(WARNING) << "name mismatch in forward and reverse fastq file. Ignoring, " + fread.getName() + ".\n"; ignore = true;
				}
				else { rread = r2read; }
		...
		savedRQual = new QualityScores(rQual->getName(), rQual->getQualityScores());
		if (thisfqualindexfile != "") { //forward index file
			FastqRead firead(inFQualIndex, tignore, format); File::gobble(inFQualIndex);
			if (tignore) { ignore = true; }
			findexBarcode.setAligned(firead.getSeq());
			if (!checkName(fread, firead)) {
				FastqRead f2iread(inFQualIndex, tignore, format); File::gobble(inFQualIndex);
				if (tignore) { ignore = true; }
				if (!checkName(fread, f2iread)) {
					LOG(WARNING) << "name mismatch in forward index file. Ignoring, " + fread.getName() + ".\n"; ignore = true;
				}
		...
		if (thisrqualindexfile != "") { //reverse index file
			FastqRead riread(inRQualIndex, tignore, format); File::gobble(inRQualIndex);
			if (tignore) { ignore = true; }
			rindexBarcode.setAligned(riread.getSeq());
			if (!checkName(fread, riread)) {
				FastqRead r2iread(inRQualIndex, tignore, format); File::gobble(inRQualIndex);
				if (tignore) { ignore = true; }
				if (!checkName(fread, r2iread)) {
					LOG(WARNING) << "name mismatch in reverse index file. Ignoring, " + fread.getName() + ".\n"; ignore = true;
				}
		...
		Sequence tfSeq(inFFasta); File::gobble(inFFasta);
		Sequence trSeq(inRFasta); File::gobble(inRFasta);
		if (!checkName(tfSeq, trSeq)) {
			Sequence t2fSeq(inFFasta); File::gobble(inFFasta);
			if (!checkName(t2fSeq, trSeq)) {
				Sequence t2rSeq(inRFasta); File::gobble(inRFasta);
				if (!checkName(tfSeq, t2rSeq)) {
					LOG(WARNING) << "name mismatch in forward and reverse fasta file. Ignoring, " + tfSeq.getName() + ".\n"; ignore = true;
				}
				else { trSeq = t2fSeq; }
		...
		rSeq.setName(trSeq.getName()); rSeq.setAligned(trSeq.getAligned());
		if (thisfqualindexfile != "") {
			fQual = new QualityScores(inFQualIndex); File::gobble(inFQualIndex);
			rQual = new QualityScores(inRQualIndex); File::gobble(inRQualIndex);
			if (!checkName(*fQual, *rQual)) {
				LOG(WARNING) << "name mismatch in forward and reverse qual file. Ignoring, " + fQual->getName() + ".\n"; ignore = true;
			}
			savedFQual = new QualityScores(fQual->getName(), fQual->getQualityScores());
...
int MakeContigsCommand::setLines(vector<string> fasta, vector<string> qual, vector<linePair>& lines, vector<linePair>& qLines, char delim) {
	...
	fastaFilePos = File::divideFile(fasta[0], processors, delim);

	//get name of first sequence in each chunk
	map<string, int> firstSeqNames; map<string, int> trimmedNames;
	for (int i = 0; i < (fastaFilePos.size() - 1); i++) {
		ifstream in;
		File::openInputFile(fasta[0], in);
	...
	ifstream in2;
	File::openInputFile(fasta[1], in2);

	string input;
	while (!in2.eof()) {
		input = File::getline(in2); File::gobble(in2);
	...
	if ((firstSeqNames.size() != 0) && (trimmedNames.size() != 0)) {
		for (map<string, int>::iterator it = firstSeqNames.begin(); it != firstSeqNames.end(); it++) {
			if (delim == '>') {
				LOG(INFO) << it->first + " is in your forward fasta file and not in your reverse file, please remove it using the remove.seqs command before proceeding." << '\n';
			}
			else {
				LOG(INFO) << it->first + " is in your forward fastq file and not in your reverse file, please remove it using the remove.seqs command before proceeding." << '\n';
			}
		}
		ctrlc_pressed = true;
		return processors;
	...
	for (int i = 0; i < (fastaFilePos.size() - 1); i++) {
		if (app.isDebug) { LOG(DEBUG) << "forward " + toString(i) + '\t' + toString(fastaFilePos[i]) + '\t' + toString(fastaFilePos[i + 1]) + '\n'; }
		lines.push_back(linePair(fastaFilePos[i], fastaFilePos[(i + 1)]));
		if (app.isDebug) { LOG(DEBUG) << "reverse " + toString(i) + '\t' + toString(qfileFilePos[i]) + '\t' + toString(qfileFilePos[i + 1]) + '\n'; }
	...
	if (qual.size() != 0) {
		...
		if (qual[0] != "NONE") {
			...
			ifstream inQual;
			File::openInputFile(qual[0], inQual);

			string input;
			while (!inQual.eof()) {
				input = File::getline(inQual); File::gobble(inQual);
			...
			if ((firstSeqNames.size() != 0) && (trimmedNames.size() != 0)) {
				for (map<string, int>::iterator it = firstSeqNames.begin(); it != firstSeqNames.end(); it++) {
					if (delim == '>') {
						LOG(INFO) << it->first + " is in your forward fasta file and reverse fasta file, but not your forward qfile, please remove it using the remove.seqs command before proceeding." << '\n';
					}
					else {
						LOG(INFO) << it->first + " is in your forward fastq file and reverse fastq file, but not your forward index, please remove it using the remove.seqs command before proceeding." << '\n';
					}
				}
				ctrlc_pressed = true;
				return processors;
		...
		if (qual[1] != "NONE") {
			ifstream inQual2;
			File::openInputFile(qual[1], inQual2);

			while (!inQual2.eof()) {
				input = File::getline(inQual2); File::gobble(inQual2);
			...
			if ((firstSeqNames.size() != 0) && (trimmedNames.size() != 0)) {
				for (map<string, int>::iterator it = firstSeqNames.begin(); it != firstSeqNames.end(); it++) {
					if (delim == '>') {
						LOG(INFO) << it->first + " is in your forward fasta file, reverse fasta file, and forward qfile but not your reverse qfile, please remove it using the remove.seqs command before proceeding." << '\n';
					}
					else {
						if (qual[0] != "NONE") {
							LOG(INFO) << it->first + " is in your forward fastq file, reverse fastq file, and forward index but not your reverse index, please remove it using the remove.seqs command before proceeding." << '\n';
						}
						else {
							LOG(INFO) << it->first + " is in your forward fastq file, reverse fastq file, but not your reverse index, please remove it using the remove.seqs command before proceeding." << '\n';
						}
					}
				}
				ctrlc_pressed = true;
		...
		//fill lines with paired forward and reverse fasta lines
		for (int i = 0; i < (fastaFilePos.size() - 1); i++) {
			if (app.isDebug) { LOG(DEBUG) << "forward " + toString(i) + '\t' + toString(qfileFilePos[i]) + '\t' + toString(qfileFilePos[i + 1]) + '\n'; }
			qLines.push_back(linePair(qfileFilePos[i], qfileFilePos[(i + 1)]));
			if (app.isDebug) { LOG(DEBUG) << "reverse " + toString(i) + '\t' + toString(temp[i]) + '\t' + toString(temp[i + 1]) + '\n'; }
	...
	if (processors == 1) { //save time
		...
		if (numFastaSeqs != numRFastaSeqs) {
			if (delim == '>') {
				LOG(LOGERROR) << "You have " + toString(numFastaSeqs) + " sequences in your forward fasta file, but " + toString(numRFastaSeqs) + " sequences in your reverse fasta file. Please use the list.seqs and get.seqs commands to make the files match before proceeding." << '\n'; ctrlc_pressed = true; return processors;
			}
			else {
				LOG(LOGERROR) << "You have " + toString(numFastaSeqs) + " sequences in your forward fastq file, but " + toString(numRFastaSeqs) + " sequences in your reverse fastq file. Please use the list.seqs and get.seqs commands to make the files match before proceeding." << '\n'; ctrlc_pressed = true; return processors;
			}
		}
		...
		if (qual.size() != 0) {
			...
			if ((numFQualSeqs != numRQualSeqs) || (numFQualSeqs != numFastaSeqs)) {
				if (delim == '>') {
					LOG(LOGERROR) << "You have " + toString(numFastaSeqs) + " sequences in your forward fasta file, " + toString(numRFastaSeqs) + " sequences in your reverse fasta file, " + toString(numFQualSeqs) + " sequences in your forward qual file, " + toString(numRQualSeqs) + " sequences in your reverse qual file. Please use the list.seqs and get.seqs commands to make the files match before proceeding." << '\n'; ctrlc_pressed = true; return processors;
				}
				else {
					if (qual[0] != "NONE") {
						LOG(LOGERROR) << "You have " + toString(numFastaSeqs) + " sequences in your forward fastq file, " + toString(numRFastaSeqs) + " sequences in your reverse fastq file and " + toString(numRQualSeqs) + " sequences in your reverse index file. Please use the list.seqs and get.seqs commands to make the files match before proceeding." << '\n'; ctrlc_pressed = true; return processors;
					}
					else if (qual[1] != "NONE") {
						LOG(LOGERROR) << "You have " + toString(numFastaSeqs) + " sequences in your forward fastq file, " + toString(numRFastaSeqs) + " sequences in your reverse fastq file and " + toString(numFQualSeqs) + " sequences in your forward index file. Please use the list.seqs and get.seqs commands to make the files match before proceeding." << '\n'; ctrlc_pressed = true; return processors;
					}
					else {
						LOG(LOGERROR) << "You have " + toString(numFastaSeqs) + " sequences in your forward fastq file, " + toString(numRFastaSeqs) + " sequences in your reverse fastq file, " + toString(numFQualSeqs) + " sequences in your forward index file, " + toString(numRQualSeqs) + " sequences in your reverse index file. Please use the list.seqs and get.seqs commands to make the files match before proceeding." << '\n'; ctrlc_pressed = true; return processors;
					}
				}
			}
...
bool MakeContigsCommand::getOligos(vector<vector<string> >& fastaFileNames, vector<vector<string> >& qualFileNames, string rootname, map<string, string>& fastaFile2Group) {
	if (app.isDebug) { LOG(DEBUG) << "oligosfile = " + oligosfile + "\n"; }
	...
	if (oligos->hasPairedBarcodes() || oligos->hasPairedPrimers()) {
		...
		LOG(LOGERROR) << "make.contigs requires paired barcodes and primers. You can set one end to NONE if you are using an index file.\n"; ctrlc_pressed = true;
	}
	...
	if (numLinkers != 0) { LOG(WARNING) << "make.contigs is not setup to remove linkers, ignoring.\n"; }
	if (numSpacers != 0) { LOG(WARNING) << "make.contigs is not setup to remove spacers, ignoring.\n"; }
		...
		for (map<int, oligosPair>::iterator itBar = barcodes.begin();itBar != barcodes.end();itBar++) {
			for (map<int, oligosPair>::iterator itPrimer = primers.begin();itPrimer != primers.end(); itPrimer++) {
				...
				if ((primerName == "ignore") || (barcodeName == "ignore")) {} //do nothing
					...
					File::openOutputFile(qualFileName, temp2);		temp2.close();
				}
			}
		}
	}
