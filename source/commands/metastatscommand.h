...
public:
	MetaStatsCommand(Settings& settings, string option);
	MetaStatsCommand(Settings& settings);
	~MetaStatsCommand() {}
...
static DWORD WINAPI MyMetastatsThreadFunction(LPVOID lpParam) {
	...
	try {
		...
		for (int c = pDataArray->start; c < (pDataArray->start + pDataArray->num); c++) {
			...
			//get filename
			string outputFileName = pDataArray->outputDir + pDataArray->File::getRootName(File::getSimpleName(pDataArray->sharedfile)) + pDataArray->thisLookUp[0]->getLabel() + "." + setA + "-" + setB + ".metastats";
		...
		LOG(FATAL) << e.what() << " in MetaStatsCommand, MyMetastatsThreadFunction";
		exit(1);
	}
}
#endif
