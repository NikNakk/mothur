...
public:
	...
	ChopSeqsCommand(Settings& settings, string option);
	ChopSeqsCommand(Settings& settings);
	~ChopSeqsCommand() {};
...
static DWORD WINAPI MyChopThreadFunction(LPVOID lpParam) {
	...
	try {
		ofstream out;
		File::openOutputFile(pDataArray->outFasta, out);

		ofstream outAcc;
		File::openOutputFile(pDataArray->outAccnos, outAcc);

		ifstream in;
		File::openInputFile(pDataArray->filename, in);

		ofstream outfTemp;
		if (pDataArray->fastaFileTemp != "") { File::openOutputFile(pDataArray->fastaFileTemp, outfTemp); }
		...
		if ((pDataArray->start == 0) || (pDataArray->start == 1)) {
			...
			in.seekg(pDataArray->start - 1); pDataArray->File::gobble(in);
		}
		...
		for (int i = 0; i < pDataArray->end; i++) { //end is the number of sequences to process
			...
			if (ctrlc_pressed) { in.close(); out.close(); outAcc.close(); pDataArray->File::remove(pDataArray->outFasta); pDataArray->File::remove(pDataArray->outAccnos); if (pDataArray->fastaFileTemp != "") { outfTemp.close(); pDataArray->File::remove(pDataArray->fastaFileTemp); } return 0; }

			Sequence seq(in); pDataArray->File::gobble(in);
			...
			//report progress
			if ((pDataArray->count) % 1000 == 0) { LOG(INFO) << toString(pDataArray->count) << '\n'; }
		...
		LOG(FATAL) << e.what() << " in ChopsSeqsCommand, MyChopThreadFunction";
		exit(1);
	}
}
#endif
