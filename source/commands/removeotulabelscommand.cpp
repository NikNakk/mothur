...
vector<string> RemoveOtuLabelsCommand::setParameters() {
	try {
		...
		LOG(FATAL) << e.what() << " in RemoveOtuLabelsCommand, setParameters";
		exit(1);
	}
}
//**********************************************************************************************************************
string RemoveOtuLabelsCommand::getHelpString() {
	try {
		string helpString = "The remove.otulabels command can be used to remove specific otus with the output from classify.otu, otu.association, or corr.axes. It can also be used to select a set of otus from a shared or list file.\n"
			"The remove.otulabels parameters are: constaxonomy, otucorr, corraxes, shared, list, label and accnos.\n"
			...
			"The remove.otulabels commmand should be in the following format: \n"
			"remove.otulabels(accnos=yourListOfOTULabels, corraxes=yourCorrAxesFile)\n";
		return helpString;
	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in RemoveOtuLabelsCommand, getHelpString";
		exit(1);
	}
}
//**********************************************************************************************************************
...
//**********************************************************************************************************************
RemoveOtuLabelsCommand::RemoveOtuLabelsCommand(Settings& settings) : Command(settings) {
	try {
		...
		LOG(FATAL) << e.what() << " in RemoveOtuLabelsCommand, RemoveOtuLabelsCommand";
		exit(1);
	}
}
//**********************************************************************************************************************
RemoveOtuLabelsCommand::RemoveOtuLabelsCommand(Settings& settings, string option) : Command(settings, option) {
...
int RemoveOtuLabelsCommand::execute() {
	...
	if (app.isDebug) { LOG(DEBUG) << "numlabels = " + toString(otulabels.size()) + "\n"; }
...
int RemoveOtuLabelsCommand::readClassifyOtu() {
	...
	while (!in.eof()) {
		...
		if (app.isDebug) { LOG(DEBUG) << "" + otu + toString(size) + tax + "\n"; }
	...
	if (wroteSomething == false) { LOG(INFO) << "Your file only contains labels from the .accnos file." << '\n'; }
	outputNames.push_back(outputFileName);  outputTypes["constaxonomy"].push_back(outputFileName);

	LOG(INFO) << "Removed " + toString(removedCount) + " otus from your constaxonomy file." << '\n';
...
int RemoveOtuLabelsCommand::readOtuAssociation() {
	...
	LOG(INFO) << "Removed " + toString(removedCount) + " lines from your otu.corr file." << '\n';
...
int RemoveOtuLabelsCommand::readCorrAxes() {
	...
	LOG(INFO) << "Removed " + toString(removedCount) + " lines from your corr.axes file." << '\n';
...
int RemoveOtuLabelsCommand::readShared() {
	...
	for (int i = 0; i < lookup[0]->getNumBins(); i++) {
		...
		if (otulabels.count(m->getSimpleLabel(settings.currentSharedBinLabels[i])) == 0) {
			wroteSomething = true;
	...
	if (wroteSomething == false) { LOG(INFO) << "Your file contains only OTUs from the .accnos file." << '\n'; }
