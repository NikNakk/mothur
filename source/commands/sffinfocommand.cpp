...
vector<string> SffInfoCommand::setParameters() {
	try {
		...
		nkParameters.add(new StringParameter("sfftxt", "", false, false));
		CommandParameter pflow("flow", "Boolean", "", "T", "", "", "", "flow", false, false); parameters.push_back(pflow);
		nkParameters.add(new BooleanParameter("trim", true, false, false));
		...
		LOG(FATAL) << e.what() << " in SffInfoCommand, setParameters";
		exit(1);
	}
}
//**********************************************************************************************************************
string SffInfoCommand::getHelpString() {
	try {
		string helpString = "The sffinfo command reads a sff file and extracts the sequence data, or you can use it to parse a sfftxt file.\n"
			"The sffinfo command parameters are sff, fasta, qfile, accnos, flow, sfftxt, oligos, group, bdiffs, tdiffs, ldiffs, sdiffs, pdiffs, checkorient and trim. sff is required. \n"
			"The sff parameter allows you to enter the sff file you would like to extract data from.  You may enter multiple files by separating them by -'s.\n"
			"The fasta parameter allows you to indicate if you would like a fasta formatted file generated.  Default=True. \n"
			"The qfile parameter allows you to indicate if you would like a quality file generated.  Default=True. \n"
			"The oligos parameter allows you to provide an oligos file to split your sff file into separate sff files by barcode. \n"
			"The group parameter allows you to provide a group file to split your sff file into separate sff files by group. \n"
			...
			"The flow parameter allows you to indicate if you would like a flowgram file generated.  Default=True. \n"
			"The sfftxt parameter allows you to indicate if you would like a sff.txt file generated.  Default=False. \n"
			"If you want to parse an existing sfftxt file into flow, fasta and quality file, enter the file name using the sfftxt parameter. \n"
			"The trim parameter allows you to indicate if you would like a sequences and quality scores trimmed to the clipQualLeft and clipQualRight values.  Default=True. \n"
			...
			"Example sffinfo(sff=mySffFile.sff, trim=F).\n"
			"Note: No spaces between parameter labels (i.e. sff), '=' and parameters (i.e.yourSffFileName).\n";
		return helpString;
	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in SffInfoCommand, getHelpString";
		exit(1);
	}
}
...
//**********************************************************************************************************************
SffInfoCommand::SffInfoCommand(Settings& settings) : Command(settings) {
	try {
		...
		LOG(FATAL) << e.what() << " in SffInfoCommand, SffInfoCommand";
		exit(1);
	}
}
//**********************************************************************************************************************

SffInfoCommand::SffInfoCommand(Settings& settings, string option) : Command(settings, option) {
	...
	if (option == "help") { help(); abort = true; calledHelp = true; }
		...
		if (sffFilename == "not found") { sffFilename = ""; }
			...
			if (filenames.size() == 0) { LOG(INFO) << "no valid files." << '\n'; abort = true; }
		}
		...
		if (accnosName == "not found") { accnosName = ""; }
			...
			hasAccnos = true;
			Utility::split(accnosName, '-', accnosFileNames);

			//go through files and make sure they are good, if not, then disregard them
			for (int i = 0; i < accnosFileNames.size(); i++) {
				bool ignore = false;
				if (accnosFileNames[i] == "current") {
					accnosFileNames[i] = settings.getCurrent("accnos");
					if (accnosFileNames[i] != "") { LOG(INFO) << "Using " + accnosFileNames[i] + " as input file for the accnos parameter where you had given current." << '\n'; }
					else {
						LOG(INFO) << "You have no current accnosfile, ignoring current." << '\n'; ignore = true;
						//erase from file list
				...
				if (!ignore) {
					...
					if (inputDir != "") {
						string path = File::getPath(accnosFileNames[i]);
						//if the user has not given a path then, add inputdir. else leave path alone.
						if (path == "") { accnosFileNames[i] = inputDir + accnosFileNames[i]; }
					}

					ifstream in;
					int ableToOpen = File::openInputFile(accnosFileNames[i], in, "noerror");

					//if you can't open it, try default location
					if (ableToOpen == 1) {
						if (settings.getDefaultPath() != "") { //default path is set
							string tryPath = settings.getDefaultPath() + File::getSimpleName(accnosFileNames[i]);
							LOG(INFO) << "Unable to open " + accnosFileNames[i] + ". Trying default " + tryPath << '\n';
							ifstream in2;
							ableToOpen = File::openInputFile(tryPath, in2, "noerror");
							in2.close();
					...
					if (ableToOpen == 1) {
						if (settings.getOutputDir() != "") { //default path is set
							string tryPath = settings.getOutputDir() + File::getSimpleName(accnosFileNames[i]);
							LOG(INFO) << "Unable to open " + accnosFileNames[i] + ". Trying output directory " + tryPath << '\n';
							ifstream in2;
							ableToOpen = File::openInputFile(tryPath, in2, "noerror");
							in2.close();
							accnosFileNames[i] = tryPath;
					...
					if (ableToOpen == 1) {
						LOG(INFO) << "Unable to open " + accnosFileNames[i] + ". It will be disregarded." << '\n';
						//erase from file list
						accnosFileNames.erase(accnosFileNames.begin() + i);
						i--;
					}
			...
			//make sure there is at least one valid file left
			if (accnosFileNames.size() == 0) { LOG(INFO) << "no valid files." << '\n'; abort = true; }
		}
		...
		if (oligosfile == "not found") { oligosfile = ""; }
			...
			hasOligos = true;
			Utility::split(oligosfile, '-', oligosFileNames);

			//go through files and make sure they are good, if not, then disregard them
			for (int i = 0; i < oligosFileNames.size(); i++) {
				bool ignore = false;
				if (oligosFileNames[i] == "current") {
					oligosFileNames[i] = settings.getCurrent("oligos");
					if (oligosFileNames[i] != "") { LOG(INFO) << "Using " + oligosFileNames[i] + " as input file for the oligos parameter where you had given current." << '\n'; }
					else {
						LOG(INFO) << "You have no current oligosfile, ignoring current." << '\n'; ignore = true;
						//erase from file list
				...
				if (!ignore) {
					...
					if (inputDir != "") {
						string path = File::getPath(oligosFileNames[i]);
						//if the user has not given a path then, add inputdir. else leave path alone.
						if (path == "") { oligosFileNames[i] = inputDir + oligosFileNames[i]; }
					}

					ifstream in;
					int ableToOpen = File::openInputFile(oligosFileNames[i], in, "noerror");

					//if you can't open it, try default location
					if (ableToOpen == 1) {
						if (settings.getDefaultPath() != "") { //default path is set
							string tryPath = settings.getDefaultPath() + File::getSimpleName(oligosFileNames[i]);
							LOG(INFO) << "Unable to open " + oligosFileNames[i] + ". Trying default " + tryPath << '\n';
							ifstream in2;
							ableToOpen = File::openInputFile(tryPath, in2, "noerror");
							in2.close();
					...
					if (ableToOpen == 1) {
						if (settings.getOutputDir() != "") { //default path is set
							string tryPath = settings.getOutputDir() + File::getSimpleName(oligosFileNames[i]);
							LOG(INFO) << "Unable to open " + oligosFileNames[i] + ". Trying output directory " + tryPath << '\n';
							ifstream in2;
							ableToOpen = File::openInputFile(tryPath, in2, "noerror");
							in2.close();
							oligosFileNames[i] = tryPath;
					...
					if (ableToOpen == 1) {
						LOG(INFO) << "Unable to open " + oligosFileNames[i] + ". It will be disregarded." << '\n';
						//erase from file list
						oligosFileNames.erase(oligosFileNames.begin() + i);
						i--;
					}
			...
			//make sure there is at least one valid file left
			if (oligosFileNames.size() == 0) { LOG(INFO) << "no valid oligos files." << '\n'; abort = true; }
		}
		...
		if (groupfile == "not found") { groupfile = ""; }
			...
			for (int i = 0; i < groupFileNames.size(); i++) {
				...
				if (!ignore) {
					...
					ifstream in;
					int ableToOpen = File::openInputFile(groupFileNames[i], in, "noerror");
			...
			//make sure there is at least one valid file left
			if (groupFileNames.size() == 0) { LOG(INFO) << "no valid group files." << '\n'; abort = true; }
		}

		if (hasGroup) {
			split = 2;
			if (groupFileNames.size() != filenames.size()) { abort = true; LOG(INFO) << "If you provide a group file, you must have one for each sff file." << '\n'; }
		}

		if (hasOligos) {
			split = 2;
			if (oligosFileNames.size() != filenames.size()) { abort = true; LOG(INFO) << "If you provide an oligos file, you must have one for each sff file." << '\n'; }
		}

		if (hasGroup && hasOligos) { LOG(INFO) << "You must enter ONLY ONE of the following: oligos or group." << '\n'; abort = true; }

		if (hasAccnos) {
			if (accnosFileNames.size() != filenames.size()) { abort = true; LOG(INFO) << "If you provide a accnos file, you must have one for each sff file." << '\n'; }
		}
		...
		if ((sfftxtFilename == "") && (filenames.size() == 0)) {
			...
			string filename = settings.getCurrent("sff");
			if (filename != "") { filenames.push_back(filename); LOG(INFO) << "Using " + filename + " as input file for the sff parameter." << '\n'; }
			else { LOG(LOGERROR) << "you must provide a valid sff or sfftxt file." << '\n'; abort = true; }
		}
...
int SffInfoCommand::execute() {
	...
	for (int s = 0; s < filenames.size(); s++) {
		...
		LOG(INFO) << "Extracting info from " + filenames[s] + " ..." << '\n';
		...
		LOG(INFO) << "It took " + toString(time(NULL) - start) + " secs to extract " + toString(numReads) + ".\n";
	}
...
int SffInfoCommand::extractSffInfo(string input, string accnos, string oligos) {
	...
	if (outputDir == "") { outputDir += File::getPath(input); }
	...
	if (hasOligos) {
		...
		if (ctrlc_pressed) { delete oligosObject; return 0; }
	...
	string rootName = outputDir + File::getRootName(File::getSimpleName(input));
	...
	if (sfftxt) { File::openOutputFile(sfftxtFileName, outSfftxt); outSfftxt.setf(ios::fixed, ios::floatfield); outSfftxt.setf(ios::showpoint);  outputNames.push_back(sfftxtFileName);  outputTypes["sfftxt"].push_back(sfftxtFileName); }
	if (fasta) { File::openOutputFile(outFastaFileName, outFasta);	outputNames.push_back(outFastaFileName); outputTypes["fasta"].push_back(outFastaFileName); }
	if (qual) { File::openOutputFile(outQualFileName, outQual);		outputNames.push_back(outQualFileName); outputTypes["qfile"].push_back(outQualFileName); }
	if (flow) { File::openOutputFile(outFlowFileName, outFlow);		outputNames.push_back(outFlowFileName);  outFlow.setf(ios::fixed, ios::floatfield); outFlow.setf(ios::showpoint); outputTypes["flow"].push_back(outFlowFileName); }
	...
	//ofstream outtemp;
	//File::openOutputFileBinary("./temp", outtemp);
	...
	if (split > 1) {
			...
			for (int j = 0;j < filehandles[0].size();j++) {
				...
				if (filehandles[i][j] != "") {
					if (namesToRemove.count(filehandles[i][j]) == 0) {
						if (File::isBlank(filehandles[i][j])) {
							//cout << i << '\t' << '\t' << j  << '\t' << filehandles[i][j] << " is blank removing" << endl;
							File::remove(filehandles[i][j]);
							File::remove(filehandlesHeaders[i][j]);
		...
		for (int i = 0; i < filehandles.size(); i++) {
			for (int j = 0; j < filehandles[i].size(); j++) {
				if (filehandles[i][j] != "") {
					...
					if (numSplitReads[i][j] == 0) { File::remove(filehandles[i][j]); }
				}
			}
		}
		//cout << "here3" << endl;
		...
		//cout << "here4" << endl;
		if (File::isBlank(noMatchFile)) { File::remove(noMatchFile); }
...
int SffInfoCommand::adjustCommonHeader(CommonHeader header) {
	...
	File::openOutputFileBinary(tempNoHeader, outNoMatchHeader);
	...
	for (int i = 0; i < filehandlesHeaders.size(); i++) {
		for (int j = 0; j < filehandlesHeaders[i].size(); j++) {
			ofstream out;
			int able = File::openOutputFileBinaryAppend(filehandlesHeaders[i][j], out);
	...
	if (!in.eof()) {
		...
		if (split > 1) {
			...
			if (trashCodeLength == 0) {
				ofstream out;
				File::openOutputFileBinaryAppend(filehandles[barcodeIndex][primerIndex], out);
				...
				ofstream out;
				File::openOutputFileBinaryAppend(noMatchFile, out);
				out.write(mybuffer, in2.gcount());
				out.close();
				numNoMatch++;
			}
...
int SffInfoCommand::printFlowSeqData(ofstream& out, seqRead& read, Header& header) {
	...
	if (header.clipQualRight == 0) {
		...
		if (app.isDebug) { LOG(DEBUG) << "" + header.name + " has clipQualRight=0.\n"; }
	}
...
int SffInfoCommand::readAccnosFile(string filename) {
	...
	while (!in.eof()) {
		...
		if (ctrlc_pressed) { seqNames.clear(); break; }
	}
	in.close();
...
int SffInfoCommand::parseSffTxt() {
	...
	ifstream inSFF;
	File::openInputFile(sfftxtFilename, inSFF);

	if (outputDir == "") { outputDir += File::getPath(sfftxtFilename); }

	//output file names
	ofstream outFasta, outQual, outFlow;
	string outFastaFileName, outQualFileName;
	string fileRoot = File::getRootName(File::getSimpleName(sfftxtFilename));
	if (fileRoot.length() > 0) {
		//rip off last .
		fileRoot = fileRoot.substr(0, fileRoot.length() - 1);
		fileRoot = File::getRootName(fileRoot);
	}
	...
	//read common header
	string commonHeader = File::getline(inSFF);
	string magicNumber = File::getline(inSFF);
	string version = File::getline(inSFF);
	string indexOffset = File::getline(inSFF);
	string indexLength = File::getline(inSFF);
	int numReads = parseHeaderLineToInt(inSFF);
	string headerLength = File::getline(inSFF);
	string keyLength = File::getline(inSFF);
	int numFlows = parseHeaderLineToInt(inSFF);
	string flowgramCode = File::getline(inSFF);
	string flowChars = File::getline(inSFF);
	string keySequence = File::getline(inSFF);
	File::gobble(inSFF);
	...
	for (int i = 0;i < numReads;i++) {
		...
		//sanity check
		if (inSFF.eof()) { LOG(LOGERROR) << "Expected " + toString(numReads) + " but reached end of file at " + toString(i + 1) + "." << '\n'; break; }
		...
		//report progress
		if ((i + 1) % 10000 == 0) { LOG(INFO) << toString(i + 1) << '\n'; }

		if (ctrlc_pressed) { break; }
	}

	//report progress
	if (!ctrlc_pressed) { if ((numReads) % 10000 != 0) { LOG(INFO) << toString(numReads) << '\n'; } }
...
bool SffInfoCommand::readOligos(string oligoFile) {
	...
	if (oligosObject->hasPairedPrimers() || oligosObject->hasPairedBarcodes()) {
		pairedOligos = true;
		LOG(LOGERROR) << "sffinfo does not support paired barcodes and primers, aborting.\n"; ctrlc_pressed = true; return true;
	}
	else {
		pairedOligos = false;
		...
		for (map<string, int>::iterator itBar = barcodes.begin();itBar != barcodes.end();itBar++) {
			for (map<string, int>::iterator itPrimer = primers.begin();itPrimer != primers.end(); itPrimer++) {
				...
				if ((primerName == "ignore") || (barcodeName == "ignore")) {} //do nothing
					...
					ofstream temp;
					map<string, string> variables;
					variables["[filename]"] = outputDir + File::getRootName(File::getSimpleName(currentFileName));
	...
	for (int i = 0; i < filehandles.size(); i++) {
		...
		for (int j = 0; j < filehandles[i].size(); j++) {
			...
			ofstream temp;
			File::openOutputFileBinary(filehandles[i][j] + "headers", temp); temp.close();
		}
	}

	if (allBlank) {
		LOG(WARNING) << "your oligos file does not contain any group names.  mothur will not create a split the sff file." << '\n';
		split = 1;
		return false;
	}
...
bool SffInfoCommand::readGroup(string oligoFile) {
	...
	for (int i = 0; i < filehandles.size(); i++) {
		...
		for (int j = 0; j < filehandles[i].size(); j++) {
			...
			ofstream temp;
			File::openOutputFileBinary(thisHeader, temp); temp.close();
		}
	}
