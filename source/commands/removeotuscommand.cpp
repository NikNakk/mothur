...
vector<string> RemoveOtusCommand::setParameters() {
	try {
		...
		LOG(FATAL) << e.what() << " in RemoveOtusCommand, setParameters";
		exit(1);
	}
}
//**********************************************************************************************************************
string RemoveOtusCommand::getHelpString() {
	try {
		string helpString = "The remove.otus command removes otus containing sequences from a specfic group or set of groups.\n"
			"It outputs a new list file containing the otus containing sequences NOT from in the those specified groups.\n"
			"The remove.otus command parameters are accnos, group, list, label and groups. The group and list parameters are required, unless you have valid current files.\n"
			...
			"The remove.otus command should be in the following format: remove.otus(accnos=yourAccnos, list=yourListFile, group=yourGroupFile, label=yourLabel).\n"
			"Example remove.otus(accnos=amazon.accnos, list=amazon.fn.list, group=amazon.groups, label=0.03).\n"
			"or remove.otus(groups=pasture, list=amazon.fn.list, amazon.groups, label=0.03).\n"
			"Note: No spaces between parameter labels (i.e. list), '=' and parameters (i.e.yourListFile).\n";
		return helpString;
	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in RemoveOtusCommand, getHelpString";
		exit(1);
	}
}
//**********************************************************************************************************************
...
///**********************************************************************************************************************
RemoveOtusCommand::RemoveOtusCommand(Settings& settings) : Command(settings) {
	try {
		...
		LOG(FATAL) << e.what() << " in RemoveOtusCommand, RemoveOtusCommand";
		exit(1);
	}
}

//**********************************************************************************************************************
RemoveOtusCommand::RemoveOtusCommand(Settings& settings, string option) : Command(settings, option) {
...
int RemoveOtusCommand::execute() {
	...
	LOG(FATAL) << e.what() << " in RemoveOtusCommand, execute";
	exit(1);
}
}
//**********************************************************************************************************************
int RemoveOtusCommand::readListGroup() {
	try {
		...
		if (wroteSomething == false) { LOG(INFO) << "At distance " + label + " your file ONLY contains otus containing sequences from the groups you wish to remove." << '\n'; }
	...
	int RemoveOtusCommand::processList(ListVector*& list, GroupMap*& groupMap, ofstream& out, ofstream& outGroup, bool& wroteSomething) {
		...
		LOG(INFO) << newList.getLabel() + " - removed " + toString(numOtus) + " of the " + toString(list->getNumBins()) + " OTUs." << '\n';
