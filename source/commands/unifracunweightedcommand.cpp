...
vector<string> UnifracUnweightedCommand::setParameters() {
	try {
		...
		LOG(FATAL) << e.what() << " in UnifracUnweightedCommand, setParameters";
		exit(1);
	}
}
//**********************************************************************************************************************
string UnifracUnweightedCommand::getHelpString() {
	try {
		string helpString = "The unifrac.unweighted command parameters are tree, group, name, count, groups, iters, distance, processors, root and random.  tree parameter is required unless you have valid current tree file.\n"
			...
			"The distance parameter allows you to create a distance file from the results. The default is false. You may set distance to lt, square or column.\n"
			"The random parameter allows you to shut off the comparison to random trees. The default is false, meaning compare don't your trees with randomly generated trees.\n"
			...
			"The unifrac.unweighted command should be in the following format: unifrac.unweighted(groups=yourGroups, iters=yourIters).\n"
			...
			"The consensus parameter allows you to indicate you would like trees built from distance matrices created with the results of the subsampling, as well as a consensus tree built from these trees. Default=F.\n"
			"Example unifrac.unweighted(groups=A-B-C, iters=500).\n"
			"The default value for groups is all the groups in your groupfile, and iters is 1000.\n"
			"The unifrac.unweighted command output two files: .unweighted and .uwsummary their descriptions are in the manual.\n"
			"Note: No spaces between parameter labels (i.e. groups), '=' and parameters (i.e.yourGroups).\n";
		return helpString;
	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in UnifracUnweightedCommand, getHelpString";
		exit(1);
	}
}
//**********************************************************************************************************************
...
//**********************************************************************************************************************
UnifracUnweightedCommand::UnifracUnweightedCommand(Settings& settings) : Command(settings) {
	try {
		...
		LOG(FATAL) << e.what() << " in UnifracUnweightedCommand, UnifracUnweightedCommand";
		exit(1);
	}
}
/***********************************************************/
UnifracUnweightedCommand::UnifracUnweightedCommand(Settings& settings, string option) : Command(settings, option) {
...
int UnifracUnweightedCommand::execute() {
	...
	LOG(INFO) << "Tree#\tGroups\tUWScore\t";
	if (random) { outSum << "UWSig"; LOG(INFO) << "UWSig"; }
	...
	for (int i = 0; i < T.size(); i++) {
		...
		if (subsample) { LOG(INFO) << "It took " + toString(time(NULL) - startSubsample) + " secs to run the subsampling." << '\n'; }
	...
	LOG(INFO) << "It took " + toString(time(NULL) - start) + " secs to run unifrac.unweighted." << '\n';
...
int UnifracUnweightedCommand::getAverageSTDMatrices(vector< vector<double> >& dists, int treeNum) {
	...
	if (app.isDebug) { LOG(DEBUG) << "about to fill matrix.\n"; }
	...
	if (app.isDebug) { LOG(DEBUG) << "done filling matrix.\n"; }
	...
	for (int a = 0; a < numComp; a++) {
		...
		if (random) {
			if (UWScoreSig[a][0] >(1 / (float)iters)) {
				...
				LOG(FILEONLY) << groupComb[a] + "\t" + toString(utreeScores[a][0]) + "\t" + toString(UWScoreSig[a][0]) + "\n";
			}
			else {
				...
				LOG(FILEONLY) << groupComb[a] + "\t" + toString(utreeScores[a][0]) + "\t<" + toString((1 / float(iters))) + "\n";
			}
		}
		else {
			outSum << setprecision(6) << groupComb[a] << '\t' << utreeScores[a][0] << endl;
			cout << setprecision(6) << groupComb[a] << '\t' << utreeScores[a][0] << endl;
			LOG(FILEONLY) << groupComb[a] + "\t" + toString(utreeScores[a][0]) + "\n";
		}
	}
