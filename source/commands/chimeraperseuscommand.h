...
public:
	ChimeraPerseusCommand(Settings& settings, string option);
	ChimeraPerseusCommand(Settings& settings);
...
static DWORD WINAPI MyPerseusThreadFunction(LPVOID lpParam) {
	...
	try {
		...
		File::openOutputFile(pDataArray->outputFName, out); out.close();
		File::openOutputFile(pDataArray->accnos, out1); out1.close();
		...
		for (int u = pDataArray->start; u < pDataArray->end; u++) {
			...
			int start = time(NULL);	 if (ctrlc_pressed) { if (pDataArray->hasCount) { delete cparser; } { delete parser; } pDataArray->File::remove(pDataArray->outputFName); pDataArray->File::remove(pDataArray->accnos); return 0; }

			LOG(INFO) << '\n' << "Checking sequences from group " + pDataArray->groups[u] + "..." + '\n';
			...
			if (ctrlc_pressed) { if (pDataArray->hasCount) { delete cparser; } { delete parser; } pDataArray->File::remove(pDataArray->outputFName); pDataArray->File::remove(pDataArray->accnos); return 0; }
			...
			for (int j = 0;j < numSeqs;j++) {
				...
				if (ctrlc_pressed) { if (pDataArray->hasCount) { delete cparser; } { delete parser; } pDataArray->File::remove(pDataArray->outputFName); pDataArray->File::remove(pDataArray->accnos); chimeraFile.close(); pDataArray->File::remove(chimeraFileName); accnosFile.close(); pDataArray->File::remove(accnosFileName); return 0; }
				...
				//report progress
				if ((j + 1) % 100 == 0) { LOG(SCREENONLY) << "Processing sequence: " + toString(j + 1) + "\n"; }
			}
			...
			if (pDataArray->dups) {
				if (!pDataArray->File::isBlank(accnosFileName)) {
					ifstream in;
					File::openInputFile(accnosFileName, in);
					string name;
					if (pDataArray->hasCount) {
						while (!in.eof()) {
						...
						ofstream out;
						File::openOutputFile(accnosFileName + ".temp", out);
						while (!in.eof()) {
							in >> name; pDataArray->File::gobble(in);
			...
			//append files
			pDataArray->File::appendFiles(chimeraFileName, pDataArray->outputFName); pDataArray->File::remove(chimeraFileName);
			pDataArray->File::appendFiles(accnosFileName, pDataArray->accnos); pDataArray->File::remove(accnosFileName);
			LOG(INFO) << '\n' << "It took " + toString(time(NULL) - start) + " secs to check " + toString(numSeqs) + " sequences from group " + pDataArray->groups[u] + "." << '\n';
		...
		LOG(FATAL) << e.what() << " in ChimeraUchimeCommand, MyPerseusThreadFunction";
		exit(1);
	}
}
/**************************************************************************************************/
