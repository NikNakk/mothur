...
vector<string> HeatMapSim::getPic(vector<SharedRAbundVector*> lookup, vector<Calculator*> calcs) {
	...
	for (int k = 0; k < calcs.size(); k++) {
		...
		if (ctrlc_pressed) { return outputNames; }

		string filenamesvg = outputDir + File::getRootName(File::getSimpleName(inputfile)) + lookup[0]->getLabel() + "." + calcs[k]->getName() + ".heatmap.sim.svg";
...
string HeatMapSim::getPic(vector< vector<double> > dists, vector<string> groups) {
	...
	string filenamesvg = outputDir + File::getRootName(File::getSimpleName(inputfile)) + "heatmap.sim.svg";
	File::openOutputFile(filenamesvg, outsvg);
	...
	for (int i = 0; i < (dists.size() - 1); i++) {
		for (int j = (i + 1); j < dists.size(); j++) {
			...
			if (ctrlc_pressed) { outsvg.close(); return filenamesvg; }
...
void HeatMapSim::printLegend(int y, float maxSim) {
	try {
		...
		LOG(FATAL) << e.what() << " in HeatMapSim, printLegend";
		exit(1);
	}
}
