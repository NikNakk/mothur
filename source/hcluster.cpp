...
HCluster::HCluster(RAbundVector* rav, ListVector* lv, string ms, string d, NameAssignment* n, float c) : rabund(rav), list(lv), method(ms), distfile(d), nameMap(n), cutoff(c) {
	...
	if ((method == "furthest") || (method == "nearest")) {
		File::openInputFile(distfile, filehandle);
	}
	else {
		processFile();
	}
...
void HCluster::setMapWanted(bool ms) {
	try {
		...
		LOG(FATAL) << e.what() << " in HCluster, setMapWanted";
		exit(1);
	}
}
/***********************************************************************/
void HCluster::updateMap() {
	try {
		...
		LOG(FATAL) << e.what() << " in HCluster, updateMap";
		exit(1);
	}
}
//**********************************************************************************************************************
...
vector<seqDist> HCluster::getSeqsFNNN() {
	...
	while (!filehandle.eof()) {
		...
		filehandle >> firstName >> secondName >> distance;    File::gobble(filehandle);
...
vector<seqDist> HCluster::getSeqsAN() {
	...
	File::openInputFile(distfile, filehandle, "no error");
...
int HCluster::combineFile() {
	...
	ofstream out;
	File::openOutputFile(tempDistFile, out);

	//FILE* in;
	//in = fopen(distfile.c_str(), "rb");

	ifstream in;
	File::openInputFile(distfile, in, "no error");
...
//cout << "next distance = " << first << '\t' << second << '\t' << dist << endl;		   
		...
		if (ctrlc_pressed) { in.close(); out.close(); File::remove(tempDistFile); return 0; }
...
int HCluster::processFile() {
	...
	//get entry
	while (!in.eof()) {
		if (ctrlc_pressed) { in.close(); out.close(); File::remove(outTemp); return 0; }
