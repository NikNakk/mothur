...
Bellerophon::Bellerophon(string name, bool filterSeqs, bool c, int win, int inc, int p, string o) : Chimera() {
	...
	numSeqs = seqs.size();
	if (numSeqs == 0) { LOG(INFO) << "Error in reading you sequences." << '\n'; exit(1); }
	...
	if (window == 0) { window = seq0.length() / 4; }
		...
		LOG(INFO) << "Your sequence length is = " + toString(seq0.length()) + ". You have selected a window size greater than the length of half your aligned sequence. I will run it with a window size of " + toString((seq0.length() / 2)) << '\n';
	...
	if (increment > (seqs[0]->getAlignLength() - (2 * window))) {
		if (increment != 10) {
			...
			LOG(INFO) << "You have selected a increment that is too large. I will use the default." << '\n';
			increment = 10;
...
int Bellerophon::print(ostream& out, ostream& outAcc, string s) {
	...
	if (ctrlc_pressed) { return numSeqs; }
	...
	for (int i = 0; i < best.size(); i++) {
		...
		if (best[i].score > 1.0) {
			...
			LOG(INFO) << best[i].name + " is a suspected chimera at breakpoint " + toString(best[i].midpoint) << '\n';
			LOG(INFO) << "It's score is " + toString(best[i].score) + " with suspected left parent " + best[i].leftParent + " and right parent " + best[i].rightParent << '\n';
		}
	}
	...
	int spot;
	spot = best.size() - 1;
	LOG(INFO) << "Minimum:\t" + toString(best[spot].score) << '\n';
	spot = best.size() * 0.975;
	LOG(INFO) << "2.5%-tile:\t" + toString(best[spot].score) << '\n';
	spot = best.size() * 0.75;
	LOG(INFO) << "25%-tile:\t" + toString(best[spot].score) << '\n';
	spot = best.size() * 0.50;
	LOG(INFO) << "Median: \t" + toString(best[spot].score) << '\n';
	spot = best.size() * 0.25;
	LOG(INFO) << "75%-tile:\t" + toString(best[spot].score) << '\n';
	spot = best.size() * 0.025;
	LOG(INFO) << "97.5%-tile:\t" + toString(best[spot].score) << '\n';
	spot = 0;
	LOG(INFO) << "Maximum:\t" + toString(best[spot].score) << '\n';
...
int Bellerophon::driverChimeras(vector<int> midpoints, linePair line) {
	...
	for (int h = line.start; h < (line.start + line.num); h++) {
		...
		if (ctrlc_pressed) { delete SparseLeft; delete SparseRight; return 0; }
		...
		//report progress
		if ((h + 1) % 10 == 0) { LOG(SCREENONLY) << "Processing sliding window: " + toString(h + 1) + "\n"; }

	}

	//report progress
	if ((line.start + line.num) % 10 != 0) { LOG(SCREENONLY) << "Processing sliding window: " + toString(line.start + line.num) + "\n"; }
...
int Bellerophon::writePrefs(string file, linePair tempLine) {
	...
	ofstream outTemp;
	File::openOutputFile(file, outTemp);
	...
	for (int i = tempLine.start; i < (tempLine.start + tempLine.num); i++) {
		...
		for (int j = 0; j < numSeqs; j++) {
			...
			if (ctrlc_pressed) { outTemp.close(); File::remove(file); return 0; }
...
int Bellerophon::readPrefs(string file) {
	...
	ifstream inTemp;
	File::openInputFile(file, inTemp);

	int start, num;

	//lets you know what part of the pref matrix you are writing
	inTemp >> start >> num;  File::gobble(inTemp);

	for (int i = start; i < num; i++) {

		for (int j = 0; j < numSeqs; j++) {

			if (ctrlc_pressed) { inTemp.close(); File::remove(file); return 0; }
