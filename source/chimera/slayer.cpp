...
Slayer::Slayer(int win, int increment, int parentThreshold, float div, int i, int snp, int mi) :
	minBS(mi), windowSize(win), windowStep(increment), parentFragmentThreshold(parentThreshold), divRThreshold(div), iters(i), percentSNPSample(snp) {
...
//								
//							if (ctrlc_pressed) { delete q; delete leftParent; delete rightParent; return "no"; }
//								
//							divs[k].bsa = BS_A;
//							divs[k].bsb = BS_B;
//							divs[k].bsMax = max(BS_A, BS_B);
...
vector<data_struct> Slayer::runBellerophon(Sequence q, Sequence pA, Sequence pB, map<int, int>& spots) {
	try {
		...
		if (length < (2 * windowSize + windowStep)) {
			//			LOG(INFO) << "Your window size is too large for " + q->getName() + ". I will make the window size " + toString(length/4) + " which is 1/4 the filtered length." << '\n';	
			windowSize = length / 4;
		}
...
//cout << breakpoint << '\t' << divR_QLA_QRB << '\t' << divR_QLB_QRA << endl;
		...
		LOG(FATAL) << e.what() << " in Slayer, runBellerophon";
		exit(1);
	}
}
/***********************************************************************/
