...
string Chimera::createFilter(vector<Sequence*> seqs, float t) {
	...
	for (int i = 0; i < seqs.size(); i++) {
		...
		if (seqAligned.length() != filterString.length()) { LOG(INFO) << seqs[i]->getName() + " is not the same length as the template sequences. Aborting!\n";  exit(1); }
	...
	if (threshold != 0.0) { LOG(INFO) << "Filter removed " + toString(numColRemoved) + " columns." << '\n'; }
...
vector<Sequence*> Chimera::readSeqs(string file) {
	...
	if (file == "saved") {
		...
		LOG(INFO) << "Reading sequences from " + file + "..."; cout.flush();
		...
		while (!in.eof()) {
			...
			if (ctrlc_pressed) { return container; }

			Sequence* current = new Sequence(in);  File::gobble(in);
...
void Chimera::setMask(string filename) {
	...
	if (filename == "default") {
		...
		ifstream infile;
		File::openInputFile(filename, infile);
...
Sequence* Chimera::getSequence(string name) {
	try {
		...
		LOG(FATAL) << e.what() << " in Chimera, getSequence";
		exit(1);
	}
}
//***************************************************************************************************************
