...
void DeCalculator::setMask(string ms) {
	try {
		...
		LOG(FATAL) << e.what() << " in DeCalculator, setMask";
		exit(1);
	}
}
//***************************************************************************************************************
void DeCalculator::runMask(Sequence* seq) {
	try {
		...
		LOG(FATAL) << e.what() << " in DeCalculator, runMask";
		exit(1);
	}
}
//***************************************************************************************************************
...
vector<int>  DeCalculator::findWindows(Sequence* query, int front, int back, int& size, int increment) {
	...
	if (size == 0) {
		if (cutoff > 1200) { size = 300; }
		else { size = (cutoff / 4); }
	}
	else if (size > (cutoff / 4)) {
		LOG(INFO) << "You have selected too large a window size for sequence " + query->getName() + ".  I will choose an appropriate window size." << '\n';
		size = (cutoff / 4);
	}
...
vector<float> DeCalculator::calcFreq(vector<Sequence*> seqs, string filename) {
	...
	vector<float> prob;
	string freqfile = File::getRootName(filename) + "freq";
	ofstream outFreq;

	File::openOutputFile(freqfile, outFreq);
...
vector< vector<float> > DeCalculator::getQuantiles(vector<Sequence*> seqs, vector<int> windowSizesTemplate, int window, vector<float> probProfile, int increment, int start, int end) {
	...
	for (int i = start; i < end; i++) {
		...
		LOG(INFO) << "Processing sequence " + toString(i) << '\n';
		...
		for (int j = 0; j < i; j++) {
			...
			if (ctrlc_pressed) { delete query; delete subject; return quan; }
...
void DeCalculator::removeObviousOutliers(vector< vector<float> >& quantiles, int num) {
	...
	/*
			...
			while (contributions.size() > 0) {
				...
				LOG(INFO) << "Removing scores contributed by sequence " + toString(largestContrib) + " in your template file." << '\n';
	...
	**************************************************************************************************
	...
	cout << "high = " << high << endl;
			...
			for(int i = 0; i < marked.size(); i++) {
				if (marked[i] > high) {
					LOG(INFO) << "Removing scores contributed by sequence " + toString(marked[i]) + " in your template file." << '\n';
					for (int i = 0; i < quantiles.size(); i++) {
...
cout << largest->second << '\t' << largest->first->score << '\t' << largest->first->member1 << '\t' << largest->first->member2 << endl;
		...
		LOG(FATAL) << e.what() << " in DeCalculator, sortContrib";
		exit(1);
	}
}
...
int DeCalculator::findLargestContrib(vector<int> seen) {
	try{
		...
		LOG(FATAL) << e.what() << " in DeCalculator, findLargestContribs";
		exit(1);
	}
}
***************************************************************************************************************
void DeCalculator::removeContrib(int bad, vector<quanMember>& quan) {
	try{
		...
		LOG(FATAL) << e.what() << " in DeCalculator, removeContrib";
		exit(1);
	}
}
*/
...
float DeCalculator::findAverage(vector<float> myVector) {
	try {
		...
		LOG(FATAL) << e.what() << " in DeCalculator, findAverage";
		exit(1);
	}
}
...
vector<Sequence> DeCalculator::findClosest(Sequence querySeq, vector<Sequence*>& thisTemplate, vector<Sequence*>& thisFilteredTemplate, int numWanted, int minSim) {
	...
	for (int i = 0; i < numWanted + 1; i++) {
		if (ctrlc_pressed) { return seqsMatches; }
