...
ChimeraCheckRDP::ChimeraCheckRDP(string filename, string temp, string n, bool s, int inc, int k, string o) : Chimera() {
	try {
		...
		LOG(FATAL) << e.what() << " in ChimeraCheckRDP, ChimeraCheckRDP";
		exit(1);
	}
}
//***************************************************************************************************************
...
Sequence ChimeraCheckRDP::print(ostream& out, ostream& outAcc) {
	...
	LOG(INFO) << "Processing: " + querySeq->getName() << '\n';
...
vector<sim> ChimeraCheckRDP::findIS() {
	...
	for (int f = start; f < (seq.length() - start); f += increment) {
		...
		if ((f - kmerSize) < 0) { LOG(LOGERROR) << "Sequence " + querySeq->getName() + " is too short for your kmerSize, quitting." << '\n'; ctrlc_pressed = true; }

		if (ctrlc_pressed) { return isValues; }
...
void ChimeraCheckRDP::readName(string namefile) {
	try {
		...
		LOG(FATAL) << e.what() << " in ChimeraCheckRDP, readName";
		exit(1);
	}
}
...
int ChimeraCheckRDP::calcKmers(map<int, int> query, map<int, int> subject) {
	try {
		...
		LOG(FATAL) << e.what() << " in ChimeraCheckRDP, calcKmers";
		exit(1);
	}
}
//***************************************************************************************************************
void ChimeraCheckRDP::makeSVGpic(vector<sim> info) {
	try {
		...
		ofstream outsvg;
		File::openOutputFile(file, outsvg);
		...
		LOG(FATAL) << e.what() << " in ChimeraCheckRDP, makeSVGpic";
		exit(1);
	}
}
//***************************************************************************************************************/
