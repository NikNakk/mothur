...
ChimeraSlayer::ChimeraSlayer(string file, string temp, bool trim, string mode, int k, int ms, int mms, int win, float div,
	...
	try {
		...
		LOG(FATAL) << e.what() << " in ChimeraSlayer, ChimeraSlayer";
		exit(1);
	}
}
//***************************************************************************************************************
...
int ChimeraSlayer::doPrep() {
	...
	if (searchMethod == "kmer") {
		string templatePath = File::getPath(templateFileName);
		string rightTemplateFileName = templatePath + "right." + File::getRootName(File::getSimpleName(templateFileName));
		databaseRight = new KmerDB(rightTemplateFileName, kmerSize);

		string leftTemplateFileName = templatePath + "left." + File::getRootName(File::getSimpleName(templateFileName));
		...
		//generate blastdb
		databaseLeft = new BlastDB(File::getRootName(File::getSimpleName(fastafile)), -1.0, -1.0, 1, -3, blastlocation, threadID);
...
vector<Sequence*> ChimeraSlayer::getTemplate(Sequence q, vector<Sequence*>& userTemplateFiltered) {
	...
	for (int i = 0; i < templateSeqs.size(); i++) {
		...
		if (ctrlc_pressed) { return userTemplate; }
	...
	if (searchMethod == "kmer") {
		...
		//generate blastdb
		databaseLeft = new BlastDB(File::getRootName(File::getSimpleName(templateFileName)), -1.0, -1.0, 1, -3, blastlocation, threadID);

		if (ctrlc_pressed) { return userTemplate; }

		for (int i = 0; i < userTemplate.size(); i++) { if (ctrlc_pressed) { return userTemplate; }   databaseLeft->addSequence(*userTemplate[i]); }
		databaseLeft->generateDB();
		databaseLeft->setNumSeqs(userTemplate.size());
	}
...
Sequence ChimeraSlayer::print(ostream& out, ostream& outAcc) {
	...
	if (chimeraFlags == "yes") {
		...
		if ((chimeraResults[0].bsa >= minBS && chimeraResults[0].divr_qla_qrb >= divR)
			||
			(chimeraResults[0].bsb >= minBS && chimeraResults[0].divr_qlb_qra >= divR)) {
			chimeraFlag = "yes";
		}
	...
	if ((leftPiece.flag == "yes") || (rightPiece.flag == "yes")) {
		...
		if (leftPiece.flag == "yes") {
			...
			if ((leftPiece.results[0].bsa >= minBS && leftPiece.results[0].divr_qla_qrb >= divR)
				||
				(leftPiece.results[0].bsb >= minBS && leftPiece.results[0].divr_qlb_qra >= divR)) {
				chimeraFlag = "yes";
			}
		}
		...
		if (rightPiece.flag == "yes") {
			if ((rightPiece.results[0].bsa >= minBS && rightPiece.results[0].divr_qla_qrb >= divR)
				||
				(rightPiece.results[0].bsb >= minBS && rightPiece.results[0].divr_qlb_qra >= divR)) {
				chimeraFlag = "yes";
			}
		}
...
vector<Sequence> ChimeraSlayer::getRefSeqs(Sequence q, vector<Sequence*>& thisTemplate, vector<Sequence*>& thisFilteredTemplate) {
	...
	else { LOG(INFO) << "not valid search."; exit(1); } //should never get here
...
//		else { smaller = tempIndexesLeft;  larger = tempIndexesRight;  } 
		...
		while (index < tempIndexesLeft.size() && index < tempIndexesRight.size()) {
			...
			if (ctrlc_pressed) { delete queryRight; delete queryLeft; return refResults; }
		...
		LOG(FATAL) << e.what() << " in ChimeraSlayer, getBlastSeqs";
		exit(1);
	}
}
//***************************************************************************************************************
vector<Sequence> ChimeraSlayer::getKmerSeqs(Sequence q, vector<Sequence*>& db, int num) {
	try {
		...
		LOG(FATAL) << e.what() << " in ChimeraSlayer, getKmerSeqs";
		exit(1);
	}
}
//***************************************************************************************************************
