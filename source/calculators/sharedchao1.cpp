...
void SharedChao1::initialTree(int n) {
	try {
		...
		if ((toString(e.what()) == "vector::_M_fill_insert") || (toString(e.what()) == "St9bad_alloc")) { LOG(INFO) << "You are using " + toString(n) + " groups which creates 2^" + toString(n + 1) + " nodes. Try reducing the number of groups you selected. " << '\n'; exit(1); }
		LOG(FATAL) << e.what() << " in SharedChao1, initialTree";
		exit(1);
	}
}
...
void SharedChao1::updateTree(vector<int> bin) {
	try {
		...
		LOG(FATAL) << e.what() << " in SharedChao1, updateTree";
		exit(1);
	}
}
...
void SharedChao1::printBranch(IntNode* node) {
	...
	if (node->left != NULL) {
		...
		LOG(INFO) << toString(node->lvalue) << '\n';
		LOG(INFO) << toString(node->rvalue) << '\n';
	}
