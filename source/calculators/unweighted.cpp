...
EstOutput Unweighted::getValues(Tree* t, int p, string o) {
	...
	for (int r = 0; r < numGroups; r++) {
		for (int l = 0; l < r; l++) {
			numComp++;
			vector<string> groups; groups.push_back((settings.getGroups())[r]); groups.push_back((settings.getGroups())[l]);
	...
	if (numComp != 1) {
		...
		if (numGroups == 0) {
			...
			for (int i = 0; i < settings.getNumGroups(); i++) {
				groups.push_back((settings.getGroups())[i]);
			}
			namesOfGroupCombos.push_back(groups);
		}
	}
...
EstOutput Unweighted::createProcesses(Tree* t, vector< vector<string> > namesOfGroupCombos, CountTable* ct) {
	try {
		...
		vector<EstOutput> ests(lines.size() - 1);

		//loop through and create all the processes you want
		for (int i = 1; i < lines.size(); i++) {
			thrds[i - 1] = thread(&Unweighted::driverWithData, this, ref(t), namesOfGroupCombos, lines[i].start, lines[i].num, ref(ct), ref(ests[i - 1]));
		}
		...
		for (int i = 0; i < thrds.size(); i++) {
			thrds[i].join();
			results.insert(results.end(), ests[i].begin(), ests[i].end());
		}

		return results;
	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in Unweighted, createProcesses";
		exit(1);
	}
}
/**************************************************************************************************/

void Unweighted::driverWithData(Tree* t, vector<vector<string>> namesOfGroupCombos, int start, int num, CountTable* ct, EstOutput& output) {
	output = driver(t, namesOfGroupCombos, start, num, ct);
}
EstOutput Unweighted::driver(Tree* t, vector< vector<string> > namesOfGroupCombos, int start, int num, CountTable* ct) {
	try {
		...
		for (int h = start; h < (start + num); h++) {
			...
			//sanity check
			if (nodeBelonging == -1) {
				LOG(WARNING) << "cannot find a nodes in the tree from grouping ";
				for (int g = 0; g < namesOfGroupCombos[h].size() - 1; g++) { LOG(INFO) << namesOfGroupCombos[h][g] + "-"; }
				LOG(INFO) << namesOfGroupCombos[h][namesOfGroupCombos[h].size() - 1];
				LOG(INFO) << ", skipping." << '\n'; results[count] = UW;
			}
			else {
		...
		LOG(FATAL) << e.what() << " in Unweighted, driver";
		exit(1);
	}
}
/**************************************************************************************************/
...
EstOutput Unweighted::getValues(Tree* t, string groupA, string groupB, int p, string o) {
	try {
		...
		LOG(FATAL) << e.what() << " in Unweighted, getValues";
		exit(1);
	}
}
/**************************************************************************************************/
...
EstOutput Unweighted::createProcesses(Tree* t, vector< vector<string> > namesOfGroupCombos, bool usingGroups, CountTable* ct) {
	...
	while (process != processors) {
		...
		if (pid > 0) {
			...
			ctrlc_pressed = false;
			for (int i = 0;i < processIDS.size();i++) {
				File::remove(outputDir + (toString(processIDS[i]) + ".unweighted.results.temp"));
			}
			recalc = true;
			break;
		}
...
int Unweighted::getRoot(Tree* t, int v, vector<string> grouping) {
	try {
		...
		LOG(FATAL) << e.what() << " in Unweighted, getRoot";
		exit(1);
	}
}
/**************************************************************************************************/
