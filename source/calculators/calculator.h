...
public:
	Calculator() : needsAll(false) {}
	virtual ~Calculator() {};
	Calculator(string n, int c, bool f) : name(n), cols(c), multiple(f), needsAll(false) {};
	Calculator(string n, int c, bool f, bool a) : name(n), cols(c), multiple(f), needsAll(a) {};
	...
	virtual void print(ostream& f) {
		f.setf(ios::fixed, ios::floatfield); f.setf(ios::showpoint);
	...
	void citation() { LOG(INFO) << getCitation() << '\n'; }
protected:
	EstOutput data;
	string name;
	int cols;
