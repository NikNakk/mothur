...
DecisionTree::DecisionTree(vector< vector<int> >& baseDataSet,
	...
	try {
		...
		LOG(FATAL) << e.what() << " in DecisionTree, DecisionTree";
		exit(1);
	}
}
...
int DecisionTree::calcTreeErrorRate(int& numCorrect, double& treeErrorRate) {
	...
	try {
		...
		LOG(FATAL) << e.what() << " in DecisionTree, calcTreeErrorRate";
		exit(1);
	}
}
...
void DecisionTree::buildDecisionTree() {
	...
	LOG(FATAL) << e.what() << " in DecisionTree, buildDecisionTree";
	exit(1);
}
}
...
int DecisionTree::splitRecursively(RFTreeNode* rootNode) {
	...
	vector<int> DecisionTree::selectFeatureSubsetRandomly(vector<int> globalDiscardedFeatureIndices, vector<int> localDiscardedFeatureIndices) {
		...
		while (featureSubsetIndices.size() < currentFeatureSubsetSize) {
			...
			if (ctrlc_pressed) { return featureSubsetIndices; }
	...
	int DecisionTree::printTree(RFTreeNode* treeNode, string caption) {
		try {
			...
			if (treeNode != NULL && treeNode->checkIsLeaf() == false) {
				LOG(INFO) << tabs + caption + " [ gen: " + toString(treeNode->getGeneration()) + " , id: " + toString(treeNode->nodeId) + " ] ( " + toString(treeNode->getSplitFeatureValue()) + " < X" + toString(treeNode->getSplitFeatureIndex()) + " ) ( predicted: " + toString(treeNode->outputClass) + " , misclassified: " + toString(treeNode->testSampleMisclassificationCount) + " )\n";

				printTree(treeNode->getLeftChildNode(), "left ");
				printTree(treeNode->getRightChildNode(), "right");
			}
			else {
				LOG(INFO) << tabs + caption + " [ gen: " + toString(treeNode->getGeneration()) + " , id: " + toString(treeNode->nodeId) + " ] ( classified to: " + toString(treeNode->getOutputClass()) + ", samples: " + toString(treeNode->getNumSamples()) + " , misclassified: " + toString(treeNode->testSampleMisclassificationCount) + " )\n";
			}
			return 0;
		}
		catch (exception& e) {
			LOG(FATAL) << e.what() << " in DecisionTree, printTree";
			exit(1);
		}
	}
	/***********************************************************************/
