...
AbstractDecisionTree::AbstractDecisionTree(vector<vector<int> >& baseDataSet,
	...
	try {
		...
		if (app.isDebug) {
			//LOG(INFO) << "outputClasses = " + toStringVectorInt(outputClasses);
			LOG(INFO) << "numOutputClasses = " + toString(numOutputClasses) + '\n';
		}

	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in AbstractDecisionTree, AbstractDecisionTree";
		exit(1);
	}
}
/**************************************************************************************************/
int AbstractDecisionTree::createBootStrappedSamples() {
	try {
		...
		LOG(FATAL) << e.what() << " in AbstractDecisionTree, createBootStrappedSamples";
		exit(1);
	}
}
/**************************************************************************************************/
...
int AbstractDecisionTree::getSplitPopulation(RFTreeNode* node, vector< vector<int> >& leftChildSamples, vector< vector<int> >& rightChildSamples) {
	try {
		...
		LOG(FATAL) << e.what() << " in AbstractDecisionTree, getSplitPopulation";
		exit(1);
	}
}
/**************************************************************************************************/
