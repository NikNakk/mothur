...
int RandomForest::calcForrestErrorRate() {
	...
	LOG(INFO) << "numCorrect = " + toString(numCorrect) + "\n";
	LOG(INFO) << "forrestErrorRate = " + toString(forrestErrorRate) + "\n";
...
int RandomForest::printConfusionMatrix(map<int, string> intToTreatmentMap) {
	...
	LOG(FATAL) << e.what() << " in RandomForest, printConfusionMatrix";
	exit(1);
}
}
...
int RandomForest::getMissclassifications(string filename, map<int, string> intToTreatmentMap, vector<string> names) {
	...
	int RandomForest::calcForrestVariableImportance(string filename) {
		...
		for (int i = 0; i < globalVariableRanks.size(); i++) {
			out << settings.currentSharedBinLabels[(int)globalVariableRanks[i].first] << '\t' << globalVariableImportanceList[globalVariableRanks[i].first] << endl;
		}
		out.close();
		return 0;
	}
	...
	int RandomForest::populateDecisionTrees() {
		...
		for (int i = 0; i < numDecisionTrees; i++) {
			...
			if (ctrlc_pressed) { return 0; }
			if (((i + 1) % 100) == 0) { LOG(INFO) << "Creating " + toString(i + 1) + " (th) Decision tree\n"; }
			...
			if (app.isDebug && doPruning) {
				LOG(INFO) << "Before pruning\n";
			...
			if (app.isDebug) {
				LOG(INFO) << "treeErrorRate: " + toString(treeErrorRate) + " numCorrect: " + toString(numCorrect) + "\n";
			}
			...
			if (app.isDebug) {
				LOG(INFO) << "treeErrorRate: " + toString(treeErrorRate) + " numCorrect: " + toString(numCorrect) + "\n";
				if (doPruning) {
					LOG(INFO) << "errorRateImprovement: " + toString(errorRateImprovement) + "\n";
				}
			}
		...
		if (app.isDebug && doPruning) {
			LOG(INFO) << "avgErrorRateImprovement:" + toString(avgErrorRateImprovement) + "\n";
		}
		// LOG(INFO) << "globalOutOfBagEstimates = " + toStringVectorMap(globalOutOfBagEstimates)+ "\n";
