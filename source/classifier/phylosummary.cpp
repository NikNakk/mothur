...
PhyloSummary::PhyloSummary(string refTfile, CountTable* c, bool r) {
	...
	if (!FileTest) {
		LOG(INFO) << "Error: can't find " + taxFileNameTest + "." << '\n'; exit(1);
	}
	else {
	...
	while (seqTaxonomy != "") {
		...
		if (childPointer != tree[currentNode].children.end()) {	//if the node already exists, update count and move on
			...
			if (groupmap != NULL) {
				...
				if (group == "not found") { LOG(WARNING) << "" + seqName + " is not in your groupfile, and will be included in the overall total, but not any group total." << '\n'; }
			...
			if (ignore) {
				...
				LOG(INFO) << "Warning: cannot find taxon " + taxon + " in reference taxonomy tree at level " + toString(tree[currentNode].level) + " for " + seqName + ". This may cause totals of daughter levels not to add up in summary file." << '\n';
				break;
			}
		}
	...
	while (seqTaxonomy != "") {
		...
		if (childPointer != tree[currentNode].children.end()) {	//if the node already exists, update count and move on
			...
			if (ignore) {
				...
				LOG(INFO) << "Warning: cannot find taxon " + taxon + " in reference taxonomy tree at level " + toString(tree[currentNode].level) + ". This may cause totals of daughter levels not to add up in summary file." << '\n';
				break;
			}
		}
