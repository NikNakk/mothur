...
PhyloTree::PhyloTree(ifstream& in, string filename) {
	...
	in >> numNodes; File::gobble(in);

	tree.resize(numNodes);

	for (int i = 0; i < tree.size(); i++) {
		in >> tree[i].name >> tree[i].level >> tree[i].parent; File::gobble(in);
	}

	//read genus nodes
	int numGenus = 0;
	in >> numGenus; File::gobble(in);

	int gnode, gsize;
	totals.clear();
	for (int i = 0; i < numGenus; i++) {
		in >> gnode >> gsize; File::gobble(in);
...
string PhyloTree::getNextTaxon(string& heirarchy, string seqname) {
	...
	if (heirarchy != "") {
		...
		if (pos == -1) { //you can't find another ;
			...
			heirarchy = "";
			LOG(INFO) << seqname + " is missing a ;, please check for other errors."; m->mothurOutEndLine();
		}
		else {
...
vector<string> PhyloTree::getSeqs(string seqTaxonomy) {
	...
	while (seqTaxonomy != "") {
		...
		if (app.isDebug) { LOG(INFO) << taxon + '\n'; }

		if (taxon == "") { LOG(INFO) << taxCopy + " has an error in the taxonomy.  This may be due to a ;;"; m->mothurOutEndLine(); break; }
		...
		if (childPointer != tree[currentNode].children.end()) {	//if the node already exists, move on
			...
			LOG(LOGERROR) << "" + taxCopy + " is not in taxonomy tree, please correct." << '\n'; ctrlc_pressed = true; return names;
		}
...
int PhyloTree::addSeqToTree(string seqName, string seqTaxonomy) {
	...
	while (seqTaxonomy != "") {
		...
		if (app.isDebug) { LOG(INFO) << seqName + '\t' + taxon + '\n'; }

		if (taxon == "") { LOG(INFO) << seqName + " has an error in the taxonomy.  This may be due to a ;;"; m->mothurOutEndLine(); if (currentNode != 0) { uniqueTaxonomies.insert(currentNode); } break; }
...
vector<int> PhyloTree::getGenusNodes() {
	try {
		...
		for (map<string, int>::iterator itName = name2Taxonomy.begin(); itName != name2Taxonomy.end(); itName++) {
			...
			else { LOG(LOGERROR) << "trouble making name2GenusNodeIndex, aborting.\n"; ctrlc_pressed = true; }
		}

		return genusIndex;
	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in PhyloTree, getGenusNodes";
		exit(1);
	}
}
/**************************************************************************************************/
...
void PhyloTree::assignHeirarchyIDs(int index) {
	...
	for (it = tree[index].children.begin();it != tree[index].children.end();it++) {
		...
		if (app.isDebug) { LOG(INFO) << toString(index) + '\t' + tree[it->second].name + '\n'; }
...
void PhyloTree::setUp(string tfile) {
	try {
		...
		LOG(FATAL) << e.what() << " in PhyloTree, setUp";
		exit(1);
	}
}
/**************************************************************************************************/
void PhyloTree::binUnclassified(string file) {
	...
	if (app.isDebug) { LOG(INFO) << "maxLevel = " + toString(maxLevel) + '\n'; }
	...
	for (itLeaf = leafNodes.begin(); itLeaf != leafNodes.end(); itLeaf++) {
		...
		if (ctrlc_pressed) { out.close(); break; }

		int level = copy[itLeaf->second].level;
		int currentNode = itLeaf->second;

		if (app.isDebug) { LOG(INFO) << copy[currentNode].name + '\n'; }

		//this sequence is unclassified at some levels
		while (level < maxLevel) {

			level++;
			if (app.isDebug) { LOG(INFO) << "level = " + toString(level) + '\n'; }
...
void PhyloTree::printTreeNodes(string treefilename) {
	ofstream outTree;
	File::openOutputFile(treefilename, outTree);
...
TaxNode PhyloTree::get(int i) {
	...
	else { cout << i << '\t' << tree.size() << endl; LOG(INFO) << "Mismatch with taxonomy and template files. Cannot continue." << '\n'; exit(1); }
}
/**************************************************************************************************/
TaxNode PhyloTree::get(string seqName) {
	...
	else { LOG(INFO) << "Cannot find " + seqName + ". Mismatch with taxonomy and template files. Cannot continue." << '\n'; exit(1); }
}
/**************************************************************************************************/
string PhyloTree::getName(int i) {
	if (i < tree.size()) { return tree[i].name; }
	else { LOG(INFO) << "Mismatch with taxonomy and template files. Cannot continue." << '\n'; exit(1); }
}
/**************************************************************************************************/
int PhyloTree::getGenusIndex(string seqName) {
	...
	else { LOG(INFO) << "Cannot find " + seqName + ". Could be a mismatch with taxonomy and template files. Cannot continue." << '\n'; exit(1); }
}
/**************************************************************************************************/
bool PhyloTree::ErrorCheck(vector<string> templateFileNames) {
	...
	if (app.isDebug) { LOG(DEBUG) << "in error check. Numseqs in template = " + toString(templateFileNames.size()) + ". Numseqs in taxonomy = " + toString(taxonomyFileNames.size()) + ".\n"; }
	...
	for (int i = 0; i < templateFileNames.size(); i++) {
		...
		if (itFind != taxonomyFileNames.end()) { //found it so erase it
			...
			LOG(INFO) << "'" + templateFileNames[i] + "' is in your template file and is not in your taxonomy file. Please correct." << '\n';
			okay = false;
		}
	...
	if (taxonomyFileNames.size() > 0) { //there are names in tax file that are not in template
		...
		for (itFind = taxonomyFileNames.begin(); itFind != taxonomyFileNames.end(); itFind++) {
			LOG(INFO) << itFind->first + " is in your taxonomy file and is not in your template file. Please correct." << '\n';
		}
	}
