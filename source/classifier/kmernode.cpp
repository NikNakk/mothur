...
int KmerNode::getNumUniqueKmers() {
	if (numUniqueKmers == 0) {
		...
		for (int i = 0;i < numPossibleKmers;i++) {
			if (ctrlc_pressed) { return numUniqueKmers; }
			if (kmerVector[i] != 0) {
				numUniqueKmers++;
			}
...
void KmerNode::printTheta() {
	...
	for (int i = 0;i < numPossibleKmers;i++) {
		if (kmerVector[i] != 0) {
			LOG(INFO) << getKmerBases(i) + '\t' + toString(kmerVector[i]) + "\n";
		}
	}
	LOG(INFO) << "";
...
double KmerNode::getSimToConsensus(vector<int>& queryKmerProfile) {
	...
	for (int i = 0;i < numPossibleKmers;i++) {
		if (ctrlc_pressed) { return present; }
...
double KmerNode::getPxGivenkj_D_j(vector<int>& queryKmerProfile) {
	try {
		...
		for (int i = 0;i < numPossibleKmers;i++) {
			if (ctrlc_pressed) { return sumLogProb; }
		...
		LOG(FATAL) << e.what() << " in KmerNode, getPxGivenkj_D_j";
		exit(1);
	}
