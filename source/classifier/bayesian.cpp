...
Bayesian::Bayesian(string tfile, string tempFile, string method, int ksize, int cutoff, int i, int tid, bool f, bool sh) :
	...
	string tempfileroot = File::getRootName(File::getSimpleName(baseName));
	...
	if (rdb.save && probFileTest && probFileTest2 && phyloTreeTest && probFileTest3 && FilesGood && (tempFile != "saved")) {
		ifstream saveIn;
		File::openInputFile(tempFile, saveIn);
	...
	if (probFileTest && probFileTest2 && phyloTreeTest && probFileTest3 && FilesGood) {
		if (tempFile == "saved") { LOG(INFO) << '\n' << "Using sequences from " + rdb.getSavedReference() + " that are saved in memory." << '\n'; }

		LOG(INFO) << "Reading template taxonomy...     "; cout.flush();
		...
		if (tfile == "saved") {
			LOG(INFO) << '\n' << "Using probabilties from " + rdb.getSavedTaxonomy() + " that are saved in memory...    ";	cout.flush();;
			wordGenusProb = rdb.wordGenusProb;
			WordPairDiffArr = rdb.WordPairDiffArr;
		}
		else {
			LOG(INFO) << "Reading template probabilities...     "; cout.flush();
		...
		if (ctrlc_pressed) { File::remove(phyloTreeName);  File::remove(probFileName); File::remove(probFileName2); }
		else {
			genusNodes = phyloTree->getGenusNodes();
			genusTotals = phyloTree->getGenusTotals();

			LOG(INFO) << "Calculating template taxonomy tree...     "; cout.flush();

			phyloTree->printTreeNodes(phyloTreeName);

			LOG(INFO) << "DONE." << '\n';

			LOG(INFO) << "Calculating template probabilities...     "; cout.flush();
			...
			if (shortcuts) {
				File::openOutputFile(probFileName, out);
				...
				File::openOutputFile(probFileName2, out2);
	...
	if (app.isDebug) { LOG(DEBUG) << "about to generateWordPairDiffArr\n"; }
	generateWordPairDiffArr();
	if (app.isDebug) { LOG(DEBUG) << "done generateWordPairDiffArr\n"; }

	//save probabilities
	if (rdb.save) { rdb.wordGenusProb = wordGenusProb; rdb.WordPairDiffArr = WordPairDiffArr; }

	LOG(INFO) << "DONE." << '\n';
	LOG(INFO) << "It took " + toString(time(NULL) - start) + " seconds get probabilities. " << '\n';
}
/**************************************************************************************************/
Bayesian::~Bayesian() {
...
string Bayesian::getTaxonomy(Sequence* seq) {
	...
	if (queryKmers.size() == 0) { LOG(INFO) << seq->getName() + " is bad. It has no kmers of length " + toString(kmerSize) + "." << '\n'; simpleTax = "unknown;";  return "unknown;"; }


	int index = getMostProbableTaxonomy(queryKmers);

	if (ctrlc_pressed) { return tax; }

	//bootstrap - to set confidenceScore
	int numToSelect = queryKmers.size() / 8;

	if (app.isDebug) { LOG(INFO) << seq->getName() + "\t"; }
...
string Bayesian::bootstrapResults(vector<int> kmers, int tax, int numToSelect) {
	...
	while (seqTax.level != 0) { //while you are not at the root
		...
		if (app.isDebug) { LOG(DEBUG) << seqTax.name + "(" + toString(((confidence / (float)iters) * 100)) + ");"; }
...
bool Bayesian::isReversed(vector<int>& queryKmers) {
	try {
		...
		LOG(FATAL) << e.what() << " in Bayesian, isReversed";
		exit(1);
	}
}
//********************************************************************************************************************
int Bayesian::generateWordPairDiffArr() {
	try {
		...
		LOG(FATAL) << e.what() << " in Bayesian, generateWordPairDiffArr";
		exit(1);
	}
}
/*************************************************************************************************
map<string, int> Bayesian::parseTaxMap(string newTax) {
	try{
		...
		LOG(FATAL) << e.what() << " in Bayesian, parseTax";
		exit(1);
	}
}
**************************************************************************************************/
void Bayesian::readProbFile(ifstream& in, ifstream& inNum, string inName, string inNumName) {
	try {
		...
		//read version
		string line2 = File::getline(inNum); File::gobble(inNum);
		float probTemp;
		...
		LOG(FATAL) << e.what() << " in Bayesian, readProbFile";
		exit(1);
	}
}
/**************************************************************************************************/
bool Bayesian::checkReleaseDate(ifstream& file1, ifstream& file2, ifstream& file3, ifstream& file4) {
	...
	vector<string> lines;
	lines.push_back(File::getline(file1));
	lines.push_back(File::getline(file2));
	lines.push_back(File::getline(file3));
	lines.push_back(File::getline(file4));
