...
void Knn::setDistName(string s) {
	...
	File::openOutputFile(outDistName, outDistance);
...
string Knn::getTaxonomy(Sequence* seq) {
	...
	if (search == "distance") { ofstream outDistance; File::openOutputFileAppend(outDistName, outDistance); outDistance << seq->getName() << '\t' << database->getName(closest[0]) << '\t' << database->getSearchScore() << endl; outDistance.close(); }
	...
	for (int i = 0; i < closest.size(); i++) {
		...
		if (it == taxonomy.end()) { //error not in file
			LOG(INFO) << "Error: sequence " + names[closest[i]] + " is not in the taxonomy file.  It will be eliminated as a match to sequence " + seq->getName() + "." << '\n';
		}
		else { closestNames.push_back(it->first); }
	}

	if (closestNames.size() == 0) {
		LOG(INFO) << "Error: All the matches for sequence " + seq->getName() + " have been eliminated. " << '\n';
		tax = "unknown;";
	}
	else {
		tax = findCommonTaxonomy(closestNames);
		if (tax == "") { LOG(INFO) << "There are no common levels for sequence " + seq->getName() + ". " << '\n'; tax = "unknown;"; }
	}
...
string Knn::findCommonTaxonomy(vector<string> closest) {
	try {
		...
		LOG(FATAL) << e.what() << " in Knn, findCommonTaxonomy";
		exit(1);
	}
}
/**************************************************************************************************/
