...
TaxEqualizer::TaxEqualizer(string tfile, int c, string o) : cutoff(c), outputDir(o) {
	...
	ifstream inTax;
	File::openInputFile(tfile, inTax);
	...
	if (!ctrlc_pressed) {
		...
		if ((cutoff != -1) && (cutoff < highestLevel)) {
			...
			LOG(INFO) << "The highest level taxonomy you have is " + toString(highestLevel) + " and your cutoff is " + toString(cutoff) + ". I will set the cutoff to " + toString(highestLevel);
			LOG(INFO) << "";
		}

		inTax.close();
		ifstream in;
		File::openInputFile(tfile, in);

		ofstream out;
		equalizedFile = outputDir + File::getRootName(File::getSimpleName(tfile)) + "equalized.taxonomy";
		File::openOutputFile(equalizedFile, out);
		...
		if (ctrlc_pressed) { File::remove(equalizedFile); }
	}
	else { inTax.close(); }
