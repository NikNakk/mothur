...
AlignTree::AlignTree(string referenceFileName, string taxonomyFileName, int cutoff) : Classify(), confidenceThreshold(cutoff) {
	...
	while (!referenceFile.eof()) {
		...
		Sequence seq(referenceFile);  File::gobble(referenceFile);
		...
		if (seq.getName() != "") {
			...
			if (it != taxonomy.end()) {
				...
				if (lengths.size() > 1) { error = true; LOG(LOGERROR) << "reference sequences must be aligned to use the align method, quitting.\n"; break; }
				addTaxonomyToTree(seq.getName(), refTaxonomy, aligned);
			}
			else {
				LOG(INFO) << seq.getName() + " is in your reference file, but not in your taxonomy file, please correct.\n"; error = true;
			}
		}
	}
	referenceFile.close();
...
int AlignTree::addTaxonomyToTree(string seqName, string& taxonomy, string& sequence) {
	...
	for (int i = 0;i < taxonomy.length();i++) {			//	step through taxonomy string...
		...
		if (taxonomy[i] == ';') {						//	looking for semicolons...
			...
			if (taxonName == "") { LOG(INFO) << seqName + " has an error in the taxonomy.  This may be due to a ;;" << '\n'; ctrlc_pressed = true; }
...
string AlignTree::getTaxonomy(Sequence* seq) {
	...
	if (querySequence.length() != length) {
		LOG(LOGERROR) << "" + seq->getName() + " has length " + toString(querySequence.length()) + ", reference sequences length is " + toString(length) + ". Are your sequences aligned? Sequences must be aligned to use the align search method.\n"; ctrlc_pressed = true; return "";
	}
	...
	for (int i = 0;i < numLevels;i++) {
		if (ctrlc_pressed) { return taxonProbabilityString; }
