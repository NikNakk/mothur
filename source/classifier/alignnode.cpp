...
void AlignNode::printTheta() {
	LOG(INFO) << "A:\t"; for (int i = 0;i < alignLength;i++) { LOG(INFO) << toString(theta[i].A) + '\t'; } LOG(INFO) << "";;
	LOG(INFO) << "T:\t"; for (int i = 0;i < alignLength;i++) { LOG(INFO) << toString(theta[i].T) + '\t'; } LOG(INFO) << "";;
	LOG(INFO) << "G:\t"; for (int i = 0;i < alignLength;i++) { LOG(INFO) << toString(theta[i].G) + '\t'; } LOG(INFO) << "";;
	LOG(INFO) << "C:\t"; for (int i = 0;i < alignLength;i++) { LOG(INFO) << toString(theta[i].C) + '\t'; } LOG(INFO) << "";;
	LOG(INFO) << "I:\t"; for (int i = 0;i < alignLength;i++) { LOG(INFO) << toString(theta[i].gap) + '\t'; } LOG(INFO) << "";;
}
...
double AlignNode::getSimToConsensus(string& query) {
	...
	for (int i = 0;i < alignLength;i++) {
		...
		if (ctrlc_pressed) { return similarity; }
...
double AlignNode::getPxGivenkj_D_j(string& query) {	//P(x | k_j, D, j)
	try {
		...
		for (int s = 0;s < alignLength;s++) {
			...
			if (ctrlc_pressed) { return PxGivenkj_D_j; }
		...
		LOG(FATAL) << e.what() << " in AlignNode, getPxGivenkj_D_j";
		exit(1);
	}
}
