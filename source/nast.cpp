...
void Nast::pairwiseAlignSeqs() {	//	Here we call one of the pairwise alignment methods to align our unaligned candidate
	...
	try {
		...
		LOG(FATAL) << e.what() << " in Nast, pairwiseAlignSeqs";
		exit(1);
	}
}
...
void Nast::removeExtraGaps(string& candAln, string tempAln, string newTemplateAlign) {
	...
	try {
		...
		//cout << "in else else newTemplateAlign = " << newTemplateAlign.length() << '\t' << i << '\t' << (i+leftRoom+rightRoom) << endl;
				...
				if (i < 0) { LOG(INFO) << "i is negative" << '\n'; }
			}
		}
	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in Nast, removeExtraGaps";
		exit(1);
	}
}
	...
	try {
		...
		while (pairwiseAlignIndex < pairwiseLength) {
			...
			if (isalpha(tempPair[pairwiseAlignIndex]) && isalpha(tempAln[fullAlignIndex])
				...
				LOG(INFO) << "We're into D " + toString(fullAlignIndex) + " " + toString(pairwiseAlignIndex) << '\n';
				pairwiseAlignIndex++;
			}
			else {
				...
				LOG(INFO) << "We're into F " + toString(fullAlignIndex) + " " + toString(pairwiseAlignIndex) << '\n';
				pairwiseAlignIndex++;
				fullAlignIndex++;
			}
		}
		...
		LOG(FATAL) << e.what() << " in Nast, regapSequences";
		exit(1);
	}
}
