...
string HeatMap::getPic(RAbundVector* rabund) {
	...
	string filenamesvg = outputDir + File::getRootName(File::getSimpleName(inputfile)) + rabund->getLabel() + ".heatmap.bin.svg";
	...
	for (int i = 0; i < scaleRelAbund.size(); i++) {
		if (ctrlc_pressed) { outsvg.close(); return "control"; }
...
string HeatMap::getPic(vector<SharedRAbundVector*> lookup) {
	...
	//sort lookup so shared bins are on top
	vector<string> sortedLabels = settings.currentSharedBinLabels;
	...
	string filenamesvg = outputDir + File::getRootName(File::getSimpleName(inputfile)) + lookup[0]->getLabel() + ".heatmap.bin.svg";
	File::openOutputFile(filenamesvg, outsvg);
	int binHeight = 20;
	int labelBump = 100;
	int binWidth = 300;
...
vector<string> HeatMap::sortSharedVectors(vector<SharedRAbundVector*>& lookup) {
	...
	else { LOG(INFO) << "Error: invalid sort option." << '\n';  return sortedLabels; }
	...
	for (int i = 0; i < looktemp[0]->size(); i++) {														//place
		...
		sortedLabels[place[i]] = settings.currentSharedBinLabels[i];
	}
...
void HeatMap::printLegend(int y, float maxbin) {
	...
	LOG(FATAL) << e.what() << " in HeatMap, printLegend";
	exit(1);
}
}
//**********************************************************************************************************************
