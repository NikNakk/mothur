		...
		for (int i = 1;i < nseqs;i++) {
			...
			if (nameMap == NULL) {
				...
				for (int j = 0;j < i;j++) {
					...
					if (ctrlc_pressed) { delete reading; fileHandle.close(); return 0; }
...
void ClusterClassic::setMapWanted(bool f) {
	try {
		...
		LOG(FATAL) << e.what() << " in ClusterClassic, setMapWanted";
		exit(1);
	}
}
/***********************************************************************/
void ClusterClassic::updateMap() {
	try {
		...
		LOG(FATAL) << e.what() << " in ClusterClassic, updateMap";
		exit(1);
	}
}
/***********************************************************************/
void ClusterClassic::print() {
	try {
		...
		for (int i = 0; i < dMatrix.size(); i++) {
			LOG(INFO) << "row = " + toString(i) + "\t";
			for (int j = 0; j < dMatrix[i].size(); j++) {
				LOG(INFO) << toString(dMatrix[i][j]) + "\t";
			}
			LOG(INFO) << "";
		}
	}
