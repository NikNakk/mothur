...
Tree* SubSample::getSample(Tree* T, CountTable* ct, CountTable* newCt, int size) {
	...
	for (int i = 0; i < Groups.size(); i++) {
		if (m->inUsersGroups(Groups[i], settings.getGroups())) {
			...
			else { LOG(LOGERROR) << "You have selected a size that is larger than " + Groups[i] + " number of sequences.\n"; ctrlc_pressed = true; }
		}
...
map<string, string> SubSample::deconvolute(map<string, string> whole, vector<string>& wanted) {
	...
	for (int i = 0; i < wanted.size(); i++) {
		...
		else { LOG(LOGERROR) << "" + dupName + " is not in your name file, please correct.\n"; ctrlc_pressed = true; }
	}
...
vector<string> SubSample::getSample(vector<SharedRAbundVector*>& thislookup, int size) {
	...
	for (int i = 0; i < thislookup.size(); i++) {
		...
		if (thisSize != size) {
			...
			for (int j = 0; j < size; j++) {
				...
				if (ctrlc_pressed) { return settings.currentSharedBinLabels; }
	...
	//save mothurOut's binLabels to restore for next label
	vector<string> subsampleBinLabels = settings.currentSharedBinLabels;
	...
	*order = sabund->getOrderVector(NULL);
	...
	if (thisSize > size) {
		...
		for (int j = 0; j < size; j++) {
			...
			if (ctrlc_pressed) { delete order; delete rabund; return 0; }
	...
	else if (thisSize < size) { LOG(LOGERROR) << "The size you requested is larger than the number of sequences in the sabund vector. You requested " + toString(size) + " and you only have " + toString(thisSize) + " seqs in your sabund vector.\n"; ctrlc_pressed = true; }
...
CountTable SubSample::getSample(CountTable& ct, int size, vector<string> Groups) {
	if (!ct.hasGroupInfo()) { LOG(LOGERROR) << "Cannot subsample by group because your count table doesn't have group information.\n"; ctrlc_pressed = true; }
	...
	for (int i = 0; i < Groups.size(); i++) {
		...
		for (int j = 0; j < names.size(); j++) {
			...
			if (ctrlc_pressed) { return sampledCt; }
		...
		if (allNames.size() < size) { LOG(LOGERROR) << "You have selected a size that is larger than " + Groups[i] + " number of sequences.\n"; ctrlc_pressed = true; }
		else {
			for (int j = 0; j < size; j++) {
...
CountTable SubSample::getSample(CountTable& ct, int size, vector<string> Groups, bool pickedGroups) {
	CountTable sampledCt;
	if (!ct.hasGroupInfo() && pickedGroups) { LOG(LOGERROR) << "Cannot subsample with groups because your count table doesn't have group information.\n"; ctrlc_pressed = true; return sampledCt; }
	...
	if (ct.hasGroupInfo()) {
		...
		if (allNames.size() < size) {
			if (pickedGroups) { LOG(LOGERROR) << "You have selected a size that is larger than the number of sequences.\n"; }
			else { LOG(LOGERROR) << "You have selected a size that is larger than the number of sequences in the groups you chose.\n"; }
			ctrlc_pressed = true; return sampledCt;
		}
		else {
			for (int j = 0; j < size; j++) {
		...
		if (allNames.size() < size) { LOG(LOGERROR) << "You have selected a size that is larger than the number of sequences.\n"; ctrlc_pressed = true; return sampledCt; }
		else {
			random_shuffle(allNames.begin(), allNames.end());
