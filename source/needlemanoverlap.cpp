...
NeedlemanOverlap::NeedlemanOverlap(float gO, float f, float mm, int r) ://	note that we don't have a gap extend
	...
	try {																	//	every gapped position
		...
		LOG(FATAL) << e.what() << " in NeedlemanOverlap, NeedlemanOverlap";
		exit(1);
	}
}
/**************************************************************************************************/
...
void NeedlemanOverlap::align(string A, string B) {
	...
	if (lA > nRows) { LOG(INFO) << "One of your candidate sequences is longer than you longest template sequence. Your longest template sequence is " + toString(nRows) + ". Your candidate is " + toString(lA) + "." << '\n'; }
