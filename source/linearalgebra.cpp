...
double LinearAlgebra::betai(const double a, const double b, const double x) {
	...
	if (x < 0.0 || x > 1.0) { LOG(LOGERROR) << "bad x in betai.\n"; ctrlc_pressed = true; return 0.0; }
...
double LinearAlgebra::gammp(const double a, const double x) {
	...
	if (x < 0.0 || a <= 0.0) { LOG(LOGERROR) << "Invalid arguments in routine GAMMP\n"; ctrlc_pressed = true; return 0.0; }
	if (x < (a + 1.0)) {
		gser(gamser, a, x, gln);
		return gamser;
	}
...
/*double LinearAlgebra::gammq(const double a, const double x) {
		...
		if (x < 0.0 || a <= 0.0) { LOG(LOGERROR) << "Invalid arguments in routine GAMMQ\n"; ctrlc_pressed = true; return 0.0; }
		if (x < (a+1.0)) {
			gser(gamser,a,x,gln);
			return 1.0-gamser;
		} else {
...
double LinearAlgebra::gcf(double& gammcf, const double a, const double x, double& gln) {
	...
	if (i > ITMAX) { LOG(LOGERROR) << "" + toString(a) + " too large, ITMAX=100 too small in gcf\n"; ctrlc_pressed = true; }
...
double LinearAlgebra::gser(double& gamser, const double a, const double x, double& gln) {
	...
	gln = gammln(a);
	if (x <= 0.0) {
		if (x < 0.0) { LOG(LOGERROR) << "x less than 0 in routine GSER\n"; ctrlc_pressed = true; }
		...
		LOG(LOGERROR) << "a too large, ITMAX too small in routine GSER\n";
		return 0.0;
	}
	return 0;
}
...
double LinearAlgebra::betacf(const double a, const double b, const double x) {
	...
	if (m1 > MAXIT) { LOG(LOGERROR) << "a or b too big or MAXIT too small in betacf." << '\n'; ctrlc_pressed = true; }
	return h;
...
vector<vector<double> > LinearAlgebra::matrix_mult(vector<vector<double> > first, vector<vector<double> > second) {
	...
	for (int i = 0;i < first_rows;i++) {
		for (int j = 0;j < second_cols;j++) {
			...
			if (ctrlc_pressed) { return product; }
...
double LinearAlgebra::calcKendall(vector<double>& x, vector<double>& y, double& sig) {
	if (x.size() != y.size()) { LOG(LOGERROR) << "vector size mismatch." << '\n'; return 0.0; }
...
double LinearAlgebra::calcWilcoxon(vector<double>& x, vector<double>& y, double& sig) {
	try {
		...
		LOG(FATAL) << e.what() << " in LinearAlgebra, calcWilcoxon";
		exit(1);
	}
}
...
vector<vector<double> > LinearAlgebra::getObservedEuclideanDistance(vector<vector<double> >& relAbundData) {
	...
	for (int i = 0;i < numSamples;i++) {
		for (int j = 0;j < numSamples;j++) {
			...
			if (ctrlc_pressed) { return dMatrix; }
...
vector<double> LinearAlgebra::solveEquations(vector<vector<double> > A, vector<double> b) {
	...
	ludcmp(A, index, d);  if (ctrlc_pressed) { return b; }
	lubksb(A, index, b);
...
void LinearAlgebra::ludcmp(vector<vector<double> >& A, vector<int>& index, double& d) {
	...
	for (int i = 0;i < n;i++) {
		...
		if (big == 0.0) { LOG(INFO) << "Singular matrix in routine ludcmp\n"; }
		vv[i] = 1.0 / big;
	}
...
void LinearAlgebra::lubksb(vector<vector<double> >& A, vector<int>& index, vector<double>& b) {
	//if(app.isDebug){   LOG(INFO) << "lubksb\n";    }
	double total;
	int n = (int)A.size();
	int ii = 0;

	for (int i = 0;i < n;i++) {
		//if(app.isDebug){   LOG(INFO) << "i loop " + toString(i) + "\n";    }
		if (ctrlc_pressed) { break; }
		int ip = index[i];
		total = b[ip];
		b[ip] = b[i];
		...
		if (ii != 0) {
			for (int j = ii - 1;j < i;j++) {
				//if(app.isDebug){   LOG(INFO) << "j loop " + toString(j) + "\n";    }
				total -= A[i][j] * b[j];
			}
		}
...
void LinearAlgebra::ludcmp(vector<vector<float> >& A, vector<int>& index, float& d) {
	...
	//if(app.isDebug){   LOG(INFO) << "end ludcmp\n";    }
}
...
vector< vector<double> > LinearAlgebra::lda(vector< vector<double> >& a, vector<string> groups, vector< vector<double> >& means, bool& ignore) {
	...
	if (rank == 0) {
		ignore = true; //LOG(LOGERROR) << "rank = 0: variables are numerically const\n"; ctrlc_pressed = true;
		return scaling;
	}
	...
	*/
	...
	if (rank == 0) {
		ignore = true;//LOG(LOGERROR) << "rank = 0: class means are numerically identical.\n"; ctrlc_pressed = true;
		return scaling;
	}
...
int LinearAlgebra::svd(vector< vector<double> >& a, vector<double>& w, vector< vector<double> >& v) {
	...
	if (numRows < numCols) { LOG(LOGERROR) << "numRows < numCols\n"; ctrlc_pressed = true; return 0; }
	...
	{                             /* loop over singular values */
		...
		{                         /* loop over allowed iterations */
			...
			if (its >= 30) {
				LOG(INFO) << "No convergence after 30,000! iterations \n"; ctrlc_pressed = true;
				return(0);
			}
