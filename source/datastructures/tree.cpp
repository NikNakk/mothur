...
Tree::Tree(string g) { //do not use tree generated by this its just to extract the treenames, its a chicken before the egg thing that needs to be revisited.
	try {
		...
		LOG(FATAL) << e.what() << " in Tree, Tree - just parse";
		exit(1);
	}
}
/*****************************************************************/
...
void Tree::addNamesToCounts(map<string, string> nameMap) {
		...
		for (int i = 0; i < numLeaves; i++) {
			...
			if (itNames == nameMap.end()) { LOG(INFO) << name + " is not in your name file, please correct." << '\n'; exit(1);  }
			else {
...
int Tree::assembleTree() {
	try {
		...
		LOG(FATAL) << e.what() << " in Tree, assembleTree";
		exit(1);
	}
}
/*****************************************************************/
...
int Tree::getSubTree(Tree* copy, vector<string> seqsToInclude, map<string, string> nameMap) {
		...
		if (numLeaves != seqsToInclude.size()) { LOG(LOGERROR) << "numLeaves does not equal numUniques, cannot create subtree.\n"; m->control_pressed = true; return 0; }
...
map<string, int> Tree::mergeGcounts(int position) {
	try {
		...
		LOG(FATAL) << e.what() << " in Tree, mergeGcounts";
		exit(1);
	}
}
/**************************************************************************************************/
...
void Tree::randomBlengths() {
	try {
		...
		LOG(FATAL) << e.what() << " in Tree, randomBlengths";
		exit(1);
	}
}
/*************************************************************************************************/
	...
	try {
		...
		if (tree[node].getLChild() != -1) {
			...
			if (itNames != names.end()) {
				...
				LOG(LOGERROR) << "" + tree[node].getName() + " is not in your namefile, please correct." << '\n';
			}
		...
		LOG(FATAL) << e.what() << " in Tree, printBranch";
		exit(1);
	}
}
/*****************************************************************/
...
int Tree::parseTreeFile() {
	...
	try {
		string filename = settings->getCurrent("tree");
		ifstream filehandle;
		...
		LOG(FATAL) << e.what() << " in Tree, parseTreeFile";
		exit(1);
	}
}
/*******************************************************/
...
int Tree::readTreeString(ifstream& filehandle) {
	...
	//cout << k << endl;
		...
		//cout << " in name while " << k << endl;
				...
				} //cout << m->Treenames.size() << '\t' << name << endl;
		...
		LOG(FATAL) << e.what() << " in Tree, readTreeString";
		exit(1);
	}
}
