...
KmerDB::KmerDB(string fastaFileName, int kSize) : Database(), kmerSize(kSize) {
	try {
		...
		LOG(FATAL) << e.what() << " in KmerDB, KmerDB";
		exit(1);
	}
...
vector<int> KmerDB::findClosestSequences(Sequence* candidateSeq, int num) {
	if (num > numSeqs) { LOG(WARNING) << "you requested " + toString(num) + " closest sequences, but the template only contains " + toString(numSeqs) + ", adjusting." << '\n'; num = numSeqs; }
...
void KmerDB::generateDB() {
	...
	File::openOutputFile(kmerDBName, kmerFile);					//	to a file
...
void KmerDB::readKmerDB(ifstream& kmerDBFile) {
	...
	//read version
	string line = File::getline(kmerDBFile); File::gobble(kmerDBFile);
