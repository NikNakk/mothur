...
FlowData::FlowData(int numFlows, float signal, float noise, int maxHomoP, string baseFlow) :
	...
	try {
		...
		LOG(FATAL) << e.what() << " in FlowData, FlowData";
		exit(1);
	}
...
bool FlowData::getNext(ifstream& flowFile) {
	...
	if (app.isDebug) { LOG(DEBUG) << "flow = " + seqName + " "; }
	flowFile >> endFlow;
	if (app.isDebug) { LOG(INFO) << toString(endFlow) + " "; }
	if (!ctrlc_pressed) {
		if (app.isDebug) { LOG(INFO) << " "; }
		for (int i = 0;i < numFlows;i++) {
			flowFile >> flowData[i];
			if (app.isDebug) { LOG(INFO) << toString(flowData[i]) + " "; }
		}
		if (app.isDebug) { LOG(INFO) << "\n"; }
		updateEndFlow();
		translateFlow();
...
string FlowData::getSequenceName(ifstream& flowFile) {
	...
	else { LOG(INFO) << "Error in reading your flowfile, at position " + toString(flowFile.tellg()) + ". Blank name." << '\n'; ctrlc_pressed = true; }
...
void FlowData::updateEndFlow() {
	try {
		...
		LOG(FATAL) << e.what() << " in FlowData, findDeadSpot";
		exit(1);
	}
}
...
void FlowData::translateFlow() {
	try {
		...
		LOG(FATAL) << e.what() << " in FlowData, translateFlow";
		exit(1);
	}
}
...
void FlowData::capFlows(int mF) {
	...
	try {
		...
		LOG(FATAL) << e.what() << " in FlowData, capFlows";
		exit(1);
	}
}
...
bool FlowData::hasGoodHomoP() {
	...
	try {
		...
		LOG(FATAL) << e.what() << " in FlowData, hasMinFlows";
		exit(1);
	}
}
...
Sequence FlowData::getSequence() {
	...
	try {
		...
		LOG(FATAL) << e.what() << " in FlowData, getSequence";
		exit(1);
	}
}
...
void FlowData::printFlows(ofstream& outFlowFile) {
	try {
		...
		LOG(FATAL) << e.what() << " in FlowData, printFlows";
		exit(1);
	}
}
...
string FlowData::getName() {
	...
	try {
		...
		LOG(FATAL) << e.what() << " in FlowData, getName";
		exit(1);
	}
}
