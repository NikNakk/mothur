...
gapOpen(gO), gapExtend(gE), match(mm), misMatch(mM) {
	...
	if (app.isDebug) { LOG(DEBUG) << "tag = " + tag + "\t pid = " + pid + "\n"; }
	...
	if (ableToOpen == 1) { LOG(LOGERROR) << "" + formatdbCommand + " file does not exist. mothur requires formatdb.exe." << '\n'; ctrlc_pressed = true; }
	...
	if (ableToOpen == 1) { LOG(LOGERROR) << "" + blastCommand + " file does not exist. mothur requires blastall.exe." << '\n'; ctrlc_pressed = true; }
	...
	ableToOpen = File::openInputFile(megablastCommand, in3, "no error"); in3.close();
	if (ableToOpen == 1) { LOG(LOGERROR) << "" + megablastCommand + " file does not exist. mothur requires megablast.exe." << '\n'; ctrlc_pressed = true; }
...
BlastDB::~BlastDB() {
	try {
		File::remove(queryFileName);				//	let's clean stuff up and remove the temp files
		File::remove(dbFileName);					//	let's clean stuff up and remove the temp files
		File::remove((dbFileName + ".nsq"));					//	let's clean stuff up and remove the temp files
		File::remove((dbFileName + ".nsi"));					//	let's clean stuff up and remove the temp files
		File::remove((dbFileName + ".nsd"));					//	let's clean stuff up and remove the temp files
		File::remove((dbFileName + ".nin"));					//	let's clean stuff up and remove the temp files
		File::remove((dbFileName + ".nhr"));					//	let's clean stuff up and remove the temp files
		File::remove(blastFileName.c_str());				//	let's clean stuff up and remove the temp files
	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in BlastDB, ~BlastDB";
		exit(1);
	}
}
/**************************************************************************************************/
...
vector<int> BlastDB::findClosestSequences(Sequence* seq, int n) {
	try {
		...
		File::openOutputFile((queryFileName + pid + toString(randNumber)), queryFile);
		...
		File::openInputFile(blastFileName + pid + toString(randNumber), m8FileHandle, "no error");

		string dummy;
		int templateAccession;
		File::gobble(m8FileHandle);
		...
		m8FileHandle.close();
		File::remove((queryFileName + pid + toString(randNumber)));
		File::remove((blastFileName + pid + toString(randNumber)));

		return topMatches;
	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in BlastDB, findClosestSequences";
		exit(1);
	}
...
vector<int> BlastDB::findClosestMegaBlast(Sequence* seq, int n, int minPerID) {
	try {
		...
		LOG(FATAL) << e.what() << " in BlastDB, findClosestMegaBlast";
		exit(1);
	}
}
/**************************************************************************************************/
void BlastDB::addSequence(Sequence seq) {
	...
	File::openOutputFileAppend(dbFileName, unalignedFastaFile);
...
void BlastDB::generateDB() {
	...
	//LOG(INFO) << "Generating the temporary BLAST database...\t";	cout.flush();
