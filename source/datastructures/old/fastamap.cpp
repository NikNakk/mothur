...
void FastaMap::readFastaFile(string inFileName) {
	ifstream in;
	File::openInputFile(inFileName, in);
	...
	while (!in.eof()) {
		...
		if (name != "") {
			...
			else { LOG(INFO) << "You already have a sequence named " + name + ", sequence names must be unique, please correct." << '\n'; }
...
void FastaMap::readFastaFile(string inFastaFile, string oldNameFileName) { //prints data
	...
	File::openInputFile(oldNameFileName, oldNameFile);
	...
	while (!oldNameFile.eof()) {
		...
		oldNameFile >> name; File::gobble(oldNameFile);
		oldNameFile >> list;
		oldNameMap[name] = list;
		File::gobble(oldNameFile);
	}
	oldNameFile.close();

	ifstream inFASTA;
	File::openInputFile(inFastaFile, inFASTA);
	string sequence;
	while (!inFASTA.eof()) {
		if (ctrlc_pressed) { break; }
...
void FastaMap::printNamesFile(string outFileName) { //prints data
	try {
		ofstream outFile;
		File::openOutputFile(outFileName, outFile);
		...
		LOG(FATAL) << e.what() << " in FastaMap, printNamesFile";
		exit(1);
	}
}
...
void FastaMap::printCondensedFasta(string outFileName) { //prints data
	try {
		ofstream out;
		File::openOutputFile(outFileName, out);
		...
		LOG(FATAL) << e.what() << " in FastaMap, printCondensedFasta";
		exit(1);
	}
}
