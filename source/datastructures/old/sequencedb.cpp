...
SequenceDB::SequenceDB() {  length = 0; samelength = true; }
/***********************************************************************/
...
SequenceDB::SequenceDB(ifstream& filehandle) {
	try {
		...
		LOG(FATAL) << e.what() << " in SequenceDB, SequenceDB";
		exit(1);
	}
}
/*******************************************************************************/
string SequenceDB::readName(ifstream& in) {
	try {
		...
		LOG(FATAL) << e.what() << " in SequenceDB, readName";
		exit(1);
	}
}
...
string SequenceDB::readSequence(ifstream& in) {
	try {
		...
		LOG(FATAL) << e.what() << " in SequenceDB, readSequence";
		exit(1);
	}
}
