...
{		//	This assumes that the template database is in fasta format, may 
	try {											//	need to alter this in the future?
		...
		if (fastaFileName == "saved") {
			...
			if (!silent) { LOG(INFO) << "It took " + toString(time(NULL) - start) + " to load " + toString(rdb.referenceSeqs.size()) + " sequences."; }
			...
			ifstream fastaFile;
			File::openInputFile(fastaFileName, fastaFile);

			while (!fastaFile.eof()) {
				Sequence temp(fastaFile);  File::gobble(fastaFile);

				if (ctrlc_pressed) { templateSequences.clear(); break; }
				...
				if (temp.getName() != "") {
					...
					if (tempLength != 0) {
						if (tempLength != temp.getAligned().length()) { LOG(LOGERROR) << "template is not aligned, aborting.\n"; ctrlc_pressed = true; }
					}
			...
			LOG(INFO) << "DONE.";
			LOG(INFO) << "";	cout.flush();
			LOG(INFO) << "It took " + toString(time(NULL) - start) + " to read  " + toString(templateSequences.size()) + " sequences." << '\n';
		...
		if (!(ctrlc_pressed)) {
			if (needToGenerate) {
				...
				if (ctrlc_pressed) { templateSequences.clear(); }
		...
		LOG(FATAL) << e.what() << " in AlignmentDB, AlignmentDB";
		exit(1);
	}
}
/**************************************************************************************************/
...
Sequence AlignmentDB::findClosestSequence(Sequence* seq) {
	try {
		...
		LOG(FATAL) << e.what() << " in AlignmentDB, findClosestSequence";
		exit(1);
	}
}
/**************************************************************************************************/
