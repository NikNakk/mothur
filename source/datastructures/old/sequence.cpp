...
Sequence::Sequence(string newName, string sequence) {
	...
	setAligned(sequence); // Note also sets unaligned
}
/***********************************************************************/
...
Sequence::Sequence(istringstream& fastaString) {
	...
	if (!ctrlc_pressed) {
		...
		if ((numAmbig / (float)numBases) > 0.25) { LOG(WARNING) << "We found more than 25% of the bases in sequence " + name + " to be ambiguous. Mothur is not setup to process protein sequences." << '\n'; }
	}
}
//********************************************************************************************************************
//this function will jump over commented out sequences, but if the last sequence in a file is commented out it makes a blank seq
...
string Sequence::getSequenceName(ifstream& fastaFile) {
	...
	if (name.length() != 0) {
		...
		LOG(INFO) << "Error in reading your fastafile, at position " + toString(fastaFile.tellg()) + ". Blank name." << '\n'; ctrlc_pressed = true;
	}
	return name;
}
//********************************************************************************************************************
...
string Sequence::getSequenceName(boost::iostreams::filtering_istream& fastaFile) {
	...
	else { LOG(INFO) << "Error in reading your fastafile, at position " + toString(fastaFile.tellg()) + ". Blank name." << '\n'; ctrlc_pressed = true; }
...
string Sequence::getCommentString(ifstream& fastaFile) {
	...
	while (fastaFile) {
		letter = fastaFile.get();
		if ((letter == '\r') || (letter == '\n') || letter == -1) {
			File::gobble(fastaFile);  //in case its a \r\n situation
			break;
		}
		else {
			temp += letter;
...
void Sequence::setUnaligned(string sequence) {
	if (sequence.find_first_of('.') != string::npos || sequence.find_first_of('-') != string::npos) {
		...
		for (int j = 0; j < sequence.length(); j++) {
			if (isalpha(sequence[j])) { os << sequence[j]; }
		}
		unaligned = os.str();
	}
	else {
...
int Sequence::filterToPos(int start) {
	...
	if (start > aligned.length()) { start = aligned.length(); LOG(LOGERROR) << "start to large.\n"; }
...
int Sequence::filterFromPos(int end) {
	...
	if (end > aligned.length()) { end = aligned.length(); LOG(LOGERROR) << "end to large.\n"; }
...
///**************************************************************************************************/
bool Sequence::checkName(string& name) {
	bool changedSeqNames = false;
	for (int i = 0; i < name.length(); i++) {
		if (name[i] == ':') { name[i] = '_'; changedSeqNames = true; }
	}
	return changedSeqNames;
