...
int Pam::buildPhase() {
	...
	if (app.isDebug) { LOG(DEBUG) << "building medoids\n"; }
	...
	for (int k = 0; k < numPartitions; k++) {
		...
		for (int i = 0; i < numSamples; i++) {  //does this need to be square?? can we do lt?
			...
			if (medoids.count(i) == 0) { //is this sample is NOT a medoid?
				...
				if (app.isDebug) { LOG(DEBUG) << "" + toString(i) + " totalGain = " + toString(totalGain) + "\n"; }
		...
		if (app.isDebug) { LOG(DEBUG) << "new medoid " + toString(medoid) + "\n"; }
	...
	if (app.isDebug) { LOG(DEBUG) << "done building medoids\n"; }
	return 0;
}
/**************************************************************************************************/
//goal to swap medoids with non-medoids to see if we can reduce the overall cost
int Pam::swapPhase() {
	if (app.isDebug) { LOG(DEBUG) << "swapping  medoids\n"; }
	...
	while (!done) {
		...
		if (dzsky < -16 * DBL_EPSILON * fabs(sky)) {
			...
			medoids.erase(nbest);
			if (app.isDebug) { LOG(DEBUG) << "swapping " + toString(hbest) + " " + toString(nbest) + "\n"; }
			sky += dzsky;
		}
	...
	if (app.isDebug) {
		for (int i = 0;i < numPartitions;i++) {
			LOG(DEBUG) << "partition 1: ";
			for (int j = 0; j < numSamples; j++) {
				LOG(INFO) << toString(zMatrix[i][j]) + " ";
			}
			LOG(INFO) << "\n";
		}
		LOG(DEBUG) << "medoids : ";
		for (set<int>::iterator it = medoids.begin(); it != medoids.end(); it++) {
			LOG(INFO) << toString(*it) + " ";
		}
		LOG(INFO) << "\n";

		LOG(DEBUG) << "laplace : " + toString(laplace);  LOG(INFO) << "\n";
	}

	if (app.isDebug) { LOG(DEBUG) << "done swapping  medoids\n"; }
	return 0;
}
...
double Pam::calcCHIndex(vector< vector<double> > dists) { //countMatrix = [numSamples][numOtus]
	try {
		...
		if (app.isDebug) { LOG(DEBUG) << "allMeandDist = " + toString(allMeanDist) + "\n"; }
		...
		LOG(FATAL) << e.what() << " in Pam, calcCHIndex";
		exit(1);
	}
}
