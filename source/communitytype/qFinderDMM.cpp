...
qFinderDMM::qFinderDMM(vector<vector<int> > cm, int p) : CommunityTypeFinder() {
	...
	// if (app.isDebug) { LOG(INFO) << "before kmeans\n"; }
	findkMeans();
	//if (app.isDebug) { LOG(INFO) << "done kMeans\n"; }

	optimizeLambda();


	//if (app.isDebug) { LOG(INFO) << "done optimizeLambda\n"; }
	...
	while (change > 1.0e-6 && iter < 100) {
		...
		// if (app.isDebug) { LOG(INFO) << "Calc_Z: \n"; }
		calculatePiK();

		optimizeLambda();

		// if (app.isDebug) { LOG(INFO) << "Iter: " + toString(iter) + "\n"; }
	...
	//if (app.isDebug) { LOG(INFO) << "done while loop\n"; }
	...
	for (currentPartition = 0;currentPartition < numPartitions;currentPartition++) {
		...
		if (app.isDebug) { LOG(INFO) << "current partition = " + toString(currentPartition) + "\n"; }

		if (currentPartition > 0) {
			logDeterminant += (2.0 * log(numSamples) - log(weights[currentPartition]));
		}
		//if (app.isDebug) { LOG(INFO) << "before hession\n"; }
		vector<vector<double> > hessian = getHessian();
		//if (app.isDebug) { LOG(INFO) << "after hession\n"; }
		vector<vector<double> > invHessian = l.getInverse(hessian);
		//if (app.isDebug) { LOG(INFO) << "after inverse\n"; }
...
void qFinderDMM::printFitData(ostream& out, double minLaplace) {
	...
	LOG(FILEONLY) << toString(numPartitions) + '\t' + toString(getNLL()) + '\t' + toString(getLogDet()) + '\t';
	LOG(FILEONLY) << toString(getBIC()) + '\t' + toString(getAIC()) + '\t' + toString(laplace);
...
int qFinderDMM::bfgs2_Solver(vector<double>& x) {
	try {
		...
		LOG(FATAL) << e.what() << " in qFinderDMM, bfgs2_Solver";
		exit(1);
	}
}
...
double qFinderDMM::negativeLogEvidenceLambdaPi(vector<double>& x) {
	try {
		...
		LOG(FATAL) << e.what() << " in qFinderDMM, negativeLogEvidenceLambdaPi";
		exit(1);
	}
}
...
void qFinderDMM::negativeLogDerivEvidenceLambdaPi(vector<double>& x, vector<double>& df) {
	try {
		...
		LOG(FATAL) << e.what() << " in qFinderDMM, negativeLogDerivEvidenceLambdaPi";
		exit(1);
	}
}
...
void qFinderDMM::optimizeLambda() {
	try {
		...
		LOG(FATAL) << e.what() << " in qFinderDMM, optimizeLambda";
		exit(1);
	}
}
/**************************************************************************************************/
