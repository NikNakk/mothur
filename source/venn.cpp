...
vector<string> Venn::getPic(SAbundVector* sabund, vector<Calculator*> vCalcs) {
	try {
		...
		for (int i = 0;i < vCalcs.size();i++) {
			string filenamesvg = outputDir + File::getRootName(File::getSimpleName(inputfile)) + "." + sabund->getLabel() + "." + vCalcs[i]->getName() + ".svg";
			outputNames.push_back(filenamesvg);
			File::openOutputFile(filenamesvg, outsvg);

			if (ctrlc_pressed) { outsvg.close(); return outputNames; }
	...
	vector<string> Venn::getPic(vector<SharedRAbundVector*> lookup, vector<Calculator*> vCalcs) {
		...
		if (lookup.size() == 1) {
			...
			for (int i = 0;i < vCalcs.size();i++) {
				string filenamesvg = outputDir + File::getRootName(File::getSimpleName(inputfile)) + lookup[0]->getLabel() + "." + vCalcs[i]->getName() + "." + lookup[0]->getGroup() + ".svg";
				outputNames.push_back(filenamesvg);
				File::openOutputFile(filenamesvg, outsvg);
			...
			//make a file for each calculator
			for (int i = 0;i < vCalcs.size();i++) {
				string filenamesvg = outputDir + File::getRootName(File::getSimpleName(inputfile)) + lookup[0]->getLabel() + "." + vCalcs[i]->getName() + "." + lookup[0]->getGroup() + "-" + lookup[1]->getGroup() + ".svg";
				...
				if (vCalcs[i]->getName() == "sharedsobs") {
					...
					if (sharedOtus && (labels.size() != 0)) {
						...
						string filenameShared = outputDir + File::getRootName(File::getSimpleName(inputfile)) + lookup[0]->getLabel() + "." + vCalcs[i]->getName() + "." + groupsTag + ".sharedotus";

						outputNames.push_back(filenameShared);
						ofstream outShared;
						File::openOutputFile(filenameShared, outShared);
			...
			for (int i = 0;i < vCalcs.size();i++) {
				...
				string filenamesvg = outputDir + File::getRootName(File::getSimpleName(inputfile)) + lookup[0]->getLabel() + "." + vCalcs[i]->getName() + "." + lookup[0]->getGroup() + "-" + lookup[1]->getGroup() + "-" + lookup[2]->getGroup() + ".svg";
			...
			for (int i = 0;i < vCalcs.size();i++) {
				...
				if ((vCalcs[i]->getName() != "sharedsobs") && (vCalcs[i]->getName() != "sharedchao")) { LOG(INFO) << vCalcs[i]->getName() + " is not a valid calculator with four groups.  It will be disregarded. " << '\n'; }
				else {
					string filenamesvg = outputDir + File::getRootName(File::getSimpleName(inputfile)) + lookup[0]->getLabel() + "." + vCalcs[i]->getName() + "." + lookup[0]->getGroup() + "-" + lookup[1]->getGroup() + "-" + lookup[2]->getGroup() + "-" + lookup[3]->getGroup() + ".svg";
					outputNames.push_back(filenamesvg);
					File::openOutputFile(filenamesvg, outsvg);
