...
int Collect::getCurve(float percentFreq = 0.01) {
	try {
		...
		for (int i = 0;i < numSeqs;i++) {
			...
			if (ctrlc_pressed) { delete lookup; delete rank; delete ccd;  return 1; }
		...
		LOG(FATAL) << e.what() << " in Collect, getCurve";
		exit(1);
	}
}
...
int Collect::getSharedCurve(float percentFreq = 0.01) {
	try {
		...
		for (int i = 0;i < numSeqs;i++) {
			...
			if (ctrlc_pressed) { for (int j = 0; j < lookup.size(); j++) { delete lookup[j]; } delete ccd;  return 1; }
			...
			if ((i == 0) || (i + 1) % increment == 0) {
				...
				for (int k = 0; k < (lookup.size() - 1); k++) { // pass cdd each set of groups to commpare
					for (int l = n; l < lookup.size(); l++) {
						...
						ccd->updateSharedData(subset, i + 1, settings.getNumGroups(), pair);
					}
					n++;
				}
				...
				pair = false;
				ccd->updateSharedData(lookup, i + 1, settings.getNumGroups(), pair);
		...
		if (numSeqs % increment != 0) {
			...
			for (int k = 0; k < (lookup.size() - 1); k++) { // pass cdd each set of groups to commpare
				for (int l = n; l < lookup.size(); l++) {
					...
					ccd->updateSharedData(subset, totalNumSeq, settings.getNumGroups(), pair);
				}
				n++;
			}
			//if this is a calculator that can do multiples then do them
			pair = false;
			ccd->updateSharedData(lookup, totalNumSeq, settings.getNumGroups(), pair);
		}
		...
		LOG(FATAL) << e.what() << " in Collect, getSharedCurve";
		exit(1);
	}
}
...
void Collect::getGroupComb() {
	...
	for (int i = 0; i < (settings.getNumGroups() - 1); i++) {
		for (int l = n; l < settings.getNumGroups(); l++) {
