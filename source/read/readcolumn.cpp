...
ReadColumnMatrix::ReadColumnMatrix(string df) : distFile(df) {
	...
	successOpen = File::openInputFile(distFile, fileHandle);
	sim = false;
...
int ReadColumnMatrix::read(NameAssignment* nameMap) {
	try {
		...
		while (fileHandle && lt == 1) {  //let's assume it's a triangular matrix...
			...
			fileHandle >> firstName; File::gobble(fileHandle);
			fileHandle >> secondName; File::gobble(fileHandle);
			fileHandle >> distance;	// get the row and column names and distance

			if (app.isDebug) { cout << firstName << '\t' << secondName << '\t' << distance << endl; }

			if (ctrlc_pressed) { fileHandle.close();  delete reading; return 0; }
		...
		if (lt == 0) {  // oops, it was square
			...
			File::openInputFile(distFile, fileHandle);  //let's start over
		...
		LOG(FATAL) << e.what() << " in ReadColumnMatrix, read";
		exit(1);
	}
}
/***********************************************************************/
