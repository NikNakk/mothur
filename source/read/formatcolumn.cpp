...
int FormatColumnMatrix::read(NameAssignment* nameMap) {
	try {
		...
		File::openOutputFile(tempOutFile, out);

		while (fileHandle && lt == 1) {  //let's assume it's a triangular matrix...

			if (ctrlc_pressed) { out.close();  File::remove(tempOutFile); fileHandle.close();  delete reading; return 0; }
		...
		string outfile = File::getRootName(squareFile) + "sorted.dist.temp";
		...
		if (ctrlc_pressed) { File::remove(tempOutFile); File::remove(outfile); delete reading; return 0; }

		//output to new file distance for each row and save positions in file where new row begins
		ifstream in;
		File::openInputFile(outfile, in);

		distFile = outfile + ".rowFormatted";
		File::openOutputFile(distFile, out);
		...
		while (!in.eof()) {
			...
			if (ctrlc_pressed) { in.close(); out.close(); File::remove(distFile); File::remove(tempOutFile); File::remove(outfile); delete reading; return 0; }
		...
		if (ctrlc_pressed) { File::remove(distFile); File::remove(tempOutFile); File::remove(outfile);  delete reading; return 0; }
		...
		if (ctrlc_pressed) { File::remove(distFile);  return 0; }

		return 1;

	}
	catch (exception& e) {
		LOG(FATAL) << e.what() << " in FormatColumnMatrix, read";
		exit(1);
	}
}
/***********************************************************************/
