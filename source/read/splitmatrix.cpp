...
int SplitMatrix::split() {
	...
	if (method == "distance") {
		...
		LOG(INFO) << "Unknown splitting method, aborting split." << '\n';
...
int SplitMatrix::splitClassify() {
	...
	if (cutoff > phylo->getMaxLevel()) { LOG(INFO) << "splitcutoff is greater than the longest taxonomy, using " + toString(phylo->getMaxLevel()) << '\n'; cutoff = phylo->getMaxLevel(); }
...
int SplitMatrix::createDistanceFilesFromTax(map<string, int>& seqGroup, int numGroups) {
	...
	for (int i = 0; i < numGroups; i++) { File::remove((fastafile + "." + toString(i) + ".temp")); }

	//parse fastafile
	while (!in.eof()) {
		Sequence query(in); File::gobble(in);
		if (query.getName() != "") {
			...
			if (it != seqGroup.end()) { //not singleton
				ofstream outFile;
				File::openOutputFileAppend((fastafile + "." + toString(it->second) + ".temp"), outFile);
	...
	for (it = copyGroups.begin(); it != copyGroups.end(); it++) {
		LOG(INFO) << "ERROR: " + it->first + " is missing from your fastafile. This could happen if your taxonomy file is not unique and your fastafile is, or it could indicate and error." << '\n';
		error = true;
	}
	copyGroups.clear();
	...
	for (int i = 0; i < numGroups; i++) {
		...
		LOG(INFO) << "/******************************************/" << '\n';
		LOG(INFO) << "Running command: dist.seqs(" + options + ")" << '\n';
		m->mothurCalling = true;
		...
		File::remove((fastafile + "." + toString(i) + ".temp"));

		//remove old names files just in case
		if (namefile != "") { File::remove((namefile + "." + toString(i) + ".temp")); }
		else { File::remove((countfile + "." + toString(i) + ".temp")); }
	}
	...
	for (int i = 0;i < numGroups;i++) {
		...
		string tempDistFile = "";
		if (classic) { tempDistFile = outputDir + File::getRootName(File::getSimpleName((fastafile + "." + toString(i) + ".temp"))) + "phylip.dist"; }
		else { tempDistFile = outputDir + File::getRootName(File::getSimpleName((fastafile + "." + toString(i) + ".temp"))) + "dist"; }
		tempDistFiles.push_back(tempDistFile);
	}

	splitNames(seqGroup, numGroups, tempDistFiles);

	if (ctrlc_pressed) { for (int i = 0; i < dists.size(); i++) { File::remove((dists[i].begin()->first)); File::remove((dists[i].begin()->second)); } dists.clear(); }
...
int SplitMatrix::splitDistanceFileByTax(map<string, int>& seqGroup, int numGroups) {
	...
	ofstream outFile;
	ifstream dFile;
	File::openInputFile(distFile, dFile);


	for (int i = 0; i < numGroups; i++) { //remove old temp files, just in case
		File::remove((distFile + "." + toString(i) + ".temp"));
	}
	...
	while (dFile) {
		...
		if (ctrlc_pressed) { dFile.close(); for (int i = 0; i < numGroups; i++) { File::remove((distFile + "." + toString(i) + ".temp")); } }

		dFile >> seqA >> seqB >> dist;  File::gobble(dFile);
		...
		if ((it != seqGroup.end()) && (it2 != seqGroup.end())) { //they are both not singletons 
			if (it->second == it2->second) { //they are from the same group so add the distance
				if (numOutputs[it->second] > 30) {
					File::openOutputFileAppend((distFile + "." + toString(it->second) + ".temp"), outFile);
	...
	for (int i = 0; i < numGroups; i++) { //remove old temp files, just in case
		...
		tempDistFiles.push_back(tempDistFile);
		File::remove((inputFile + "." + toString(i) + ".temp"));

		//write out any remaining buffers
		if (numOutputs[i] > 0) {
			File::openOutputFileAppend((distFile + "." + toString(i) + ".temp"), outFile);
			outFile << outputs[i];
			outFile.close();
			outputs[i] = "";
			numOutputs[i] = 0;
	...
	if (ctrlc_pressed) {
		for (int i = 0; i < dists.size(); i++) {
			File::remove((dists[i].begin()->first));
			File::remove((dists[i].begin()->second));
		}
		dists.clear();
	}
...
int SplitMatrix::splitDistanceLarge() {
	...
	while (dFile) {
		...
		if (ctrlc_pressed) { dFile.close();  for (int i = 0;i < numGroups;i++) { if (groups[i].size() > 0) { File::remove((distFile + "." + toString(i) + ".temp")); } } return 0; }
				...
				if (groupIDA != -1 && groupIDB != -1) { //merge distance files of two groups you merged above
					...
					if (groupIDA < groupIDB) {
						...
						if (wroteOutPut[groupIDB]) {
							...
							File::appendFiles(fileName2, fileName);
							...
							if (numOutputs[groupID] > 60) {
								ofstream tempOut;
								File::openOutputFile(fileName, tempOut);
...
int SplitMatrix::splitNames(map<string, int>& seqGroup, int numGroups, vector<string>& tempDistFiles) {
	...
	for (int i = 0;i < numGroups;i++) { File::remove((inputFile + "." + toString(i) + ".temp")); }

	singleton = inputFile + ".extra.temp";
	ofstream remainingNames;
	File::openOutputFile(singleton, remainingNames);

	bool wroteExtra = false;

	ifstream bigNameFile;
	File::openInputFile(inputFile, bigNameFile);

	//grab header line 
	string headers = "";
	if (countfile != "") { headers = File::getline(bigNameFile); File::gobble(bigNameFile); }

	string name, nameList;
	while (!bigNameFile.eof()) {
		bigNameFile >> name >> nameList;
		File::getline(bigNameFile); File::gobble(bigNameFile); //extra getline is for rest of countfile line if groups are given.

		//did this sequence get assigned a group
		it = seqGroup.find(name);

		if (it != seqGroup.end()) {
			File::openOutputFileAppend((inputFile + "." + toString(it->second) + ".temp"), outFile);
		...
		if (fileHandle) {
			...
			if (!fileHandle.eof()) {  //check
				...
				if (countfile != "") {
					...
					File::openOutputFile(newtempNameFile, out);
					out << "Representative_Sequence\ttotal" << endl;
					out.close();
					File::appendFiles(tempNameFile, newtempNameFile);
					File::remove(tempNameFile);
				...
				ifstream in;
				File::openInputFile(tempNameFile, in);

				while (!in.eof()) {
					in >> name >> nameList;  File::gobble(in);
					wroteExtra = true;
					remainingNames << name << '\t' << nameList << endl;
				}
				in.close();
	...
	if (!wroteExtra) {
		...
		out << "Representative_Sequence\ttotal" << endl;
		out.close();
		File::appendFiles(singleton, newtempNameFile);
		File::remove(singleton);
...
vector< map< string, string> > SplitMatrix::getDistanceFiles() {
	try {
		...
		LOG(FATAL) << e.what() << " in SplitMatrix, getDistanceFiles";
		exit(1);
	}
}
/***********************************************************************/
