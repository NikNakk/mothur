...
vector<string> ReadPhylipVector::read(vector< vector<double> >& matrix) {
	...
	ifstream in;
	File::openInputFile(distFile, in);
	...
	if (!m->isContainingOnlyDigits(numTest)) { LOG(LOGERROR) << "expected a number and got " + numTest + ". I suspect you entered a column formatted file as a phylip file, quitting." << '\n'; exit(1); }
