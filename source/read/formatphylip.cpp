...
int FormatPhylipMatrix::read(NameAssignment* nameMap) {
	...
	while ((d = fileHandle.get()) != EOF) {
		...
		if (isalnum(d)) {  //you are square
			...
			File::openInputFile(filename, fileHandle);
			fileHandle >> nseqs; File::gobble(fileHandle);
	...
	if (square == 0) {
		...
		for (int i = 1;i < nseqs;i++) {
			...
			for (int j = 0;j < i;j++) {
				...
				if (ctrlc_pressed) { outTemp.close(); File::remove(tempFile); fileHandle.close();  delete reading; return 0; }
		...
		string outfile = File::getRootName(tempFile) + "sorted.dist.temp";
		...
		if (ctrlc_pressed) { File::remove(tempFile); File::remove(outfile);  delete reading; return 0; }
		...
		while (!in.eof()) {
			if (ctrlc_pressed) { in.close(); out.close(); File::remove(tempFile); File::remove(distFile); File::remove(outfile);  delete reading; return 0; }
		...
		if (ctrlc_pressed) { File::remove(distFile);   delete reading; return 0; }
		...
		for (int i = 0;i < nseqs;i++) {
			...
			for (int j = 0;j < nseqs;j++) {
				if (ctrlc_pressed) { fileHandle.close(); out.close(); File::remove(distFile);   delete reading; return 0; }
