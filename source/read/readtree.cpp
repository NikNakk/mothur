...
ReadTree::ReadTree() {
	...
	LOG(FATAL) << e.what() << " in ReadTree, ReadTree";
	exit(1);
}
}
/***********************************************************************/
int ReadTree::AssembleTrees() {
	try {
		...
		LOG(FATAL) << e.what() << " in ReadTree, AssembleTrees";
		exit(1);
	}
}
/***********************************************************************/
int ReadTree::readSpecialChar(istream& f, char c, string name) {
	try {
		...
		if (d == EOF) {
			LOG(INFO) << "Error: Input file ends prematurely, expecting a " + name + "\n";
			exit(1);
		}
		if (d != c) {
			LOG(INFO) << "Error: Expected " + name + " in input file.  Found " + toString(d) + ".\n";
			exit(1);
		}
		if (d == ')' && f.peek() == '\n') {
			File::gobble(f);
	...
	int ReadTree::readNodeChar(istream& f) {
		...
		if (d == EOF) {
			LOG(INFO) << "Error: Input file ends prematurely, expecting a left parenthesis\n";
			exit(1);
		}
		return d;
	}
	...
	float ReadTree::readBranchLength(istream& f) {
		...
		if (!(f >> b)) {
			LOG(INFO) << "Error: Missing branch length in input tree.\n";
			exit(1);
		}
		File::gobble(f);
		return b;
	...
	int ReadNewickTree::read(CountTable* ct) {
		...
		if ((c = filehandle.peek()) != '#') {
			while ((c = filehandle.peek()) != EOF) {
				if (ctrlc_pressed) { filehandle.close(); return 0; }
				while ((c = filehandle.peek()) != EOF) {
					if (ctrlc_pressed) { filehandle.close(); return 0; }
					// get past comments
					if (c == '[') {
	...
	int ReadNewickTree::readTreeString(CountTable* ct) {
		...
		if (ch == '(') {
			...
			if (lc == -1) { LOG(INFO) << "error with lc" << '\n'; ctrlc_pressed = true; return -1; } //reports an error in reading
			...
			if (rooted != 1) {
				...
				if (rc == -1) { LOG(INFO) << "error with rc" << '\n'; ctrlc_pressed = true; return -1; } //reports an error in reading
				if (filehandle.peek() == ')') {
		...
		//note: treeclimber had the code below added - not sure why?
			...
			if (n != 0) {
				LOG(INFO) << "Internal error: The only taxon is not taxon 0.\n";
				//exit(1);
				readOk = -1; return -1;
			}
			lc = rc = -1;
	...
	int ReadNewickTree::readNewickInt(istream& f, int& n, Tree* T, CountTable* ct) {
		...
		//cout << "child = " << child << endl;		
			...
			if (childrenNodes.size() < 2) { LOG(INFO) << "Error in tree, please correct." << '\n'; return -1; }
			...
			if (f.peek() == ':') {
				...
				if (n >= numNodes) { LOG(INFO) << "Error: Too many nodes in input tree\n";  readOk = -1; return -1; }
			...
			if (group.size() == 0) {
				LOG(INFO) << "Name: " + name + " is not in your groupfile, and will be disregarded. \n";  //readOk = -1; return n1;
