...
TreeReader::TreeReader(string tf, string cf) : treefile(tf), countfile(cf) {
	try {
		...
		LOG(FATAL) << e.what() << " in TreeReader, TreeReader";
		exit(1);
	}
}
/***********************************************************************/
...
bool TreeReader::readTrees() {
	try {
		...
		if (readOk != 0) { LOG(INFO) << "Read Terminated." << '\n';  delete read; ctrlc_pressed = true; return 0; }
		...
		if (numNamesInTree < ct->getNumSeqs()) {
			...
			for (int i = 0; i < namesSeqsCt.size(); i++) {
				...
				if (ctrlc_pressed) { for (int i = 0; i < trees.size(); i++) { delete trees[i]; } return 0; }

				//then you did not find it so report it 
				if (count == m->Treenames.size()) {
					LOG(INFO) << namesSeqsCt[i] + " is in your name or group file and not in your tree. It will be disregarded." << '\n';
		...
		LOG(FATAL) << e.what() << " in TreeReader, readTrees";
		exit(1);
	}
}
/***********************************************************************/
